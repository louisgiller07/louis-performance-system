...
**Contexte historique factuel — jamais un signal d'arbitrage (V0.3_008A, CLOSED / PRODUCTION ROLLOUT COMPLETE)** : `RecentRecoveryContext` (J-1 uniquement — `docs/07_GLOSSARY.md`) applique un principe encore plus strict que la lecture de support ci-dessus : il n'est **jamais** consommé (`SignalTrace.consume()`) et ne mute jamais `DimensionState`/`session`/`decision`. C'est un pur passthrough `RawContext → DailyPlan`, rendu dans une section dédiée, jamais injecté dans `reasoning` (déjà une simple concaténation, voir GAP-003, `docs/12_BACKLOG.md`). L'état du jour courant (check-in) reste l'unique source de vérité pour l'arbitrage — le contexte J-1 ne fait qu'enrichir la présentation athlete-facing d'une adaptation déjà déclenchée par le jour courant, jamais en déclencher une nouvelle à lui seul. Précédence explicite : état du jour courant > contexte récent J-1 > historique plus ancien (non consommé du tout en V0.3_008A).

**Continuité technique — mémoire factuelle, jamais un signal d'arbitrage (V0.3_008B, CLOSED / PRODUCTION ROLLOUT COMPLETE)** : même discipline que `RecentRecoveryContext` ci-dessus, appliquée à la dernière tâche technique valide (`RecentTechnicalContext`/`prior_task_reference` — `docs/07_GLOSSARY.md`) : jamais consommé, ne mute jamais `DimensionState`/`session`/`decision`/`execution_task` du jour, pur passthrough rendu dans un sous-bloc dédié de la carte technique du jour, jamais injecté dans `reasoning`. Distinction supplémentaire propre à ce jalon : `RawContext.recent_technical_context` (ce que le moteur sait, résolu indépendamment de la session finale du jour) vs `DailyPlan.dh_or_technical.prior_task_reference` (ce que le coach a réellement surfacé, uniquement quand la session finale du jour est elle-même DH-family) — la connaissance du moteur n'implique jamais automatiquement une présentation athlete-facing. V1 est explicitement **mémoire**, pas **progression** : aucune règle automatique (répéter/simplifier/progresser selon `technical_outcome`) n'est implémentée — une future décision produit, après dogfood réel, tranchera si une telle règle est justifiée.
...
**`execution_task` — champ optionnel de `dh_or_technical`** : `focus` = ce qui est travaillé (thème/attention), `execution_task` = comment le travailler aujourd'hui, `load_guidance` = comment rouler selon la charge du jour — trois champs distincts, jamais fusionnés. *Corrigé V0.3_008B0* : `focus` et `execution_task` sont des concepts **indépendants** — `execution_task` est toujours la tâche fixe par kind DH (`sessionPrescriptionPolicy.ts#DH_GENERIC_EXECUTION_TASK`), présente dès que la session finale est DH-family, **que `technique_primary_focus` personnel soit configuré ou non** (contrairement au comportement V0.3_006C1 initial, qui la rendait absente dès qu'un focus personnel existait — voir `docs/11_DECISION_LOG.md`, V0.3_008B0, CROSS-SLICE CONTRACT MISMATCH avec V0.3_007C). Invariant anti-inférence inchangé et toujours strict : jamais dérivé du texte libre `technique_primary_focus` personnel — aucune tentative déterministe de "comprendre" un texte libre arbitraire, aucun LLM, aucune inférence par mot-clé/regex/taxonomie ; la tâche reste le texte fixe par kind quel que soit le contenu du focus. `DailyPlanView.tsx` affiche désormais les deux champs avec des labels explicites (« Focus : » / « Tâche du jour : ») pour qu'un athlète ne lise jamais la tâche générique comme une interprétation de son focus personnel.
