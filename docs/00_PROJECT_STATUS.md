...
**Phase actuelle :** **M5 COMPLETE.** M4 reste COMPLETE (client web de production, `https://louis-performance-system.vercel.app`). M5 (longitudinal/pattern evidence/insights) a franchi M5_001 à M5_007 — tous **CLOSED**, voir `docs/11_DECISION_LOG.md` pour le détail milestone par milestone. M5_006C/M5_006D restent implémentation locale uniquement (aucune migration, réutilisent le schéma/RPC déjà déployés par M5_006A/M5_006B). M5_007 (insights déterministes + ledger de revue humaine) est **CLOSED** — le schéma/RPC de revue humaine M5_007 est déployé sur Supabase (`uvolpldwwyvadlamulvr`, 4 migrations, `local=remote=26`, `pending=0`). Le moteur déterministe de génération d'insights est implémenté et testé dans `longitudinal-engine`, mais n'est pas invoqué automatiquement par un runtime de production — aucun scheduler, aucune Edge Function, aucune UI web, aucune intégration `daily-run`. Aucun peuplement automatique de production (`pattern_insight_identities`/`pattern_insight_reviews` = 0/0 sur remote), aucun pattern appris n'influence `daily-run`, `accepted_as_insight` n'active jamais le coaching. **M5 dans son ensemble est COMPLETE.**
...
- **V0.3_001 — Longitudinal Intelligence Runtime + Human Insight Review** : V0.3_001A **CLOSED LOCALLY** (voir DONE ci-dessus) ; V0.3_001B (backfill + durcissement complet remote) et V0.3_001C (`submit-review` + UI Insights) **NOT STARTED**. V0.3_001 dans son ensemble : **IN PROGRESS**.

### NOT STARTED

- **V0.3_001B — Backfill + durcissement complet (remote)** et **V0.3_001C — API `submit-review` + surface web Insights** : **NOT STARTED**. V0.3_001A est CLOSED LOCALLY (voir DONE ci-dessus) ; aucun déploiement Edge remote, aucun backfill historique remote, aucune API de revue, aucune UI Insights à ce jour.
...
11. **V0.3_001B** — backfill + durcissement complet (remote). **NOT STARTED.**

## Prochain milestone

**V0.3_001B — Backfill + durcissement complet (remote).** V0.3_001A **CLOSED LOCALLY, 2026-08-28** (implémentation `d69c81593aff4b25ad58aef3b2ac7c703a7d1d45`, durcissement `a9b50c13f312879b48d0fbe82f3776f5ad88e11b` — orchestrateur, `refresh-longitudinal`, `get-insights`, correction du détecteur recommendation, aucun déploiement remote). Voir `docs/11_DECISION_LOG.md` et `docs/06_ARCHITECTURE.md` pour le détail complet. V0.3_001C (API de revue canonique + surface web Insights) reste **NOT STARTED**. Objectif produit inchangé : rendre utilisable le pipeline M5 déjà construit (`evidence → agrégat → candidat d'insight → revue humaine`), **jamais** au-delà — aucune activation automatique de coaching, aucune intégration `daily-run`.
...
Statut V0.3_001 : **IN PROGRESS** — V0.3_001A **CLOSED LOCALLY, 2026-08-28** (implémentation `d69c81593aff4b25ad58aef3b2ac7c703a7d1d45`, durcissement `a9b50c13f312879b48d0fbe82f3776f5ad88e11b`, aucun déploiement remote) ; V0.3_001B/C **NOT STARTED**. Détail dans `docs/11_DECISION_LOG.md`.
