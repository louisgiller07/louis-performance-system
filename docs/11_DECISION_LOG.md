...
## 2026-08-25 — M5_006A : fondation de persistance des evidences détecteur, `pattern_evidence_identities/_revisions/_source_refs` + `persist_pattern_evidence` + `longitudinal-engine/src/persistence/**` (COMMITTÉ sur origin/main, migrations locales uniquement — non déployé remote)
...
**Régressions confirmées (Supabase disponible)** : `longitudinal-engine` non-DB = **213/213**, suite complète `npm test` = **277/277** (213 + 22 + 32 + 5 + 5 = 277, tous les tests M5_002B/M5_004/M5_005 préexistants inchangés). M1/M2 = **226/226**, edge = **9/9**, M3 HTTP = **26/26**, completed-session unit = **73/73**, completed-session HTTP = **70/70**, web = **242/242**, web build = PASS. M5_006A n'a modifié aucun code `head-coach-engine`/`web` — ces suites étaient déjà vertes, reconfirmées inchangées.
...
**Migrations — locales uniquement, aucune écriture remote** : 5 nouvelles migrations locales (`20260825090000` à `20260825092000`), appliquées via `supabase db reset` local uniquement. Projet lié reconfirmé exactement `uvolpldwwyvadlamulvr` en lecture seule. Comptage : **local = 18, remote = 13** — les 5 nouvelles migrations n'existent délibérément pas encore côté remote (`npx supabase migration list --linked` le confirme, lecture seule, aucun `db push` exécuté) ; ceci est l'état **attendu** à ce stade, pas une dérive de parité.

**Décision** : **M5_006A implémentation VALIDÉE et COMMITTÉE sur `origin/main`** (commit `feat: add append-only pattern evidence ledger`) **— PAS encore CLOSED, PAS encore déployée remote.** Le schéma (`pattern_evidence_identities`/`_revisions`/`_source_refs`), la RPC (`persist_pattern_evidence`, contrat de provenance final `{role, source_kind, source_id}` snake_case), les trois vues, et l'adaptateur de persistance M5_005 (`longitudinal-engine/src/persistence/**`) implémentent l'architecture verrouillée, avec un seul écart empirique documenté et justifié (verrou advisory au lieu de `SELECT ... FOR UPDATE`, pour honorer réellement l'exigence de moindre privilège du même lock). Aucun changement de sémantique M5_004/M5_005. Aucun détecteur supplémentaire, aucune agrégation, aucun scheduler, aucune intégration `daily-run`, aucun changement `head-coach-engine`/`web`/`supabase/functions`. Les 5 migrations locales restent non appliquées côté remote (`local = 18`, `remote = 13`) — la revue de déploiement remote reste une étape distincte, non entamée par ce commit. **M5 dans son ensemble reste EN COURS.**

**Impact** : `supabase/migrations/20260825090000_M5_006A_pattern_evidence_identities.sql` (édité en place lors de la revue finale — jamais appliqué remote, jamais une 6ᵉ migration), `supabase/migrations/20260825090500_M5_006A_pattern_evidence_revisions.sql` (idem), `supabase/migrations/20260825091000_M5_006A_pattern_evidence_source_refs.sql` (inchangé cette revue), `supabase/migrations/20260825091500_M5_006A_persist_pattern_evidence_rpc.sql` (édité en place — verrou advisory, encodage de clé non ambigu), `supabase/migrations/20260825092000_M5_006A_pattern_evidence_views.sql` (inchangé cette revue), `supabase/preflight/pattern_evidence_append_only_security_check.sql`, `longitudinal-engine/src/persistence/**` (`types.ts`, `recommendationVsActualAdapter.ts`, `index.ts` — `errors.ts` supprimé lors de la revue finale, devenu inutile après le correctif d'erreurs non enveloppées), `longitudinal-engine/src/index.ts` (export `persistence/**` ajouté), `longitudinal-engine/tests/unit/persistence/**` (5 tests), `longitudinal-engine/tests/supabase/patternEvidenceSchema.integration.test.ts` (22 tests, dont le test de rollback atomique réécrit sur le vrai chemin RPC), `longitudinal-engine/tests/supabase/persistPatternEvidenceRpc.integration.test.ts` (22 tests), `longitudinal-engine/tests/supabase/patternEvidenceViews.integration.test.ts` (5 tests), `longitudinal-engine/tests/supabase/recommendationVsActualPersistence.integration.test.ts` (5 tests), cette entrée. Aucun changement `head-coach-engine/src/**`, `web/**`, `supabase/functions/**`, `docs/03*`/`05*`/`06*`. `docs/06_ARCHITECTURE.md` explicitement **non modifié** dans ce lot (non autorisé pour cette passe d'implémentation).

**Statut** : implémentation validée et **committée sur `origin/main`**, toutes les preuves empiriques (pures + DB-dépendantes, migrations locales uniquement) au vert, **aucune application remote** ; revue de déploiement remote (5 migrations en attente) et clôture M5_006A restent des étapes distinctes, non entamées ; M5 dans son ensemble reste en cours
