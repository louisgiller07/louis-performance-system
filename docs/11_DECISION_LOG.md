...
## 2026-08-23 — M5_004 : calculateur d'outcomes de décision déterministe, `longitudinal-engine/src/calculators/**` + `src/supabase/outcomeOrchestrator.ts` (LOCAL, non committé, toutes preuves empiriques passées)
...
**Décision** : **M5_004 implémentation locale VALIDÉE, toutes les preuves empiriques (pures et DB-dépendantes) sont vertes — toujours PAS COMPLETE/CLOSED** (non committée, non poussée, aucun déclencheur de production). Le calculateur (`longitudinal-engine/src/calculators/**`) et l'orchestrateur de persistance (`src/supabase/outcomeOrchestrator.ts`) implémentent l'architecture verrouillée : ancrage `decisionDate`, quatre états d'exécution bruts jamais interprétés comme adhérence, invariant de couverture de timeline, validation `observedThroughDate` en tout premier dans l'orchestrateur (avant tout pré-filtre de maturité), snapshot immuable/source éditable, aucun déclencheur, aucune migration, aucun changement `head-coach-engine`/M5_001–003. **M5 dans son ensemble reste en cours, non complet** — M5_004 n'implémente aucune sémantique d'interprétation M5_005 (détecteurs, evidence, recommandation-vs-réel), aucun changement Safety, et ne garantit aucun peuplement automatique en production tant qu'aucun déclencheur n'est câblé.
...
**Statut** : implémentation locale validée, **toutes les preuves empiriques passées (pures + DB-dépendantes)**, **non committée, non poussée, aucun déclencheur/déploiement production** ; M5 dans son ensemble reste en cours
