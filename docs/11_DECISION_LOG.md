...
## 2026-08-27 — M5_006D : durcissement local des preuves d'agrégation (`patternEvidenceAggregation.integration.test.ts` uniquement), aucun changement de production

**Contexte** : la revue avant clôture a signalé deux lacunes dans les preuves d'intégration de l'implémentation initiale. (1) Le test « tête-de-révision-uniquement » partageait `DETECTOR_ID`/`DETECTOR_VERSION` avec le test de cycle de vie T1→T2→T3 — il pouvait seulement prouver que l'identité X elle-même n'apparaît qu'une fois (révision 2), jamais le contrat complet verrouillé de l'agrégat (`evidenceCount`/`supportingCount`/`contradictingCount`/`neutralCount`/`directionalEvidenceCount`/ratios/`evidenceBalance`/`sourceEvidenceRefs`) pour une population isolée, puisque l'evidence active du test de cycle de vie polluait le même groupe `(detectorRuleId, detectorRuleVersion)`. (2) Le filtrage de plage par date et l'isolation par athlète de l'adaptateur (`getCurrentEffectivePatternEvidence`) n'étaient jusqu'ici prouvés qu'indirectement (implicitement via `RANGE` fixe et un seul athlète) — jamais par un test dédié contre la vraie vue `pattern_evidence_current_effective`.

**Correctifs — tous confinés à un seul fichier de test** :
1. **Tête-de-révision-uniquement, isolée** : nouveau `detectorRuleId` dédié (`revhead_isolated_detector_*`) via les nouvelles aides paramétrées `persistActiveFor`/`findAggregate(aggregates, detectorRuleId)`. Preuve désormais complète : `evidenceCount=1, supportingCount=0, contradictingCount=1, neutralCount=0, directionalEvidenceCount=1, supportingRatio=0, contradictingRatio=1, neutralRatio=0, evidenceBalance=contradicting_only, sourceEvidenceRefs.length=1` (révision 2, `contradicting`), et assertion explicite qu'aucune référence à la révision 1 (superseded) n'apparaît nulle part.
2. **Filtrage de plage — nouvelle preuve DB dédiée** : détecteur isolé, 5 lignes couvrant avant/à-`fromDate`/intérieur/à-`toDate`/après ; l'adaptateur retourne exactement les 3 lignes dans les bornes (bornes inclusives incluses), les 2 hors-plage absentes ; les 3 lignes retournées s'agrègent ensuite sans erreur (le filtrage côté serveur garantit déjà l'invariant de plage de l'agrégateur pur).
3. **Isolation par athlète — nouvelle preuve DB dédiée** : second athlète de test (`athleteB`, `decisionIdB`, tous deux scaffoldés dans `beforeAll`), même `detectorRuleId`/`detectorRuleVersion`/plage pour les deux athlètes ; interroger pour A ne retourne jamais les lignes de B et réciproquement.

**Aucun bug de production trouvé** : `aggregateEffectivePatternEvidence.ts` (33 tests unitaires purs, inchangés), `patternEvidenceAggregationAdapter.ts`, `rowMapping.ts` restent strictement inchangés — le contrat pur d'agrégation était déjà correct ; seules les preuves d'intégration étaient insuffisamment isolées/incomplètes.

**Preuves empiriques** : suite `longitudinal-engine` complète après un reset local frais (`supabase db reset --no-seed`) = **510/510**, reconfirmé identique sur une seconde exécution consécutive sur ce même état, build = PASS. Suite unitaire d'agrégation inchangée = **33/33**. Suite d'intégration M5_006D = **5/5** (T1→T2→T3 cycle de vie, tête-de-révision-uniquement isolée, filtrage de plage, isolation par athlète, exclusivité de portée de l'adaptateur — contre 3/3 avant durcissement). Régressions gelées reconfirmées : M5_004=59/59, résolveur=15/15, M5_005=31/31, M5_006C détecteur=50/50, adaptateur=10/10, M1/M2=226/226, web=242/242, web build=PASS.

**Migrations — inchangées** : zéro migration ajoutée. `local = remote = 22`, `pending = 0`.
...
## 2026-08-27 — M5_007 : insights déterministes + ledger de revue humaine, `longitudinal-engine/src/insights/**` + `pattern_insight_identities`/`pattern_insight_reviews` (LOCAL uniquement — 4 migrations locales, non déployées remote, non clos)

**Contexte** : M5_006D a livré l'agrégation déterministe d'evidence effective (`PatternEvidenceAggregate`). M5_007 ajoute la couche suivante et **dernière** du pipeline verrouillé : `evidence effective → agrégat M5_006D → candidat d'insight M5_007 → revue humaine`. **S'arrête strictement là** : M5_007 ne modifie jamais le coaching, ne modifie jamais `daily-run`, ne modifie jamais Safety, n'active jamais un pattern, ne crée aucune personnalisation automatique, aucun score de confiance/significativité, n'infère aucune causalité, n'utilise aucun LLM, et n'invente aucun seuil de « revoyabilité » — **chaque agrégat produit exactement un candidat**, y compris un agrégat à une seule evidence.

**Registre verrouillé — exactement 3 paires détecteur/version** : `recommendation_vs_actual_execution@1.0.0 → recommendation_execution_alignment`, `sleep_quality_to_same_day_energy_correlation@1.0.0 → sleep_energy_same_day_association`, `pain_persistence_across_recent_checkins@1.0.0 → pain_persistence_between_recent_checkins`. Un détecteur/version hors de ce registre fait échouer fort `UnsupportedPatternInsightProjectorError` — aucun fallback générique. `insightProjectorVersion` verrouillé à `"1.0.0"`.

**Mapping de direction — exact, sans autre calcul** : `supporting_only`/`supporting_majority → supporting` ; `contradicting_only`/`contradicting_majority → contradicting` ; `balanced → mixed` ; `neutral_only → neutral`. Aucun seuil.

**Copie humaine — verrouillée verbatim** (`src/insights/registry.ts`) : titre + une formulation par direction + caveats, pour chacun des 3 `insightKind`. Le caveat de `pain_persistence_between_recent_checkins` rappelle explicitement que l'insight « ne remplace jamais les règles Safety, un diagnostic ou un avis professionnel ».

**`PatternInsightSnapshot` — exactement 23 champs verrouillés** (`insightProjectorVersion`/`athleteId`/`insightKind`/`detectorRuleId`/`detectorRuleVersion`/`rangeFromDate`/`rangeToDate`/`direction`/`title`/`statement`/`caveats`/`evidenceCount`/`supportingCount`/`contradictingCount`/`neutralCount`/`directionalEvidenceCount`/`supportingRatio`/`contradictingRatio`/`neutralRatio`/`evidenceBalance`/`firstEventDate`/`lastEventDate`/`sourceEvidenceRefs`) — `sourceEvidenceRefs` transmis verbatim depuis l'agrégat M5_006D d'origine, jamais recalculé.

**`PatternInsightReviewDecision` — exactement 3 valeurs** : `accepted_as_insight` | `dismissed` | `needs_more_evidence`. Aucune valeur `active`/`activated`/`applied_to_coaching`/`confidence` — `accepted_as_insight` enregistre **uniquement** qu'un humain a accepté la formulation/le cadrage de l'insight, jamais une activation de coaching. Aucun runtime de ce schéma ne lit `pattern_insight_reviews` pour changer le comportement de `daily-run`.

**État de revue du candidat — dérivé, jamais persisté lui-même** : `unreviewed` (aucune revue courante pour l'identité) ; `reviewed_current` (une revue courante existe ET toutes les dimensions d'empreinte du `candidateSnapshot` stocké correspondent exactement au snapshot fraîchement construit — `insightProjectorVersion`/`athleteId`/`insightKind`/`detectorRuleId`/`detectorRuleVersion`/`rangeFromDate`/`rangeToDate`/`sourceEvidenceRefs`, égalité profonde sensible à l'ordre) ; `reviewed_stale` sinon. Une décision `accepted_as_insight` devenue périmée (plage déplacée, ensemble d'evidence changé, révision supersédée, version de projecteur changée) est rapportée telle quelle pour contexte mais **jamais silencieusement requalifiée** en `reviewed_current`.

**API pure** : `buildPatternInsightCandidates({aggregates, currentReviews})` — aucun Supabase, aucune horloge, aucun aléatoire, aucun LLM. Ordonnancement déterministe : `detectorRuleId` ASC puis `detectorRuleVersion` ASC puis `insightKind` ASC, invariant au mélange de l'entrée. `aggregates=[] → []`.

**Adaptateurs Supabase** : `SupabasePatternInsightReviewAdapter.getCurrentPatternInsightReviews(athleteId)` — lecture seule, exclusivement `pattern_insight_review_current`, jamais `pattern_insight_review_history` directement. `persistPatternInsightReview(client, {athleteId, candidate, decision, reviewerNote})` — persiste **toujours** `candidate.snapshot` comme `candidate_snapshot` canonique ; un appelant ne peut jamais fournir un snapshot arbitraire déconnecté du candidat réellement projeté. Erreurs RPC propagées non enveloppées (même discipline que tous les adaptateurs existants).

**Schéma — 4 migrations additives, zéro modification de schéma existant** :
1. `pattern_insight_identities` (unique `(athlete_id, detector_rule_id, detector_rule_version, insight_kind)`, FK `athlete_id → athletes(id) ON DELETE RESTRICT`) + `pattern_insight_reviews` (chaîne `review_number`/`supersedes_id` numérotée par identité, `decision`, `candidate_snapshot jsonb not null` objet opaque, `reviewer_note` optionnel 1-2000 caractères trimés) — RLS activé, `authenticated`=SELECT seul, `service_role`=SELECT+INSERT seuls, `anon`=zéro.
2. Intégrité : trigger de chaîne de prédécesseur pour `pattern_insight_reviews` (miroir exact de `check_pattern_evidence_revision_predecessor`, M5_006A) + `reject_append_only_mutation()` (déjà existante, réutilisée verbatim, jamais redéfinie) attachée aux 4 triggers UPDATE/DELETE des deux tables.
3. `persist_pattern_insight_review` — RPC atomique, `SECURITY INVOKER`, `search_path=public`, `EXECUTE` réservé à `service_role`. Verrouillage par `pg_advisory_xact_lock` avec le même encodage de tuple préfixé-par-longueur que `persist_pattern_evidence`/`transition_pattern_evidence_lifecycle` (jamais `SELECT ... FOR UPDATE`). Comportement exact : identité absente → création + revue #1 (`inserted`) ; décision + snapshot (égalité `jsonb` native) + note identiques à la tête courante (`IS NOT DISTINCT FROM` pour la note, NULL=NULL compte comme identique) → `unchanged` ; sinon → nouvelle revue supersédant la tête (`superseded`). Aucun `UPDATE` jamais émis par cette fonction.
4. `pattern_insight_review_current` (tête de revue par identité, `DISTINCT ON`) + `pattern_insight_review_history` (chaîne complète) — les deux `security_invoker=true`.

**Preuves empiriques (vraie DB)** : matrice de comportement RPC complète (insertion/inchangé/supersession × changement de décision/note/snapshot, chaîne dense de 3 revues) ; chaîne de prédécesseur × 4 cas malformés rejetés ; append-only UPDATE/DELETE rejeté sur les deux tables ; moindre privilège (`service_role`=SELECT+INSERT, `authenticated`=SELECT, `anon`=zéro) ; RLS activé + preuve par vrais clients authentifiés (propriétaire voit sa revue, INSERT direct rejeté, athlète croisé invisible, `anon` sans accès) ; propriétés de sécurité de la RPC par introspection SQL directe (`prosecdef=false` = SECURITY INVOKER, `EXECUTE` accordé à `service_role` seul, corps de fonction sans `FOR UPDATE`, présence de `pg_advisory_xact_lock`) ; concurrence (revues identiques concurrentes → une seule tête logique, aucun `review_number` dupliqué ; revues différentes concurrentes → chaîne #1/#2 sérialisée, aucune violation d'unicité) ; vues `current`/`history` (tête exacte, séquence complète).

**Tests** : 45 unitaires purs (3 projecteurs supportés, échec fort hors-registre × 3, matrice complète des 6 `evidenceBalance` → direction, copie exacte × 3 `insightKind` × 4 directions + caveats, absence de seuil, forme exacte du snapshot à 23 clés, ordonnancement déterministe + invariance au mélange, dérivation d'état de revue × 6 dont 3 causes de péremption distinctes, non-activation par décision de revue), 36 intégration (comportement RPC, schéma/intégrité, sécurité RPC, concurrence, vues, RLS). **Total M5_007 = 81.** Suite `longitudinal-engine` complète = **591/591** (510 + 81), premier passage au vert. Régressions gelées reconfirmées : M5_004=59/59, résolveur=15/15, M5_005=31/31, M5_006C détecteur=50/50, adaptateur=10/10, intégration=3/3, M5_006D agrégation unitaire=33/33, intégration=5/5, M1/M2=226/226, edge=9/9, M3 HTTP=26/26, completed-session unit=73/73, completed-session HTTP=70/70, web=242/242, web build=PASS. `npm run build` (longitudinal-engine + head-coach-engine) : succès propre.

**Migrations — 4 ajoutées, additives uniquement** : `20260827090000`/`090500`/`091000`/`091500`. `local = 26`, `remote = 22`, `pending = 4`. Aucune migration existante modifiée. Aucun `db push` remote dans cette passe.

**Décision** : **M5_007 implémentation locale VALIDÉE — PAS COMPLETE/CLOSED, aucune écriture remote, aucun déploiement.** Le projecteur d'insight pur (`buildPatternInsightCandidates.ts`) et le ledger de revue humaine (schéma + RPC + vues + adaptateurs) sont implémentés, testés et verrouillés dans `longitudinal-engine`. Aucun déploiement Edge Function, aucun déploiement Vercel, aucun scheduler, **aucun calcul d'insight ni revue automatique en production, aucune intégration `daily-run`**. Aucun pattern appris n'influence `daily-run` en M5. Safety A1-A5 inchangée. M1-M4 restent frozen. **M5 dans son ensemble reste EN COURS.**

**Impact** : `longitudinal-engine/src/insights/**` (nouveau : `types.ts`/`errors.ts`/`registry.ts`/`buildPatternInsightCandidates.ts`/`index.ts`), `longitudinal-engine/src/supabase/patternInsightReviewAdapter.ts` (nouveau), `longitudinal-engine/src/persistence/insightReviewAdapter.ts` (nouveau), `longitudinal-engine/src/supabase/rowMapping.ts` (`mapPatternInsightReviewCurrentRow` + allow-lists ajoutés), `longitudinal-engine/src/{index,supabase/index,persistence/index}.ts` (exports ajoutés), `longitudinal-engine/tests/unit/insights/buildPatternInsightCandidates.test.ts` (45 tests), `longitudinal-engine/tests/supabase/patternInsightReviewLedger.integration.test.ts` (36 tests), 4 migrations `supabase/migrations/2026082709{00,05,10,15}00_M5_007_*.sql`, cette entrée. Aucun changement `head-coach-engine/src/**`, `web/**`, `supabase/functions/**`, `docs/03*`/`05*`/`06*`.

**Statut** : implémentation locale validée, toutes les preuves empiriques (pures + DB-dépendantes) au vert dès le premier passage, **4 migrations locales additives, aucune écriture remote, aucune clôture** ; revue de clôture M5_007 reste une étape distincte, non entamée ; M5 dans son ensemble reste en cours.

---

## 2026-08-27 — M5_007 : durcissement local de l'intégration adaptateur + preuves trigger-level, avant revue de rollout remote (LOCAL uniquement, aucun déploiement)

**Contexte** : la revue avant rollout remote a signalé un écart réel — les 36 tests d'intégration de l'implémentation initiale prouvaient la RPC/le schéma/les vues en appelant `client.rpc(...)`/`client.from(...)` directement, jamais les adaptateurs TypeScript de production (`SupabasePatternInsightReviewAdapter`/`persistPatternInsightReview`) eux-mêmes — cette couche restait donc entièrement non testée. De même, les preuves append-only existantes passaient par le client `service_role` (qui n'a de toute façon aucun privilège UPDATE/DELETE), prouvant le moindre privilège mais jamais le trigger lui-même — un GRANT futur trop large aurait pu silencieusement contourner ces tests sans que rien ne le détecte.

**Preuves adaptateur ajoutées** :
1. **Adaptateur de lecture, unitaire** (`tests/unit/supabase/patternInsightReviewAdapter.test.ts`, client Supabase factice miroir de `persistDailyRun.test.ts`) : interroge **exclusivement** `pattern_insight_review_current` (un seul appel `.from()`, jamais `_history`/`pattern_insight_reviews`/`pattern_insight_identities`/aucune vue `pattern_evidence_*`), applique `athlete_id = athlete demandé` via `.eq()`, mappe les lignes en camelCase, et **lance** (ne convertit jamais silencieusement en `[]`) une erreur de requête.
2. **Adaptateur d'écriture, unitaire** (`tests/unit/persistence/insightReviewAdapter.test.ts`, `stubClient` miroir de `painPersistenceAdapter.test.ts`) : la charge RPC exacte est dérivée de `candidate.snapshot` (`p_detector_rule_id`/`p_detector_rule_version`/`p_insight_kind` proviennent tous du snapshot, jamais d'un paramètre séparé) ; `p_candidate_snapshot` **est** `candidate.snapshot` lui-même (`toBe`, pas `toEqual`) — un appelant ne peut jamais substituer un snapshot arbitraire ; les 3 décisions et `reviewerNote` null/non-null sont couverts ; une erreur RPC sentinelle traverse **non enveloppée** (`rejects.toBe(sentinelError)`).
3. **Chaîne bout-en-bout, vraie DB, vrais adaptateurs de production** (`tests/supabase/patternInsightReviewAdapter.integration.test.ts`, nouveau fichier) : `buildPatternInsightCandidates` → `persistPatternInsightReview` (jamais `client.rpc` directement) → `SupabasePatternInsightReviewAdapter.getCurrentPatternInsightReviews` (jamais `client.from` directement) → `buildPatternInsightCandidates` reconstruit avec les revues réellement lues → `reviewState=reviewed_current`, `currentReview.decision=accepted_as_insight`, tous les champs reconstruits identiques. Puis une révision plus récente simulée (seuls `revisionId`/`revisionNumber` de `sourceEvidenceRefs` changent) reconstruite avec les **mêmes** `currentReviews` → `reviewState=reviewed_stale`, la décision `accepted_as_insight` antérieure jamais requalifiée automatiquement. Isolation par athlète prouvée via l'adaptateur de lecture réel (jamais uniquement via RLS) dans les deux sens.

**Preuves trigger-level ajoutées** (`describe("append-only — trigger-level...")`) : DML direct via `psql` sous le rôle superutilisateur `postgres` (contourne RLS **et** tous les GRANT) contre les deux tables — UPDATE/DELETE échouent avec le message exact `append-only violation: ... on pattern_insight_identities|pattern_insight_reviews`, prouvant le trigger `reject_append_only_mutation()` lui-même, indépendamment de tout privilège de rôle. Les preuves de moindre privilège existantes (`service_role`/`authenticated`/`anon`) sont conservées intactes en complément — les deux couches (privilège ET trigger) sont maintenant prouvées séparément.

**Durcissement des formes de schéma verrouillées** : `candidate_snapshot` — objet JSON accepté, tableau/chaîne/nombre/`null` JSON/`NULL` SQL tous rejetés (6 cas) ; `review_number = 0` rejeté ; `reviewer_note` — `NULL` accepté, 1 caractère accepté, exactement 2000 caractères accepté, 2001 rejeté, espace en tête/en fin rejeté, chaîne vide rejetée (7 cas) ; absence de colonne `updated_at` confirmée sur les deux tables ; les 3 FK (`pattern_insight_identities.athlete_id`, `pattern_insight_reviews.insight_identity_id`, `pattern_insight_reviews.supersedes_id`) confirmées `ON DELETE RESTRICT` (`confdeltype='r'`) par introspection `pg_constraint` ; unicité naturelle `(athlete_id, detector_rule_id, detector_rule_version, insight_kind)` et `(insight_identity_id, review_number)` prouvées par tentative de violation directe ; index unique partiel sur `supersedes_id` confirmé par introspection `pg_indexes`. RPC : `proconfig` contient bien `search_path=public` (introspection directe, absente des preuves initiales) ; encodage du verrou consortial confirmé identique (composants `p_athlete_id`/`p_detector_rule_id`/`p_detector_rule_version` préfixés par longueur, `hashtextextended`).

**Aucun bug de production trouvé** : les 4 migrations M5_007 satisfont déjà tous les contrats verrouillés ci-dessus sans modification — `git diff --stat -- supabase/migrations/` est vide pour cette passe. `SupabasePatternInsightReviewAdapter`, `persistPatternInsightReview`, `rowMapping.ts` restent strictement inchangés — le contrat de production était déjà correct ; seule la couverture de test de la couche adaptateur/trigger était insuffisante.

**Tests — état final** : projecteur d'insight unitaire = **45/45** (inchangé). Adaptateurs unitaires (lecture + écriture) = **19/19** (nouveau). Ledger d'intégration = **61/61** (36 existants + 25 nouveaux : verrou consortial détaillé, `search_path`, 4 preuves trigger-level, 19 preuves de forme de schéma verrouillée). Adaptateur bout-en-bout d'intégration = **2/2** (nouveau fichier). **Total M5_007 = 127** (contre 81 avant durcissement). Suite `longitudinal-engine` complète après un reset local frais (`supabase db reset --no-seed`, 26 migrations appliquées) = **637/637** (591 + 46), premier passage au vert, aucune relance. Régressions gelées reconfirmées : M5_004=59/59, résolveur=15/15, M5_005=31/31, M5_006C détecteur=50/50, adaptateur=10/10, intégration=3/3, M5_006D agrégation unitaire=33/33, intégration=5/5, M1/M2=226/226, edge=9/9, M3 HTTP=26/26, completed-session unit=73/73, completed-session HTTP=70/70, web=242/242, web build=PASS. `npm run build` (longitudinal-engine + head-coach-engine) : succès propre.

**Migrations — inchangées** : zéro migration ajoutée ou modifiée dans cette passe. `local = 26`, `remote = 22`, `pending = 4` (les 4 mêmes migrations M5_007 : `20260827090000`/`090500`/`091000`/`091500`). Aucun `db push` remote.

**Portée** : `longitudinal-engine/tests/**` uniquement (1 fichier modifié, 3 fichiers nouveaux), plus cette entrée. Aucun changement `longitudinal-engine/src/**`, `head-coach-engine/src/**`, `web/**`, `supabase/functions/**`, `docs/03*`/`05*`/`06*`, aucune migration.

**Statut** : durcissement local validé, **zéro migration ajoutée/modifiée, aucune écriture remote, aucun déploiement, aucune clôture** ; revue de rollout remote M5_007 reste une étape distincte, non entamée ; M5 dans son ensemble reste en cours.

---

## 2026-08-27 — M5_007 : CLOSED (implémentation + durcissement + rollout remote), M5 dans son ensemble COMPLETE

**Contexte** : les deux commits M5_007 (`c54e9802` implémentation, `6e802910` durcissement) sont poussés sur `origin/main`. Le rollout remote a ensuite été exécuté depuis le HEAD propre `6e80291063c2f707d750d7eb0df36b1b3198e720`, sans nouveau commit ni modification du dépôt. Il a appliqué exactement les 4 migrations M5_007 sur `uvolpldwwyvadlamulvr` via un unique `supabase db push`, puis la revue post-rollout read-only a reconfirmé les contrats déployés sans écart.

**Objets remote déployés** : enum `pattern_insight_review_decision` (`accepted_as_insight`/`dismissed`/`needs_more_evidence`, ordre exact confirmé) ; tables `pattern_insight_identities`/`pattern_insight_reviews` (colonnes exactes, aucune `updated_at`, 3 FK `ON DELETE RESTRICT` confirmées `confdeltype='r'`, unicité naturelle + `(insight_identity_id, review_number)` + index unique partiel sur `supersedes_id` tous confirmés par introspection `pg_indexes`/`pg_constraint`) ; RPC `persist_pattern_insight_review` (`prosecdef=false` = SECURITY INVOKER, `proconfig=[search_path=public]`, `EXECUTE` accordé à `service_role` seul, corps confirmé contenant `pg_advisory_xact_lock` avec encodage préfixé-par-longueur sur `athlete_id`/`detector_rule_id`/`detector_rule_version`/`insight_kind`, jamais `FOR UPDATE`) ; vues `pattern_insight_review_current`/`pattern_insight_review_history` (les deux `security_invoker=true`, colonnes exactes). RLS activée sur les deux tables, policies scopées au propre athlète via `auth.uid()`, grants exacts (`authenticated`=SELECT seul, `service_role`=SELECT+INSERT seuls, `anon`=zéro). Ledger remote confirmé vide après déploiement : `pattern_insight_identities`/`pattern_insight_reviews` = 0/0 — aucune ligne de démonstration créée. Objets M5 préexistants (`pattern_evidence_identities`/`_revisions`/`_source_refs`/`_lifecycle_transitions`, `persist_pattern_evidence`, `transition_pattern_evidence_lifecycle`, `persist_active_pattern_evidence`, `pattern_evidence_current_effective`) confirmés inchangés.

**Tests — état final** : projecteur d'insight = **45/45**, adaptateurs unitaires (lecture + écriture) = **19/19**, ledger d'intégration = **61/61**, adaptateur bout-en-bout d'intégration = **2/2**. **Total M5_007 = 127.** Suite `longitudinal-engine` complète = **637/637**. Régressions gelées reconfirmées : M5_004=59/59, résolveur=15/15, M5_005=31/31, M5_006C détecteur=50/50, adaptateur=10/10, intégration=3/3, M5_006D agrégation unitaire=33/33, intégration=5/5, M1/M2=226/226, edge=9/9, M3 HTTP=26/26, completed-session unit=73/73, completed-session HTTP=70/70, web=242/242, web build=PASS. Le rollout remote lui-même n'a produit aucun nouveau commit de code/test — ces chiffres sont ceux du durcissement, reconfirmés inchangés après rollout.

**Migrations — 4 déployées** : `20260827090000`/`090500`/`091000`/`091500`. `local = remote = 26`, `pending = 0`.

**Décision** : **M5_007 = CLOSED.** Le schéma/RPC de revue humaine M5_007 est déployé sur Supabase (`uvolpldwwyvadlamulvr`). Le moteur déterministe de génération d'insights (`buildPatternInsightCandidates.ts`) et ses adaptateurs de lecture/écriture sont implémentés, testés et verrouillés dans `longitudinal-engine`, mais **ne sont invoqués par aucun runtime distant, aucun scheduler, aucune Edge Function, aucune UI web, aucune intégration `daily-run`**. `accepted_as_insight` enregistre uniquement une acceptation humaine de la formulation de l'insight — jamais une activation de coaching. Aucun pattern appris n'influence `daily-run`. Safety A1-A5 inchangée. M1-M4 restent frozen.

**M5 — audit de statut global** : M5_007 est le dernier jalon M5 explicitement défini dans les documents canoniques. Aucun jalon `M5_008` ni `M5_xxx` ultérieur n'existe dans la feuille de route canonique. `docs/00_PROJECT_STATUS.md` nommait M5_007 comme unique jalon M5 restant avant cette clôture. `docs/12_BACKLOG.md` ne définit aucun jalon M5 supplémentaire après M5_007 (le backlog n'a d'ailleurs jamais eu de section `## M5` dédiée, contrairement à M1/M3/M4 — le suivi M5 s'est fait exclusivement via la chaîne séquentielle d'entrées de ce journal : M5_001A/B → M5_002A/B → M5_003 → M5_004 → M5_005 → M5_006A/B/C/D → M5_007). Les travaux futurs d'activation / patterns appris / catégories P1/P2 n'existent qu'en tant que catégories d'enrichissement futures non planifiées — aucune ne porte à ce jour un identifiant `M5_xxx` restant. Donc : M5_007 CLOSED + aucun jalon M5 défini restant = **M5 COMPLETE.** Aucun prochain jalon concret n'est actuellement défini dans les documents canoniques ; `docs/12_BACKLOG.md` §P1/§P2 liste des catégories d'enrichissement candidates non architecturées, à titre indicatif uniquement.

**Impact** : aucun changement de code dans cette entrée (revue de clôture uniquement) ; `docs/00_PROJECT_STATUS.md`, `docs/12_BACKLOG.md`, cette entrée.

**Statut** : **CLOSED.** Commits : implémentation `c54e980296f5a30fc6e916d7c8a94ddf14c86fdb` (`feat: add pattern insights and human review ledger`) ; durcissement `6e80291063c2f707d750d7eb0df36b1b3198e720` (`test: harden M5_007 review integration`). Le rollout remote a été exécuté ensuite depuis ce HEAD propre, sans commit supplémentaire. Déploiement remote schéma/RPC uniquement — aucun déploiement Edge Function, aucun déploiement Vercel, aucune intégration `daily-run`. **M5 dans son ensemble : COMPLETE.**

---

## 2026-08-27 — V0.3_001 : verrouillage d'architecture — Longitudinal Intelligence Runtime + Human Insight Review (ARCHITECTURE LOCKED, NOUVELLE IMPLÉMENTATION RUNTIME NON COMMENCÉE)

**Statut de cette entrée : ARCHITECTURE V0.3_001 LOCKED — NEW V0.3_001 RUNTIME IMPLEMENTATION NOT STARTED.** Cette entrée enregistre une décision d'architecture approuvée avant tout travail d'implémentation. **Les briques M5 sous-jacentes existent déjà selon leur statut canonique** : calculateur d'outcomes + RPC `persist_decision_outcome` déployés (M5_001B/M5_004) ; ledger d'evidence append-only déployé (M5_006A) ; cycle de vie actif/retiré + ses RPC/vues déployés (M5_006B) ; ledger de revue humaine + RPC `persist_pattern_insight_review` + vues déployés (M5_007). L'agrégation déterministe (M5_006D) et le projecteur d'insight déterministe (M5_007) sont une logique TypeScript pure, implémentée et testée dans `longitudinal-engine`, sans migration ni objet DB propre à ces deux milestones. **En revanche, aucun des nouveaux éléments runtime V0.3_001 décrits ci-dessous n'est encore implémenté** : aucun orchestrateur opérationnel, aucune nouvelle opération serveur, aucune correction du détecteur recommendation-vs-actual, aucun backfill V0.3_001, aucune API `submit-review` V0.3_001 et aucune UI Insights.

**Objectif produit** : rendre utilisable le pipeline M5 déjà construit — `evidence effective → agrégat M5_006D → candidat d'insight M5_007 → revue humaine`, **jamais** au-delà. Interdits explicites et verrouillés : insight/revue → `daily-run`, activation automatique de coaching par pattern appris, score de confiance/signification, causalité inférée, autorité décisionnelle LLM. Safety A1-A5 inchangée, comportement de décision quotidienne M1-M4 frozen — `daily-run` n'est modifié en aucune façon par cette architecture.

**Modèle de dates — deux concepts strictement séparés** :
1. `insightAggregationRange` — constante de domaine **statique, immuable, jamais dérivée de l'horloge** : `ALL_HISTORY_RANGE = {fromDate: "1900-01-01", toDate: "9999-12-31"}`. Choix vérifié empiriquement contre le parseur canonique verrouillé de M5_006D (`parseCanonicalDateUtc`) : `1900-01-01` et `9999-12-31` passent tous deux le round-trip `Date.UTC`/`getUTC*` ; une année dans la zone spéciale JS 0-99 (testé `0099-12-31`) échoue au round-trip (réinterprétée comme 1999), confirmant que `1900` n'est pas un choix stylistique mais évite une vraie limitation du moteur JS. `9999` est le plafond réel du format `YYYY-MM-DD` à 4 chiffres exact déjà verrouillé. Comparaison lexicographique valide sur toute la plage (format zéro-préfixé uniforme). `aggregateEffectivePatternEvidence.ts` n'est **pas modifié** — la plage statique est une politique runtime/produit fournie en argument, jamais un défaut caché dans le contrat M5_006D.
2. `longitudinalProcessingDate` — date calendaire courante **côté serveur uniquement**, fuseau produit V1 fixe `Europe/Zurich` (aucune colonne fuseau par athlète n'existe dans le schéma — limitation V1 explicite, documentée, non résolue ici). Utilisée **exclusivement** comme `observedThroughDate` pour `calculateAndPersistOutcomes` (maturité J+1/J+3/J+7). N'entre **jamais** dans `PatternInsightSnapshot`, dans l'empreinte de fraîcheur d'une revue, ni dans une identité de candidat. Un navigateur ne fournit jamais cette date (contrairement au paramètre `date` de `daily-run` : `decision_outcomes` n'a aucun mécanisme de supersession contrairement à `pattern_evidence_*`, donc une date future forgée y créerait une ligne prématurée et permanente — risque réel, pas hypothétique, écarté en gardant cette date entièrement serveur).

**Correction de persistance — `recommendation_vs_actual_execution@1.0.0`** : audit direct du code confirme que le résultat `no_evidence` actuel ne porte pas de `evidenceKey` (celui de la branche `evidence` dépend de `completedSessionId`, absent par construction dans `no_evidence`), rendant impossible d'identifier l'identité `pattern_evidence_identities` antérieure sans requête non canonique. Correction verrouillée : `evaluationKey = evidenceKey = decision:<decisionId>` pour les deux branches (identité unique par décision, plus correcte que le design actuel qui pourrait fragmenter une même décision en plusieurs identités si le lien `completed_session` change). `no_evidence` déclenche alors `transition_pattern_evidence_lifecycle` (`target=withdrawn`, `reason_code = detection.reason` mappé directement, `context = {}`), miroir exact des adaptateurs sommeil-énergie/persistance-douleur déjà existants — aucune logique de cycle de vie dupliquée, aucun `SELECT`-puis-transition applicatif, aucune nouvelle migration (`transition_pattern_evidence_lifecycle` existe déjà, générique, déployée depuis M5_006B). `detector_rule_version` reste `1.0.0` — correction structurelle d'ancrage de persistance, jamais un changement de sémantique de classification (matrice supporting/neutral/contradicting inchangée). **Cette correction est approuvée mais non implémentée.** **Précondition d'implémentation stricte pour V0.3_001A** : vérification remote read-only que `pattern_evidence_identities` count = 0 pour `(recommendation_vs_actual_execution, 1.0.0)` avant tout changement de code ; si non nul, arrêt, retour pour décision d'architecture/versionnement, aucune réécriture de ligne historique, aucune rétention silencieuse de `1.0.0`.

**Modèle d'exécution** : on-demand, aucun scheduler initialement. Trois opérations conceptuelles, **aucune implémentée** (chemins HTTP non gelés) : `refresh-longitudinal` (écriture, idempotente, orchestre outcomes + les 3 détecteurs existants), `get-insights` (lecture, calcul de candidat **côté serveur uniquement**, aucune table de persistance de candidat), `submit-review` (écriture humaine explicite uniquement, jamais automatique).

**Frontière serveur/authentification** : réutilise conceptuellement le motif déjà établi par `daily-run` (JWT → frontière authentifiée → athlète propre via RLS → `service_role` interne si nécessaire) sans jamais exposer d'identifiants `service_role` au navigateur et sans jamais autoriser d'appel RPC direct navigateur vers les RPC réservées `service_role`. Partager le motif d'authentification ne signifie pas intégrer ces opérations dans `daily-run` — ce sont de nouvelles opérations serveur strictement séparées ; `daily-run` n'est pas modifié.

**Candidats côté serveur uniquement** : autorité candidat navigateur = AUCUNE ; autorité candidat serveur = COMPLÈTE. Le navigateur ne fournit jamais `candidateSnapshot`/`title`/`statement`/`caveats`/`direction`/comptages/ratios/`evidenceBalance`/dates d'événement/`athleteId`.

**Jeton de fraîcheur de revue — complet, 7 dimensions** : `detectorRuleId`/`detectorRuleVersion`/`insightKind`/`insightProjectorVersion`/`rangeFromDate`/`rangeToDate`/`sourceEvidenceRefs`, échoées par le navigateur (plus `decision`/`reviewerNote` séparément) ; `athleteId` résolu depuis la session authentifiée, jamais fourni par le navigateur. Le serveur reconstruit indépendamment le candidat courant et compare les 7 dimensions exactement — réutilise, sans le modifier, le même comparateur que la dérivation `reviewed_stale` déjà verrouillée dans `buildPatternInsightCandidates.ts`. Aucun hash inventé. Matrice de course verrouillée : jeton identique → persistance du snapshot **serveur** ; `sourceEvidenceRefs` changé, `insightProjectorVersion` changé, plage changée, version de règle non concordante, ou `insightKind` non concordant → `stale_candidate`/conflit typé, aucune écriture, retour du candidat frais ; candidat introuvable → `candidate_not_found`, aucune écriture ; rejeu d'une revue identique déjà réussie → idempotence déjà native de `persist_pattern_insight_review`.

**Persistance de candidat** : modèle M5_007 inchangé, retenu explicitement — evidence persistée, revue humaine persistée, agrégat et candidat toujours calculés, jamais persistés, aucune nouvelle table.

**Discipline de backfill** : le traitement historique réutilise la même orchestration canonique que le fonctionnement normal, mais la première exécution remote reste une étape de rollout séparément supervisée (preuve locale → aperçu remote read-only → invocation explicitement approuvée → validation read-only → preuve d'idempotence par seconde invocation consécutive) — jamais déclenchée implicitement par un premier chargement de page.

**Échelle MVP** : balayage complet de la timeline athlète pertinente à chaque `refresh-longitudinal`, aucune infrastructure de curseur/file/job en arrière-plan — simplification V1/athlète-unique délibérée, revisitée seulement sur symptôme qualitatif réel (latence perceptible, ou passage multi-athlète), jamais sur un seuil numérique choisi à l'avance.

**Décomposition verrouillée** : V0.3_001A (correction de runtime + orchestration, sans UI), V0.3_001B (backfill + durcissement complet), V0.3_001C (API de revue canonique + surface web Insights minimale). Toutes sous un seul objectif produit.

**Hors périmètre explicite** : enrichissement des domaines Technique DH/Mental/Nutrition, planificateur hebdomadaire, runtime `ActiveExperiment`, Garmin/Zwift/Strava, LLM, activation de coaching par pattern appris, scheduler/cron, table de persistance de candidat, scores de confiance/signification, causalité.

**Migrations** : aucune prévue pour V0.3_001 selon l'architecture verrouillée. Les objets de persistance nécessaires existent déjà via les milestones DB précédents (decision outcomes, M5_006A, M5_006B, M5_007). M5_006D reste un agrégateur déterministe TypeScript sans migration ni objet DB propre.

**Impact** : aucun changement de code, de test ou de migration dans cette entrée — verrouillage d'architecture uniquement ; `docs/00_PROJECT_STATUS.md`, `docs/04_DAILY_DECISION_ENGINE.md`, `docs/06_ARCHITECTURE.md`, `docs/10_TEST_PLAN.md`, `docs/12_BACKLOG.md`, cette entrée.

**Statut** : **ARCHITECTURE LOCKED.** Implémentation non commencée. Les briques M5 sous-jacentes existent déjà selon leur statut canonique propre ; aucun élément runtime V0.3_001 (orchestrateur, opérations serveur, correction du détecteur recommendation, backfill, API `submit-review`, UI Insights) n'est encore implémenté, migré ou déployé. Prochaine étape distincte : implémentation V0.3_001A.

---

## 2026-08-28 — V0.3_001A : CLOSED LOCALLY (implémentation + durcissement), aucun déploiement remote

**Contexte** : les deux commits V0.3_001A (`d69c81593aff4b25ad58aef3b2ac7c703a7d1d45` implémentation, `a9b50c13f312879b48d0fbe82f3776f5ad88e11b` durcissement) sont poussés sur `origin/main`. Cette entrée clôture la revue technique — aucun code/test/migration modifié dans cette entrée elle-même.

**Découvertes techniques retenues** :
- **Risque de matérialisation `buildTimeline`** : contrairement à `aggregateEffectivePatternEvidence` (validation O(1) de plage), `buildTimeline` matérialise un `AthleteDay` par date calendaire de la plage fournie — `INSIGHT_AGGREGATION_RANGE` (1900-01-01..9999-12-31) ne peut donc jamais lui être passée directement. Une `LONGITUDINAL_TIMELINE_RANGE` compacte, dérivée des données sources réelles, est requise (voir `docs/06_ARCHITECTURE.md` §V0.3_001 pour le modèle de plages complet, désormais documenté canoniquement).
- **Marge de lookback détecteur** : la dérivation compacte doit reculer `fromDate` d'un maximum de `SLEEP_ENERGY_BASELINE_WINDOW_DAYS` (60j) et `PAIN_PERSISTENCE_LOOKBACK_DAYS` (3j), bornée par le plancher de domaine, sous peine d'`InsufficientTimelineCoverageError` sur toute unité d'évaluation proche du début réel de l'historique d'un athlète.
- **Correction d'identité `recommendation_vs_actual_execution`** : `evaluationKey = evidenceKey = decision:<decisionId>` pour `evidence` et `no_evidence`, corrigeant l'ancien format `decision:<id>:completion:<completedSessionId>` qui ne pouvait pas identifier une identité antérieure depuis la branche `no_evidence`.
- **Répartition serveur/RLS** : `refresh-longitudinal` lit via `ctx.supabase` (authentifié RLS) et écrit exclusivement via `ctx.supabaseAdmin` (RPC déjà déployées) ; `get-insights` n'utilise jamais `service_role` (vues `security_invoker=true`, grants `authenticated` confirmés sur les 5 tables sources + 2 vues de lecture).
- **Contrat de build Edge propre** : `dist/**` est un artefact généré, jamais suivi par git ; `npm run build:clean` (suppression Node `fs.rmSync` puis rebuild) précède systématiquement tout test Edge HTTP.
- **Durcissement du transport** : réponse `refresh-longitudinal` enrichie de résumés par détecteur (3 rule ids exactes, 8 compteurs chacun) et d'une liste d'erreurs sanitisées (`scope`/`detectorRuleId`/`evaluationUnitId`/`code` fixe uniquement — jamais le texte brut Postgres/RPC), prouvé par 17 tests unitaires injectant un sentinel à travers les formateurs de production réels.
- **Contrat strict `get-insights`** : tout paramètre de requête (`athleteId`, `range`, `fromDate`, inconnu) est rejeté `400 invalid_request` — jamais silencieusement ignoré.

**Refactor de types de persistance** : `longitudinal-engine/src/persistence/types.ts` (forme de résultat plate pré-cycle-de-vie de M5_006A, y compris une variante `"skipped_no_evidence"` sans `identityId`, structurellement incompatible avec le modèle de cycle de vie) a été supprimé. Vérification empirique que ce fichier n'avait plus aucun consommateur réel dans le dépôt une fois l'adaptateur recommendation aligné sur la convention de résultat consciente du cycle de vie déjà utilisée par les adaptateurs sommeil-énergie/persistance-douleur — le fichier obsolète n'avait plus aucun consommateur restant dans le dépôt et a été retiré sans risque. Ce n'est pas un changement d'architecture produit.

**Relink recommendation — précision de preuve** : l'intégration réelle prouve `completed_session` X lié → evidence active ; déliaison → retiré ; même `no_evidence` répété → retrait inchangé ; relien du **même** `completed_session` X → même identité réactivée. Un second `completed_session` Y **différent** pour le même athlète/jour après existence de provenance est structurellement inatteignable sous les contraintes réelles du schéma (unicité `athlete_id`/`session_date`, cohérence même-jour du détecteur, FK `ON DELETE RESTRICT` de provenance). Des tests purs de détecteur séparés prouvent séparément que deux `completedSessionId` différents produisent la même `evidenceKey` fondée sur la décision. Aucun changement de schéma n'a été proposé ni requis pour fabriquer ce cas impossible.

**Preuve remote (read-only, jamais `evynmzyjhobdpmxdiwsy`)** : précondition confirmée immédiatement avant le changement de code, puis reconfirmée à la clôture par requête SQL directe (`supabase db query --linked`, jointures relationnelles réelles sur `pattern_evidence_identities`/`_revisions`/`_source_refs`/`_lifecycle_transitions` — méthode plus rigoureuse qu'un comptage de lignes `INSERT` sur un dump `pg_dump`, qui peut légitimement émettre des blocs `COPY` ; le comptage par dump d'une passe antérieure est donc superseded par cette requête directe). Résultat final pour `(recommendation_vs_actual_execution, 1.0.0)` : identities=0, revisions=0, source_refs=0, lifecycle_transitions=0. Migration parity `local=remote=26`, `pending=0`.

**Tests — état final** : `longitudinal-engine` 689/689 (unitaire + intégration DB réelle + tests Edge unitaires), harness HTTP V0.3 30/30 (×2 exécutions consécutives immédiates, suivies immédiatement de M3 HTTP 26/26 et completed-session HTTP 70/70, sans intervention manuelle — preuve de répétabilité du runtime Edge). Régressions gelées reconfirmées : M1/M2=226/226, daily-run edge=9/9, completed-session unit=73/73, web=242/242, web build=PASS, longitudinal build=PASS.

**Migrations — AUCUNE ajoutée.** Tous les objets DB nécessaires existaient déjà (M5_001B/M5_004/M5_006A/M5_006B/M5_007).

**Remote** : aucune écriture, aucun déploiement Edge Function, aucun backfill historique.

**Décision** : **V0.3_001A = CLOSED LOCALLY.** L'orchestration runtime (`refresh-longitudinal`, `get-insights`, correction d'identité du détecteur recommendation) est implémentée, testée et verrouillée, **localement uniquement**. `submit-review` et la surface web Insights (V0.3_001C) restent non implémentées ; le backfill remote et le durcissement complet (V0.3_001B) restent non exécutés. `daily-run` non modifié, Safety A1-A5 inchangée, aucune activation automatique de coaching. **V0.3_001 dans son ensemble reste IN PROGRESS.**

**Impact** : aucun changement de code dans cette entrée (revue de clôture uniquement) ; `docs/00_PROJECT_STATUS.md`, `docs/04_DAILY_DECISION_ENGINE.md`, `docs/06_ARCHITECTURE.md`, `docs/10_TEST_PLAN.md`, `docs/12_BACKLOG.md`, cette entrée.

**Statut** : **CLOSED LOCALLY.** Commits : implémentation `d69c81593aff4b25ad58aef3b2ac7c703a7d1d45`, durcissement `a9b50c13f312879b48d0fbe82f3776f5ad88e11b`. Prochaine étape distincte : V0.3_001B (backfill + durcissement complet remote).

---

## 2026-08-28 — V0.3_001B : CLOSED REMOTE (déploiement + premier backfill + idempotence remote prouvés)

**Contexte** : depuis l'implémentation runtime-aligned HEAD `27fcb564dff2558b1a1dafc81c914d46a8957011` (code identique à celui de la clôture V0.3_001A — voir l'entrée précédente ; les commits intermédiaires n'ont touché que `longitudinal-engine/tools/**`/`longitudinal-engine/tests/**`), `refresh-longitudinal` et `get-insights` déployés sur `uvolpldwwyvadlamulvr` (`--use-api`, `verify_jwt: true` conservé). Aucune migration. `daily-run`/`completed-session` non retouchées. Artefact de production `refresh-longitudinal` identique (`ezbr_sha256` `c855e79897c23369f9cb058fc94ed9824f16b1334dcfde7fa1577f41a1fe907b`) à travers ses 3 versions déployées, confirmant zéro dérive de packaging entre les commits tooling-only intermédiaires.

**Premier backfill remote (authentifié, athlète réel)** : `status: complete`, `outcomes: {attempted:42, writeSucceeded:42, alreadyExisted:0, skippedImmature:0, errorCount:0}`, les 3 détecteurs uniquement `skippedNoPrior` (recommandation 14, sommeil-énergie 3, douleur 3) — `completed_sessions=0` rend `no_evidence` déterministe pour la recommandation ; ledger d'evidence resté vide. Vérification relationnelle directe (SQL read-only, aucun UUID exposé) : 42 outcomes joints sans orphelin, 14 décisions sources exactement représentées (aucune manquante, aucune hors périmètre), exactement 3 horizons par décision, aucun horizon inattendu, un seul scope athlète, aucune incohérence `decision_outcomes.athlete_id`/`decisions.athlete_id`.

**Contrat d'idempotence** : audité depuis le code réel (`outcomeOrchestrator.ts`, migration `persist_decision_outcome`) puis prouvé en local avant tout second appel remote — `calculateAndPersistOutcomes` court-circuite via `thread.outcomesByHorizon` (données déjà chargées depuis la timeline fraîchement relue) **avant** tout appel RPC ; `persist_decision_outcome` (clé `(athlete_id, decision_id, horizon, calculator_id, calculator_version)`) n'autorise aucune `UPDATE` (ni dans le corps de la fonction, ni au niveau des grants — `service_role` n'a que `SELECT, INSERT`) et rejette explicitement un contenu conflictuel sur la même identité (immutabilité) ; si `persist_decision_outcome` est rejoué directement avec un snapshot identique, il renvoie l'id existant sans écriture. `sourceFingerprint` intègre `decision_outcomes` (changement du fingerprint empiriquement prouvé après le premier backfill, via `timeline.decisionThreads[].outcomesByHorizon`).

**Second appel remote (preuve d'idempotence, athlète réel)** : `status: complete`, `outcomes: {attempted:0, writeSucceeded:0, alreadyExisted:42, skippedImmature:0, errorCount:0}`, mêmes 3 détecteurs entièrement `skippedNoPrior` (14/3/3), `errors: []`. Revalidation relationnelle complète : l'ensemble des métriques agrégées et relationnelles revérifiées (comptages, répartition par horizon, jointure `decision_outcomes`↔`decisions`, cohérence athlète) est resté strictement identique à l'état post-premier-backfill — aucun delta.

**État final remote** : `decision_outcomes=42`, `pattern_evidence_*=0/0/0/0`, `pattern_insight_*=0/0`.

**Outillage opérateur ajouté** (`longitudinal-engine/tools/**`, jamais importé par `supabase/functions/**`/`web/**`/`head-coach-engine/**`, absent de `dist/` de production) : `previewRemoteRefresh.ts`/`getInsightsCanary.ts` (lecture seule) et `invokeApprovedRefresh.ts`/`invokeApprovedIdempotencyRefresh.ts` (écriture guardée, chacun scopé à un seul scénario déjà audité — jamais un outil d'invocation générique). **Ce sont des outils de rollout/débogage opérateur, jamais un chemin d'exécution produit** — aucun d'entre eux n'est appelé automatiquement, ni par `daily-run`, ni par un scheduler, ni par le web.

**Limites d'architecture toujours vraies (état déployé actuel)** : aucune activation automatique de coaching, `daily-run` non modifié, Safety A1-A5 inchangée, aucune persistance de candidat d'insight, aucun scheduler ni appelant automatique de `refresh-longitudinal` déployé à ce stade, aucune UI Insights, `submit-review` non implémenté. `accepted_as_insight` (M5_007) n'influence jamais `daily-run`.

**Décision** : **V0.3_001B = CLOSED REMOTE.** L'implémentation runtime V0.3_001A (orchestration, correction du détecteur recommendation, `refresh-longitudinal`, `get-insights`) reste **CLOSED LOCALLY** en tant que jalon distinct ; V0.3_001B a déployé ce même runtime sur `uvolpldwwyvadlamulvr` et vérifié en remote un premier backfill ainsi qu'une preuve d'idempotence, sans aucun changement de code de production. Prochaine étape distincte et non commencée : **V0.3_001C** (API `submit-review`, validation complète du jeton de fraîcheur 7 dimensions, gestion `stale_candidate`/`candidate_not_found`, surface web Insights minimale). **V0.3_001 dans son ensemble reste IN PROGRESS.**

**Impact** : aucun changement de code dans cette entrée (revue de clôture uniquement). Une clôture documentaire distincte (mise à jour de `docs/00_PROJECT_STATUS.md`, `docs/04_DAILY_DECISION_ENGINE.md`, `docs/06_ARCHITECTURE.md`, `docs/10_TEST_PLAN.md`, `docs/12_BACKLOG.md`, cette entrée) suit l'implémentation runtime-aligned HEAD `27fcb564dff2558b1a1dafc81c914d46a8957011` et ne modifie aucun comportement runtime — son propre HEAD de commit sera nécessairement différent de celui-ci.

**Statut** : **CLOSED REMOTE.** Commits tooling (aucun n'a modifié `longitudinal-engine/src/**`, `supabase/functions/**`, ni de migration) : preview `a1d41e4a94fdcb9b65fd52a1717c82e25caf196a`, durcissement garde `731b3be0a45316fb174d7939fe750e9592974fde`, canary `f6e5add666d587b9011634fd563382d23c77e020`, runner premier backfill `87cf003921e129a83b8b1401f1037e797824e12f`, runner idempotence — **implémentation runtime-aligned HEAD** — `27fcb564dff2558b1a1dafc81c914d46a8957011`.

---

## 2026-08-28 — V0.3_001C : verrouillage de la sémantique de linéarisation de fraîcheur (DÉCISION D'ARCHITECTURE, AVANT TOUTE IMPLÉMENTATION)

**Contexte** : avant d'entamer l'implémentation de `submit-review`, un audit read-only a identifié une ambiguïté réelle dans la matrice de course déjà verrouillée (`docs/06_ARCHITECTURE.md`/`docs/10_TEST_PLAN.md`/cette entrée, ligne 1589 ci-dessus) : le flux proposé (lecture RLS → reconstruction/comparaison serveur du candidat → RPC `persist_pattern_insight_review` séparée) n'est pas une transaction PostgreSQL unique — chaque appel `ctx.supabase`/`ctx.supabaseAdmin.rpc(...)` depuis une Edge Function est sa propre transaction, sans mécanisme de connexion tenue ouverte entre appels. La matrice existante décrivait la comparaison mais ne précisait pas si la fraîcheur devait rester valide jusqu'au commit de l'écriture ou seulement au moment de la comparaison — un vrai point non spécifié, pas une inférence architecturale.

**Décision** : **SEMANTICS A approuvée.** La linéarisation de fraîcheur se produit à la reconstruction/comparaison serveur réussie du candidat canonique contre les 7 dimensions du jeton navigateur — pas à la persistance. Une divergence déjà présente à cet instant → `stale_candidate` (ou `candidate_not_found` si le candidat n'existe plus), aucune écriture. Une comparaison réussie autorise la persistance du `candidate.snapshot` serveur ; une mutation d'evidence effective survenant après cette comparaison ne rend pas rétroactivement l'écriture invalide — la revue persistée reste une trace append-only valide de l'état effectivement revu, et peut se projeter immédiatement `reviewed_stale` lors d'une lecture `get-insights` ultérieure. C'est un comportement attendu, cohérent avec la tolérance déjà verrouillée du modèle `reviewed_current`/`reviewed_stale` (`docs/11_DECISION_LOG.md`, entrée M5_007 : *"Une décision accepted_as_insight devenue périmée... est rapportée telle quelle pour contexte mais jamais silencieusement requalifiée"*) — jamais une erreur de persistance.

**Correction explicite à l'audit précédent** : une RPC PostgreSQL multi-instructions unique ne prouverait pas, à elle seule, une garantie « fraîche jusqu'au commit » contre une mutation d'evidence concurrente sans verrouillage partagé supplémentaire couvrant aussi les *nouvelles* identités d'evidence entrant dans la plage d'agrégation — l'audit précédent avait qualifié cela de « résolu gratuitement par MVCC », ce qui était incorrect et n'est pas retenu.

**Aucune sémantique B** : pas de verrou partagé evidence/revue, pas de machinerie de nouvelle tentative sérialisable, pas de réimplémentation SQL de l'agrégation de candidat, pas de RPC de reconstruction atomique, pas de nouvelle table de coordination evidence/revue, pas de nouveau verrou de portée evidence.

**Migration** : aucune. `persist_pattern_insight_review` et les RPC d'écriture d'evidence restent strictement inchangées.

**Limite inchangée** : `accepted_as_insight` n'influence toujours pas `daily-run`. Safety A1-A5 inchangée.

**Impact** : `docs/06_ARCHITECTURE.md` (ajout d'un paragraphe « Point de linéarisation de fraîcheur » après la section jeton de fraîcheur existante, les 7 dimensions elles-mêmes inchangées), `docs/10_TEST_PLAN.md` (ajout de deux scénarios de course explicites : mutation déjà visible avant comparaison → `stale_candidate` ; mutation après comparaison réussie → écriture autorisée, projection `reviewed_stale` ultérieure attendue), cette entrée. Aucun changement `longitudinal-engine/src/**`, `supabase/functions/**`, `supabase/migrations/**`, `web/**`.

**Statut** : **DÉCISION VERROUILLÉE — 001C-1 (`submit-review` Edge Function + porte de fraîcheur, réutilisant la reconstruction TypeScript serveur existante et la RPC M5_007 existante inchangée) peut maintenant être scopée sans ambiguïté d'architecture restante.**

---

## 2026-08-28 — V0.3_001C : verrouillage du sélecteur de candidat et de sa cardinalité (DÉCISION D'ARCHITECTURE, AVANT TOUTE IMPLÉMENTATION)

**Contexte** : un second point d'ambiguïté a été identifié avant l'implémentation de `submit-review` — l'algorithme proposé recherchait le candidat courant par le triplet exact `(detectorRuleId, detectorRuleVersion, insightKind)`, ce qui rendrait `detectorRuleVersion`/`insightKind` divergents indétectables comme tels : la recherche manquerait simplement toute correspondance et produirait à tort `candidate_not_found` au lieu de `stale_candidate`, alors que la matrice de course déjà verrouillée exige `stale_candidate` pour ces deux divergences. Un audit direct du code (`aggregateEffectivePatternEvidence.ts` : regroupement exact par `(detectorRuleId, detectorRuleVersion)` ; `registry.ts` : `insightKind` dérivé purement de ce même couple) a confirmé que ni `detectorRuleVersion` ni `insightKind` ne peuvent faire partie du sélecteur de recherche sans casser cette exigence.

**Décision** : **sélecteur de candidat = `(athleteId résolu côté serveur, detectorRuleId)`.** `detectorRuleVersion` et `insightKind` restent des dimensions de fraîcheur uniquement, jamais des critères de recherche. Après reconstruction complète des candidats canoniques courants côté serveur : 0 correspondance → `candidate_not_found`, aucune écriture ; exactement 1 correspondance → comparaison des 7 dimensions de fraîcheur (inchangées) ; plus d'1 correspondance → violation d'invariant, **aucune sélection arbitraire** (jamais premier élément/`array[0]`/ordre de tri), aucune écriture, réponse `internal_error` sanitisée standard, violation journalisée côté serveur — pas de nouveau code d'erreur métier public en V0.3_001C pour ce cas, qui est un état impossible sous le modèle prévu, pas un état de revue destiné à l'utilisateur.

**Invariant requis** : pour un couple (athlète, `detectorRuleId`) donné, la projection canonique de candidats n'expose **au plus qu'un seul candidat courant** à un instant donné. Cet invariant porte uniquement sur les candidats **courants reconstruits** — l'evidence historique append-only peut légitimement subsister indéfiniment (le ledger est append-only par conception) et n'a pas besoin d'être supprimée pour le respecter. Explicitement **non retenu** : « l'ancienne identité d'evidence ne doit plus exister », « une seule version peut exister dans le registre », ou toute formulation exigeant une suppression d'historique. Le mécanisme exact par lequel une future transition de version de détecteur préserverait cette unicité du candidat courant reste hors périmètre V0.3_001C.

**Choix d'implémentation non verrouillés dans cette entrée** (confirmés comme relevant de 001C-1, sans nouveau tour de revue documentaire) : statuts HTTP exacts de `stale_candidate`/`candidate_not_found`, forme exacte de la réponse de succès (dérivée de préférence du résultat existant de `persistPatternInsightReview`), rejet des champs de requête inconnus (déjà convention établie dans ce dépôt).

**Migration** : aucune. Aucun schéma, RPC, `candidateId` ajouté.

**Limite inchangée** : `accepted_as_insight` n'influence toujours pas `daily-run`. Safety A1-A5 inchangée.

**Impact** : `docs/06_ARCHITECTURE.md` (ajout d'un paragraphe « Sélecteur de candidat et cardinalité » après le paragraphe de linéarisation de fraîcheur existant), `docs/10_TEST_PLAN.md` (ajout de 4 scénarios explicites : divergence `detectorRuleVersion` → `stale_candidate`, divergence `insightKind` → `stale_candidate`, absence de candidat → `candidate_not_found`, pluralité de candidats → `internal_error` sans sélection arbitraire), cette entrée. Aucun changement `longitudinal-engine/src/**`, `supabase/functions/**`, `supabase/migrations/**`, `web/**`.

**Statut** : **DÉCISION VERROUILLÉE — le sélecteur de candidat et sa règle de cardinalité sont maintenant sans ambiguïté restante pour 001C-1.**

---

## 2026-08-28 — V0.3_001C : clôture remote et clôture V0.3_001

**Contexte** : avant clôture, un audit a identifié une ambiguïté canonique réelle — aucun texte verrouillé n'exigeait explicitement une écriture `submit-review` réelle en production avant clôture de V0.3_001C, mais deux précédents projet réels (M3/`daily-run`, « M3_005 canary + M3_006 run réel » ; V0.3_001B/`refresh-longitudinal`, premier backfill remote) montrent que ce projet a historiquement clos ses chemins d'écriture remote seulement après une écriture authentifiée réelle. Dans les deux précédents, une précondition naturelle existait déjà (check-in réel de Louis pour M3 ; données historiques réelles pour le backfill V0.3_001B). L'état naturel actuel de production ne contient aucun candidat courant (`pattern_evidence_identities`/`_revisions`/`pattern_evidence_current_effective` = 0/0/0, confirmé par lecture authentifiée réelle `get-insights` → `HTTP 200`, `candidateCount = 0`), rendant cette précondition absente pour `submit-review`.

**Décision** : **SEMANTICS B approuvée.** Pour un endpoint d'écriture de revue humaine tel que `submit-review`, si l'état naturel de production ne contient aucun candidat courant valide sur lequel une revue humaine réelle peut légitimement porter, la clôture n'exige pas la fabrication de données applicatives dans le seul but d'exercer le chemin d'écriture. Dans ce cas, la clôture remote repose sur (A) la preuve de déploiement/authentification/surface de lecture remote et (B) la preuve réelle du chemin d'écriture **locale** (jamais un mock). Voir `docs/06_ARCHITECTURE.md`, « Discipline de rollout `submit-review` », pour le texte verrouillé complet.

**Rationale explicite** : ne jamais fabriquer un athlète, une décision, une séance, un check-in, une evidence ou une revue de production dans le seul but d'exercer un endpoint d'écriture de revue humaine. Une écriture réelle réussie reste une preuve de rollout utile **quand elle est naturellement possible** — elle n'est jamais transformée en obligation qui forcerait la création de données de production artificielles.

**Preuve locale du chemin d'écriture (001C-1/001C-2, réelle — endpoint HTTP réel, Postgres/RPC locaux réels, jamais mockée)** : insertion/inchangé/supersession réels via `persist_pattern_insight_review` ; égalité DB du `candidate_snapshot` côté serveur uniquement ; soumissions concurrentes identiques → exactement un `inserted` + un `unchanged`, aucune ligne dupliquée ; soumissions concurrentes divergentes → exactement un `inserted` + un `superseded`, chaîne de supersession valide, aucune écriture perdue ; isolation inter-athlète par réutilisation réelle d'un jeton d'un autre athlète → `candidate_not_found`, zéro écriture croisée ; linéarisation de fraîcheur (sémantique A) prouvée par un verrou consultatif PostgreSQL réel objectivement observé via `pg_locks` (jamais une pause arbitraire) — écriture valide, projection `reviewed_stale` ultérieure confirmée. `V0.3` HTTP harness = **84/84**.

**Preuve web (001C-3, réelle)** : surface `/insights` authentifiée, chargement `get-insights` uniquement (jamais `refresh-longitudinal` automatique), corps `submit-review` construit explicitement (7 dimensions + `decision` + `reviewerNote`, jamais `athleteId`/`candidateSnapshot`), `stale_candidate`/`candidate_not_found` gérés par refetch authoritatif sans resoumission automatique. `web` = **293/293**, `web build` = **PASS**.

**Preuve remote (001C-4A/001C-4B1)** : cible confirmée `uvolpldwwyvadlamulvr` (jamais `evynmzyjhobdpmxdiwsy`) ; migrations 26 local/26 remote/0 pending ; primitives M5_007 présentes ; `submit-review` déployé `ACTIVE v1`, `verify_jwt: true`, `ezbr_sha256=bf42eddead6f320a8dd38afd1770371b1e5b97972b9d921c93df8aae2ad9a40b` ; `get-insights`/`refresh-longitudinal` inchangés (mêmes versions, mêmes `ezbr_sha256` avant/après) ; lecture authentifiée remote réelle `get-insights` → `HTTP 200`, plage `1900-01-01..9999-12-31`, `candidateCount = 0` ; production web déployée sur `nalynt/louis-performance-system` (`https://louis-performance-system.vercel.app`), `/insights` sert la SPA réelle ; delta d'écriture applicatif remote sur l'ensemble du rollout/preflight = **0** (identités/revues/evidence toutes restées à 0/0/0 avant et après).

**Écriture de revue réussie en production** : **NON EXÉCUTÉE.** Raison : zéro candidat naturel en production. Aucune donnée applicative synthétique créée. La première revue humaine future sur un candidat naturel réel constitue une observation de rollout non bloquante — elle ne rouvre pas V0.3_001C, ne nécessite ni supervision ni planification, et n'est jamais provoquée artificiellement.

**Limites toujours vraies** : `accepted_as_insight` n'influence toujours pas `daily-run` ni aucune activation de coaching. Safety A1-A5 inchangée. Aucun scheduler ni appelant automatique introduit. `daily-run`/`head-coach-engine`/`longitudinal-engine/src/**`/migrations non modifiés par cette clôture (documentation uniquement).

**Décision** : **V0.3_001C = CLOSED REMOTE.** La décomposition verrouillée de V0.3_001 (`docs/11_DECISION_LOG.md`, entrée d'architecture du 2026-08-27) contient exactement V0.3_001A + V0.3_001B + V0.3_001C, aucun V0.3_001D. **V0.3_001 dans son ensemble = COMPLETE.**

**Impact** : `docs/06_ARCHITECTURE.md` (statut/en-tête V0.3_001, paragraphe `submit-review` du modèle d'exécution, nouvelle section « Discipline de rollout `submit-review` »), `docs/10_TEST_PLAN.md` (statut/en-tête, nouvelle sous-section V0.3_001C), `docs/00_PROJECT_STATUS.md`, `docs/12_BACKLOG.md`, cette entrée. Aucun changement `longitudinal-engine/src/**`, `supabase/functions/**`, `supabase/migrations/**`, `web/**`.

**Statut** : **CLOSED REMOTE (V0.3_001C) — COMPLETE (V0.3_001 dans son ensemble).** Implémentation runtime-aligned HEAD `3a796dffd4deb780e7b02ebf8aaa54aaac03d34b` (dernier commit de code/tests avant cette clôture documentaire, `feat: add V0.3_001C insights web surface`) ; cette entrée de clôture documentaire aura nécessairement son propre HEAD de commit, différent de celui-ci.

---

## 2026-08-28 — V0.3_002A : architecture verrouillée pour l'enrichissement des domaines Technique DH / Mental / Nutrition

**Contexte** : après clôture complète de V0.3_001, audit produit du prochain jalon a identifié l'enrichissement des domaines 1 (Technique DH), 2 (Mental), 5 (Nutrition) comme prochaine zone de travail approuvée par l'architecte. Un audit runtime détaillé a corrigé une hypothèse initiale erronée : les faits de `docs/02_ATHLETE_PROFILE.md` (spots, forces/faiblesses techniques, profil mental de course) ne sont ingérés par aucun chemin de données réel (`RawContext`/`ContextState`) — un document Markdown n'est pas une entrée runtime. Un second audit a par ailleurs prouvé que `training.ts` ne consomme les signaux `stress_high`/`motivation_low` que lorsque `mental = RED` (règle `MENTAL_RED`), jamais en `AMBER`, et que `SignalTrace` expose déjà des accesseurs non consommants (`has`, `consumedByRule`) suffisants pour distinguer propriété de décision et coaching de support sans aucune modification de son implémentation. Une revue de correction a ensuite resserré le contrat de sortie Technique (un seul champ `focus`, pas de champ priorité séparé), le contrat numérique hydratation (aucun point médian inventé depuis une plage), la frontière de la config de profil athlète (faits personnels uniquement, pas un sac de constantes heuristiques génériques), l'assertion de test Mental (comportementale, pas structurelle sur le code source), et la sémantique du test Nutrition "séance de force" (planifiée, jamais présentée comme déjà exécutée par le moteur).

**Décision** :
1. Créer le jalon parent **V0.3_002 — Domain Coaching Enrichment**, décomposé en V0.3_002A (ce jalon, architecture) → V0.3_002B (Technique DH) → V0.3_002C (Mental) → V0.3_002D (Nutrition) → V0.3_002E (intégration/régressions) → V0.3_002F (rollout remote/clôture).
2. Verrouiller le modèle de propriété de signal **Option C** : un signal a au plus un propriétaire de décision (`consume()`) ; un domaine de coaching de support peut le lire (`has()`/`consumedByRule()`) sans le reconsommer. Le domaine Mental doit s'exécuter strictement après le domaine Préparation physique dans `buildDailyPlan.ts`.
3. Approuver une **Option A bornée** pour les faits de coaching personnalisés stables : une config statique typée mono-athlète, source-controlled, sous `head-coach-engine/src/**`, explicitement marquée PROVISIONAL/V0.3/mono-athlète — jamais un état opérationnel encodé comme vrai-maintenant (disponibilité de spot, météo, etc.), et jamais un sac de constantes heuristiques génériques (les baselines d'hydratation C5.3/C5.4 restent une config de domaine, pas un fait personnel de Louis). La config elle-même n'est pas créée en V0.3_002A — seule l'architecture est verrouillée.
4. Verrouiller la portée minimale de 002B/C/D aux champs de sortie `DailyPlan` déjà existants (`dh_or_technical.{active,focus,spot_hint}`, `mental.{active,focus,action_hint}`, `nutrition.{active,focus,hydration_target_l,notes}`), sans nouveau champ, sans migration, sans nouveau champ `daily_checkins`, sans changement de production web attendu. `focus` (Technique) reste une seule chaîne de cue actionnable, jamais un champ priorité séparé ; la proximité course influence `spot_hint`, jamais `focus`. `hydration_target_l` n'est peuplé que si une cible numérique canonique unique existe déjà (C5.3) — jamais un point médian inventé depuis une plage (C5.4 reste textuel en `notes`).
5. Différer explicitement : retest/outcome de drill technique, coaching post-erreur en direct (C2.3), pit-routine précise (C2.2, faute de `race_phase` fiable au quotidien), debrief post-course structuré, C5.5 (bloqué sur `ActiveExperiment`/T9 non implémenté).

**Alternatives considérées** :
- Option B (Mental devient propriétaire primaire des signaux mentaux, Training réagit à sa sortie) — rejetée : nécessiterait de modifier `training.ts`, en violation du gel M1.
- Option B pour le profil (config structurée chargée dans `RawContext` via nouvelle table) — non retenue pour le scope minimum V0.3_002 (migration + charge de test supplémentaire non justifiées pour un unique athlète) ; reste une option future si multi-athlète.
- Nommer directement un spot réel dans `spot_hint` — rejetée : le runtime ne connaît aucune donnée de disponibilité réelle (ouverture bikepark, météo, logistique courante).
- Test Mental structurel (assertion sur l'ordre d'appel dans le code source) — rejeté au profit d'une preuve comportementale intégrée (décision Training inchangée + `SignalTrace` propriétaire + sortie Mental supportive + second `consume()` échoue), moins fragile aux refactorings.

**Impact** : `docs/03_COACHING_MODEL.md` (§3 propriété de signal, pointeurs domaine-spécifiques C1/C2/C5), `docs/04_DAILY_DECISION_ENGINE.md` (§7.17, roadmap §8), `docs/06_ARCHITECTURE.md` (nouvelle section §V0.3_002), `docs/10_TEST_PLAN.md` (§T11-T14), `docs/00_PROJECT_STATUS.md` (phase actuelle), `docs/12_BACKLOG.md` (phase actuelle, retrait Technique/Mental/Nutrition du pool P1 générique, nouveau bloc V0.3_002A). Aucun changement `head-coach-engine/src/**`, `head-coach-engine/tests/**`, `web/**`, `supabase/**`, `longitudinal-engine/**`.
...
## 2026-08-30 — V0.3_002E : Cross-domain integration CLOSED LOCALLY

**Contexte** : suite à V0.3_002D (Nutrition, CLOSED LOCALLY), jalon d'intégration/régressions cross-domaine pour les trois domaines déjà implémentés (Technique, Mental, Nutrition) contre Safety/Training/Recovery/`EventContext`/propriété de signal. Jalon **test-only** par conception (docs/06_ARCHITECTURE.md §Contrat d'intégration V0.3_002E) : aucune nouvelle sémantique de domaine, aucun nouveau champ `DailyPlan`, aucun nouveau rule_id.

**Décision** :
1. `head-coach-engine/tests/t14_crossDomainIntegration.test.ts` (NEW) est le seul fichier créé — aucun fichier de production modifié, aucun test historique (M1-M4, T11, T12, T13, `engineVersion.test.ts`) modifié. Toutes les fixtures passent par le vrai pipeline `RawContext → buildDailyPlan` — aucune dimension dérivée injectée directement, aucune instrumentation directe de `SignalTrace` (les preuves d'ownership sont observables via `triggered_rules` uniquement).
2. Indépendance `PRE_EVENT`/`RACE_WEEK` prouvée sur les trois quadrants réels : `event_context.phase === "PRE_EVENT"` (source exclusive du `focus` Mental) et `active_mode === "RACE_WEEK"` (source exclusive du `focus` Nutrition) ne dérivent jamais l'un de l'autre et coexistent sans interférence quand les deux sont vrais simultanément.
3. Coexistence réelle des trois domaines confirmée sur une séance DH finale + Mental AMBER : `dh_or_technical`/`mental`/`nutrition` tous actifs, exactement une `TriggeredRule` `MENTAL_AMBER_STRESS`, zéro `TECHNIQUE_*`/`NUTRITION_*`.
4. La garantie de late-push Mental (établie en V0.3_002C sur un fixture minimal) est étendue à une paire GREEN/AMBER appariée avec Technique **et** Nutrition simultanément actifs des deux côtés — `decision`/`final_session`/`training`/`reasoning`/`override_reason`/`dh_or_technical`/`nutrition`/`recovery` strictement égaux ; seuls `mental`/`triggered_rules` diffèrent par exactement une règle.
5. Propriété `MENTAL_RED` confirmée sur une séance DH réelle : `training.ts`'s branche mental-RED ne fait jamais que dégrader `load_profile` pour un kind DH (jamais un changement de `kind`), donc Technique/Nutrition-DH suivent correctement le `final_session` stabilisé toujours-DH ; Mental n'émet aucune nouvelle règle observable (`triggered_rules` ne contient qu'un seul `MENTAL_RED`, jamais de `MENTAL_AMBER_*` associé). T14 n'instrumente jamais `SignalTrace` (aucun import, aucune exposition) — il s'agit d'une preuve de composition cross-domaine observable via `DailyPlan`/`triggered_rules`, pas d'une preuve directe d'appel `consume()`. La preuve directe qu'aucun second `consume()` n'a lieu reste couverte par T12 (V0.3_002C), qui instrumente `SignalTrace` réellement.
6. Propagation `final_session` + asymétrie `planned_session` acceptée prouvées sur un unique fixture réel : `planned_session=GRIP_WORK` + `arms_grip`/`legs` RED → Training (`training.ts` C3.5) pivote vers `RECOVERY_ACTIVE` ; Technique et la branche DH de Nutrition suivent correctement (inactifs) ; la branche force de Nutrition reste active car elle lit intentionnellement `ctx.planned_session` brut, jamais `final_session` — comportement documenté comme asymétrie acceptée (contrat V0.3_002D), pas une incohérence.
7. Composition jour de course réel (`computeEventContext`, jamais un `EventContext` construit à la main) : Nutrition race-day et Mental AMBER coexistent ; `mental.focus` est structurellement absent, jamais seulement observé absent, car `computeEventContext` ne peut jamais retourner `phase === "PRE_EVENT"` quand `in_progress` est vrai (branches `if/else if` mutuellement exclusives).
8. Safety dominant une pression maximale (`suspected_concussion` + `active_mode=RACE_WEEK` + une course réelle en fenêtre `PRE_EVENT` + séance DH planifiée) : `triggered_rules` strictement `[A1]`, `dh_or_technical`/`mental`/`nutrition` exactement `{active:false}` — confirmé structurellement par le early-return de `buildSafetyPlan` (les trois fonctions de domaine ne sont jamais invoquées sur ce chemin), pas seulement observé par test.
9. `ENGINE_VERSION` **inchangé** (`head-coach-engine@0.2.0-m1-v0.3_002d`) — aucune sémantique persistée du `DailyPlan` modifiée par ce jalon test-only ; `tests/engineVersion.test.ts` reste l'unique propriétaire de l'assertion exacte, non modifié.
10. Reconciliation de la clause "renderer web" du contrat verrouillé V0.3_002E (docs/06) : le périmètre accepté de 002E est strictement `head-coach-engine`-only (`web/**` hors scope approuvé) ; l'exigence originale — le renderer web existant accepte le `DailyPlan` enrichi réel — n'a donc **pas** été ré-exercée par 002E lui-même et l'exigence n'est pas clôturée par 002E : elle reste un gate ouvert. Aucun changement de production/web n'a eu lieu durant 002E (donc aucun nouveau risque introduit), mais cela ne constitue pas une preuve bout-en-bout. Cette exigence est reportée explicitement comme **gate requis de V0.3_002F**, avant rollout remote et avant toute clôture de V0.3_002 dans son ensemble.

**Preuve** : implémentation (`fa8d1c740c6a4971384d46172ca70a5bb9f85e20`, `test: add V0.3_002E cross-domain integration coverage`) poussée sur `origin/main`. `npm test` **359/359**, `npm run test:edge` **9/9**, build TypeScript strict PASS. Aucune régression sur les suites frozen M1-M4/V0.3_002B/V0.3_002C/V0.3_002D pré-existantes (aucune n'a été modifiée).

**Alternatives considérées** :
- Instrumenter `SignalTrace` directement dans T14 (comme T12 le fait) pour prouver l'absence de double-consommation — **REJETÉE** : le périmètre approuvé pour 002E interdit explicitement d'exposer/modifier `SignalTrace` ou d'ajouter un accesseur de test ; la preuve reste entièrement observable via `triggered_rules`, suffisante car Technique/Nutrition n'importent structurellement jamais `SignalTrace` (aucun paramètre, aucun import).
- Réutiliser littéralement le fixture `mental RED` déjà existant de T12 (`AEROBIC_INTERVALS`) tel que mentionné dans le texte verrouillé du contrat d'intégration — **NON RETENUE littéralement** : un fixture DH réel équivalent a été construit à la place, prouvant le même contrat comportemental (kind DH préservé, charge seule dégradée) sur un cas directement pertinent pour Technique/Nutrition-DH, sans dupliquer T12.
- Ajouter une preuve `web/` (`isValidDailyPlan` sur un `DailyPlan` enrichi réel) pour satisfaire littéralement la clause "renderer web" du contrat verrouillé — **NON RETENUE pour 002E** (hors du périmètre explicitement approuvé, `web/**` exclu du hard file-scope lock — pas un rejet sur le fond) ; conséquence : l'exigence reste ouverte et est **reportée à V0.3_002F** comme gate requis, plutôt que traitée comme déjà satisfaite.

**Impact** : `docs/00_PROJECT_STATUS.md`, `docs/04_DAILY_DECISION_ENGINE.md` (roadmap §8), `docs/06_ARCHITECTURE.md` (reconciliation clause "renderer web" + sous-section de clôture §V0.3_002E), `docs/10_TEST_PLAN.md` (statut T14), `docs/12_BACKLOG.md` (phase actuelle, nouveau bloc V0.3_002E). Aucun changement `web/**`, `supabase/functions/**`, `supabase/migrations/**`, `longitudinal-engine/**`, `head-coach-engine/src/**`.

**Statut** :
V0.3_002E = CLOSED LOCALLY
V0.3_002 (ensemble) = IN PROGRESS
...
## 2026-08-31 — V0.3_003A : architecture verrouillée pour Planning / Session Intent

**Contexte** : après clôture complète de V0.3_002, un audit du prochain jalon produit a identifié deux candidats sérieux — `ActiveExperiment`/T9 (déjà typé en M1, un experiment réel `sleep-liquids-cutoff-2026-08` approchant sa fenêtre de revue) et Planning/Session Intent. L'audit a révélé que `planned_sessions` n'a aujourd'hui **aucun chemin produit athlète** : `web/src/**` ne contient ni route ni composant de gestion de séance planifiée, alors même que le moteur consomme déjà pleinement `planned_session` (Training, branche force de Nutrition, carte "Prévu vs Aujourd'hui" de `DailyPlanView`) et que le schéma `planned_sessions` anticipait déjà une saisie manuelle athlète dès la baseline V0.2 (`source DEFAULT 'manual'`, RLS déjà correctement permissive). Décision produit : **Planning avant ActiveExperiment** — Planning active un comportement moteur déjà construit et retire une opération hors-bande existante, sur tous les jours d'usage normal ; ActiveExperiment reste un candidat plus étroit (texte nutrition/récupération pendant une fenêtre d'expérimentation), différé sans être annulé.

**Décision** :
1. Créer le jalon parent **V0.3_003 — Planning / Session Intent**, décomposé en V0.3_003A (ce jalon, architecture) → V0.3_003B (data-access/write path) → V0.3_003C (web weekly planner) → V0.3_003D (Today integration + régressions e2e) → V0.3_003E (rollout production).
2. **Aucune migration** — le schéma `planned_sessions` existant (contrainte `unique_planned_per_day` `(athlete_id, planned_date)` déjà présente depuis la baseline, RLS `planned_sessions_own_data` déjà `FOR ALL`/athlète-propre, `intervention` JSONB déjà prioritaire en lecture depuis M2_003) est déjà entièrement suffisant, vérifié depuis les migrations réelles.
3. **Architecture d'écriture verrouillée** : CRUD Supabase authentifié direct sous la RLS existante, aucune nouvelle Edge Function/RPC — `planningRepo.ts` reproduit exactement le pattern déjà prouvé de `checkinRepo.ts`. Justifié par la différence de nature entre `decisions` (historique immuable, nécessite un verrou `service_role`) et `planned_sessions` (intention mutable, conçue dès la baseline comme athlète-éditable).
4. **Contrat mutabilité, avec preuve source exacte** : `planned_sessions` = intention mutable, upsert sur `(athlete_id, planned_date)` ; `decisions` reste append-only et inchangée. Audit précis de `buildDailyPlan.ts:302` : `DailyPlan.planned_session_before = ctx.planned_session` directement — jamais `baseline` (variable locale non persistée = `raceProtocol.recommended_session ?? ctx.planned_session ?? fallback`). Conséquence : quand une intention athlète brute existait au moment d'un run, elle reste toujours préservée append-only — dans `decisions.daily_plan.planned_session_before` (JSONB, source de vérité riche exacte) et, coarsened, dans `decisions.planned_session_before` (colonne dénormalisée, jamais la source complète) — y compris quand le protocole de course l'emporte sur elle dans l'arbitrage réel. Quand aucune intention n'existait (`ctx.planned_session = null`), `planned_session_before` est légitimement `null` — le fallback ne supplante alors rien. Aucune UPDATE n'est introduite sur `decisions`, aucune ledger de révision `planned_sessions` créée. Aucun changement `DailyPlan`/moteur n'est requis : l'architecture existante satisfait déjà cet invariant sans modification.
5. **Colonnes engine-inertes — préservation empiriquement prouvée** : `primary_objective`/`planned_duration_min`/`planned_time_of_day`/`notes`/`training_block_id` confirmées non lues par le moteur (`getPlannedSessionFor` ne sélectionne que 3 colonnes). Un test local réel (stack Supabase locale, athlète scratch sanctionné via `testDb.ts`, aucun mock, supabase-js 2.112.3) a confirmé que l'upsert exact proposé pour le planificateur (payload omettant ces cinq colonnes) préserve intégralement leur valeur préexistante sur conflit, tout en mettant correctement à jour `session_type`/`intervention`/`planned_intent`/`source`. Contrat verrouillé : **OMIT AND PRESERVE** — aucun mécanisme read-merge-write, aucune clarification explicite à `NULL` pour ces cinq colonnes.
6. **DELETE vs REST verrouillé comme deux états distincts** : absence de row ⇒ repli T-X/inférence existant inchangé (aucune intention à supplanter) ; row `{kind:"REST"}` explicite ⇒ intention réellement déclarée, soumise au même arbitrage que toute autre séance planifiée.
7. **`planned_intent` mis à `NULL` systématiquement** par toute row écrite par le planificateur — audité comme le seul champ parmi les colonnes existantes ayant un effet moteur réel (dérogation de soft constraint `strong`), et l'UI V0.3_003 ne collecte aucune justification textuelle pour une telle dérogation.
8. **Catalogue de kinds verrouillé** : 16 `TrainingInterventionKind` réels au total, 15 plannables par l'athlète, `RACE_ACTIVITY` exclusivement moteur (protocole T-X/course en cours). Sélecteur de charge affiché uniquement pour les 11 `LoadVariableKind`.
9. **Mapping `session_type` réutilisé** : `trainingInterventionToSessionType.ts` (déjà le miroir documenté exact du mapping backend), aucune nouvelle table de correspondance ; stratégie de régression test-only par import source direct (frontière déjà prouvée V0.3_002F).
10. **Aucun mécanisme d'invalidation de plan affiché nécessaire** : `App.tsx` confirme une structure de routes plate sans layout persistant — le démontage de route entre `/today` et `/plan` rend le scénario de plan périmé structurellement impossible, tant que l'édition reste exclusivement sur `/plan` (invariant de conception verrouillé). Le résumé `/today` doit néanmoins recharger l'intention courante à chaque montage — aucun cache de planification persistant au-dessus de la route.
11. **Horizon UI v1** : 7 jours glissants, route `/plan` dédiée + résumé lecture-seule sur `/today`.
12. **Clôture V0.3_003E requise, pas optionnelle** : canary scratch remote (jamais l'athlète réel) et smoke web production sont tous deux des exigences de clôture, étant donné que ce jalon introduit un nouveau chemin d'écriture athlète-authentifié réel.
13. **`ActiveExperiment` explicitement différé, pas annulé** — redevient le candidat suivant après clôture V0.3_003 ; aucun couplage identifié entre les deux.

**Alternatives considérées** :
- Nouvelle Edge Function/RPC pour l'écriture des séances planifiées — **REJETÉE** : symétrie non justifiée avec `daily-run`/`decisions` ; `planned_sessions` a toujours été conçue comme athlète-mutable, la RLS existante est déjà l'unique autorité nécessaire.
- Conserver la valeur existante de `planned_intent` lors d'une édition planificateur — **REJETÉE** : risquerait une dérogation de soft constraint invisible et non voulue par l'athlète, faute de collecte de justification textuelle dans l'UI v1.
- Modéliser `planned_sessions` en append-only à l'image de `decisions` — **REJETÉE** : la contrainte d'unicité `(athlete_id, planned_date)` existante contredit ce modèle et n'a aucune justification produit ici.
- Mécanisme `planningRevision` symétrique à `checkinRevision` pour l'invalidation de `DailyPlanPanel` — **REJETÉE après audit réel du router** : le démontage de route entre `/today` et `/plan` rend ce mécanisme redondant.
- Traiter le canary remote 003E comme optionnel (par analogie avec le précédent `submit-review`) — **REJETÉE** : ce précédent s'appliquait à l'absence de candidat naturel en production pour un chemin de lecture/écriture déjà existant ; V0.3_003 introduit un chemin d'écriture RLS athlète-authentifié entièrement nouveau, qui exige une preuve empirique remote.
- Choisir ActiveExperiment plutôt que Planning comme prochain jalon — **REPORTÉE** : Planning active davantage de comportement moteur déjà construit et retire une opération hors-bande existante sur un usage quotidien normal ; ActiveExperiment reste un candidat plus étroit, non annulé.

**Impact** : `docs/00_PROJECT_STATUS.md` (phase actuelle), `docs/04_DAILY_DECISION_ENGINE.md` (roadmap §8), `docs/05_DATA_MODEL.md` (§`planned_sessions`, contrat futur explicitement au temps futur — implémentation non commencée), `docs/06_ARCHITECTURE.md` (nouvelle section §V0.3_003, incluant le contrat de clôture 003E requis), `docs/10_TEST_PLAN.md` (§T15-T18), `docs/12_BACKLOG.md` (phase actuelle, nouveau bloc V0.3_003A, clarification vs le planificateur automatique P2). Aucun changement `head-coach-engine/src/**`, `head-coach-engine/tests/**`, `supabase/migrations/**`, `web/**`, `longitudinal-engine/**`.

**Statut** :
V0.3_003A = CLOSED / ARCHITECTURE LOCKED
...
## 2026-08-31 — V0.3_003D : Today Integration + Real Local E2E CLOSED LOCALLY

**Contexte** : V0.3_003C (entrée ci-dessus) a clos le workflow web `/plan`. Cette entrée clôt la connexion locale de l'intention Planning à la boucle quotidienne (`/today`) et la preuve cross-stack réelle du chemin déjà documenté.

**Décision** :
1. Implémentation + durcissement (`05b3123fbec9c636839688e74c38f7f3096ec720`, `feat: integrate V0.3_003D planning with today`) poussés sur `origin/main`. Production : `web/src/features/planning/TodayPlanningSummary.tsx` (nouveau), `web/src/pages/TodayPage.tsx` (nouvelle section), `web/src/features/planning/PlanningDayCard.tsx` (correctif ciblé). Test/infrastructure de test : `TodayPlanningSummary.test.tsx`, `head-coach-engine/tests/supabase/testDbSafety.test.ts`, `head-coach-engine/tests/supabase/t17_planningE2E.integration.test.ts` (nouveaux) ; `TodayPage.test.tsx`, `PlanningDayCard.test.tsx`, `head-coach-engine/tests/supabase/testDb.ts` (modifiés). Aucun fichier `head-coach-engine/src/**` modifié.
2. **Today** : section lecture-seule "Prévu aujourd'hui", placée après la date et avant le check-in — représente uniquement la row `planned_sessions` courante de l'athlète, jamais le fallback/protocole de course/`DailyPlan.final_session` (section Head Coach distincte, plus bas). Réutilise `loadPlannedSessions(athleteId, today, today)` (003B, aucune nouvelle méthode). Chargement au montage uniquement — aucun contexte/cache/`planningRevision`, le démontage de route reste l'unique mécanisme de fraîcheur (`daily-run` ne mute jamais `planned_sessions`, confirmé par audit source). États : "Non planifié", "Repos", libellé riche partagé, label coarse legacy sûr (jamais l'enum brut, y compris pour une valeur runtime non mappée). Lien "Modifier dans Plan" vers `/plan` ; aucune mutation inline. Échec de lecture Planning non-critique ("Planning indisponible"), n'affecte jamais check-in/daily-run/décision Head Coach.
3. **Correctif ciblé 003C** : `PlanningDayCard.tsx` retombait sur l'enum brut `row.session_type` pour une row legacy non mappée — corrigé pour utiliser le même label générique sûr que `TodayPlanningSummary.tsx`. Défaut de cohérence UI découvert en revue, aucun changement de sémantique d'édition/sauvegarde/suppression/architecture d'état.
4. **T17 — preuve cross-stack réelle locale, CLOSED / PASS** : `t17_planningE2E.integration.test.ts` exécute `runDailyFor(admin, athleteId, date)` directement (même pattern que les fichiers d'intégration existants, jamais l'Edge Function HTTP) contre une vraie stack Supabase locale, athlètes scratch, aucun mock. Six scénarios destructifs, tous PASS : jamais planifié (repli `INFERENCE_FALLBACK` déterministe) ; planifié puis supprimé (scénario distinct, même convergence — DELETE ≠ REST) ; REST explicite (survit intact, `final_session.kind="REST"` sous conditions neutres) ; `STRENGTH_LOWER`/`HEAVY` (charge préservée, branche Nutrition force existante activée) ; `DH_TECHNICAL`/`MODERATE` (domaine Technique existant activé) ; **précédence protocole de course** — intention athlète `DH_TECHNICAL`/`MODERATE` + course réelle du jour ⇒ le protocole de course gagne l'arbitrage réel (`final_session.kind="RACE_ACTIVITY"`, coarse persisté `"RACE_PREP"`) tandis que `planned_session_before` reste exactement `DH_TECHNICAL`/`MODERATE` (riche et coarse, mémoire et `decisions.daily_plan.planned_session_before`/`decisions.planned_session_before` persistés) — preuve empirique directe, plus seulement auditée, de la distinction intention athlète vs baseline moteur/course. Chaque décision identifiée via `result.persistence.decision_id`, jamais par ordre non spécifié.
5. **Durcissement infrastructure de test partagée** : `head-coach-engine/tests/supabase/testDb.ts`'s `createTestClient()` refuse désormais un client privilégié contre toute cible autre que `http://127.0.0.1:54321`/`http://localhost:54321` (hôte et port stricts — un port différent ou un hôte 127.0.0.2 sont rejetés), avant toute mutation réseau. Corrige une lacune de sécurité pré-existante (aucun des fichiers d'intégration `head-coach-engine` existants n'avait de garde de cible locale avant ce jalon ; ces fichiers restent inchangés et continuent de fonctionner normalement contre la stack locale réelle — dette de test-infrastructure pré-existante, non introduite par 003D, non un blocker produit). Preuve négative empirique : URL de production + opt-in + clé locale valide présents ⇒ les 6 scénarios T17 restent SKIPPED, sans tentative réseau.
6. Résultats de test finaux : `web` par défaut **485 passed / 9 skipped** (494 total, 36 fichiers, aucune stack Supabase requise) ; `web` complet avec opt-in **494/494 PASS**. `head-coach-engine` avec clé locale sans opt-in T17 **378 passed / 6 skipped** (384 total) ; avec opt-in T17 **384/384 PASS** ; edge **9/9** ; `ENGINE_VERSION` inchangé.
7. Aucune migration, aucune Edge Function, aucune action Supabase explicite (production). **Aucune action de déploiement Vercel explicite dans V0.3_003D** — implémenté et poussé sur `origin/main` ; **l'état effectif du déploiement Vercel après ce push (auto-deploy Git éventuel ou non) n'a pas été vérifié dans ce jalon** ; la production n'est donc ni confirmée à jour ni confirmée non déployée — cette question reste ouverte et sera tranchée en premier par V0.3_003E.
8. Moteur strictement inchangé : `head-coach-engine/src/**` non touché, `DailyPlan` inchangé (aucun nouveau champ requis pour Today), `ENGINE_VERSION` inchangé (`head-coach-engine@0.2.0-m1-v0.3_002d`).
9. V0.3_003E (rollout production + clôture finale V0.3_003) devient le prochain jalon concret. Avant tout choix de commande de déploiement, le mécanisme Vercel actuel doit être réaudité — aucune commande n'est présumée à l'avance dans cette entrée.

**Alternatives considérées** : aucune nouvelle alternative architecturale — V0.3_003A/B/C restent la référence verrouillée ; ce jalon implémente le contrat déjà décidé et le prouve empiriquement, corrigé uniquement sur des défauts UI/infrastructure de test réels découverts en revue (points 3 et 5 ci-dessus, des durcissements, jamais des changements de contrat).

**Impact** : `docs/00_PROJECT_STATUS.md`, `docs/04_DAILY_DECISION_ENGINE.md`, `docs/05_DATA_MODEL.md`, `docs/06_ARCHITECTURE.md` (§V0.3_003), `docs/10_TEST_PLAN.md` (§T17), `docs/12_BACKLOG.md`. Aucun changement `head-coach-engine/src/**`, `supabase/migrations/**`, `longitudinal-engine/**`, `web/src/features/planning/{planningRepo,planningTypes,planningValidation}.ts`/`PlanPage.tsx` (003B/003C restent inchangés hors le correctif ciblé point 3).

**Statut** :
V0.3_003D = CLOSED LOCALLY
V0.3_003 (ensemble) = IN PROGRESS
V0.3_003E = NOT STARTED
Prochain jalon = V0.3_003E — rollout production + clôture finale V0.3_003

---

## 2026-09-02 — V0.3_003E : Production Rollout + V0.3_003 COMPLETE

**Contexte** : V0.3_003D (entrée ci-dessus) avait implémenté et poussé sur `origin/main` le workflow Planning complet (`/plan`, résumé "Prévu aujourd'hui", T17 réel local) sans jamais vérifier l'état effectif du déploiement Vercel. Cette entrée clôt le rollout production et, avec lui, V0.3_003 dans son ensemble.

**Décision** :
1. **Audit préalable** : le projet Vercel `louis-performance-system`/`nalynt` n'est pas connecté à Git — un push `origin/main` ne déclenche aucun déploiement automatique. Seul mécanisme : `npx vercel deploy --prod --scope nalynt` depuis `web/` (racine liée confirmée via `web/.vercel/project.json` : `prj_PmxPGlFwH5beHjzMFcwV1pOAf9CS`/`team_IiWY4r1APguWS767dwqkmrHe`/`louis-performance-system`). Ceci explique pourquoi V0.3_003C/003D n'avaient jamais atteint la production avant ce jalon. Cible Supabase confirmée `uvolpldwwyvadlamulvr` (`evynmzyjhobdpmxdiwsy` INACTIVE/non lié), migration parity 26/26, `daily-run` ACTIVE/version 2/`verify_jwt: true`.
2. **Premier déploiement — ÉCHEC, zéro impact production.** Build distant Vercel : `Command "npm run build" exited with 2`, alors que le build local identique passait. Cause : Vercel ne reçoit que `web/` (Root Directory) ; `tsc -b` du build de production type-vérifiait 3 tests `web/src/**` (`DailyPlanView.enriched.test.tsx`, `planningMappingParity.test.ts`, `planningRepo.integration.test.ts`) important par chemin relatif hors de cette frontière vers `head-coach-engine/**` — valides en local (monorepo entier présent), inexistants dans le contexte de build isolé Vercel. Défaut de frontière de build, non un défaut Planning/engine/Supabase. Alias de production resté sur le déploiement `Ready` précédent pendant tout l'échec ; déploiement échoué conservé comme preuve d'audit (jamais supprimé).
3. **Correctif build-only** (`ede2776dd75ee84ed5060298a679054fe86cb8cd`, `build: isolate web production typecheck`) : `web/tsconfig.build.json` (nouveau — `extends "./tsconfig.app.json"`, exclut `src/**/*.test.ts(x)`, `tsBuildInfoFile` distinct) + `web/package.json` (`build` → `tsc -b tsconfig.build.json && vite build` ; nouveau script `typecheck` → `tsc -b tsconfig.app.json`, préservant intégralement le graphe local complet). Aucun fichier `web/src/**` modifié, `package-lock.json` inchangé. Preuve avant nouvelle tentative : typecheck complet local PASS (3 tests cross-package confirmés toujours dans le graphe via `tsc --listFilesOnly`) ; build production local PASS ; build isolé dans une copie temporaire de `web/` sans `head-coach-engine/` sibling — FAIL reproduit sur HEAD pré-correctif (preuve négative), PASS post-correctif avec `dist/` produit (preuve positive) ; `web` par défaut **485 PASS / 9 SKIPPED**, opt-in complet **494/494 PASS**.
4. **Second déploiement — SUCCÈS.** `dpl_HQDuP893LoDSuqxdfzWXMbABmVzT`, alias canonique `https://louis-performance-system.vercel.app` → `Ready`/`production`. Bundle réellement servi (jamais inféré depuis `dist/` local) : `/assets/index-F_rQBrBr.js`, confirmé contenir `Plan`/`/plan`/`Prévu aujourd'hui`/`Modifier dans Plan`/Aujourd'hui/Historique/Insights ; project ref `uvolpldwwyvadlamulvr` présent, `evynmzyjhobdpmxdiwsy` absent. Smoke direct `/`, `/today`, `/plan`, `/history`, `/insights` : tous HTTP 200.
5. **T18 — canary remote authentifié, CLOSED / PASS.** Athlète/utilisateur scratch entièrement neufs (jamais l'athlète réel de Louis), créés/nettoyés via un script éphémère hors dépôt (jamais `testDb.ts`, dont la garde cible locale 003D n'a pas été touchée). Fixtures neutres (bloc `IN_SEASON`, check-in neutre, aucune course — scénario identique à T17 §20). **Écriture `planned_sessions` (`DH_TECHNICAL`/`MODERATE`) par le client authentifié scratch sous la policy RLS `planned_sessions_own_data` réelle, jamais `service_role`** ; relecture propre confirmée par ce même client. `daily-run` de production invoqué sous JWT scratch authentifié (jamais `service_role`) : `HTTP 200`, `engine_version` confirmé directement `head-coach-engine@0.2.0-m1-v0.3_002d`, `planned_session_before`/`final_session` riches `DH_TECHNICAL`/`MODERATE`/`DH_TECHNICAL`, `dh_or_technical.active=true`. Décision persistée relue via l'id exact retourné par l'endpoint (jamais "dernière ligne") : `daily_plan.planned_session_before` (JSONB riche) et projections dénormalisées coarse `planned_session_before`/`final_session` = `DH_TECHNICAL` confirmées. Canary remote de précédence course délibérément **SKIP** — déjà prouvé localement par T17 §21 ; cette passe vérifie déploiement/auth/RLS/Edge/persistance, pas une duplication du comportement moteur.
6. **Nettoyage** : complet et vérifié — 0 ligne restante sur `athletes`/`planned_sessions`/`daily_checkins`/`decisions`/`health_flags`/`training_blocks`, utilisateur d'authentification absent. **Écritures sur l'athlète réel : zéro** sur l'ensemble du jalon.
7. **Smoke web production** : bundle réellement servi + smoke de routes direct + canary RLS/`daily-run` authentifié bout-en-bout (ci-dessus). Smoke navigateur visuel authentifié : **non automatisé** (aucune dépendance d'automatisation navigateur ajoutée, non requis pour T18 — non-bloquant compte tenu des preuves directes ci-dessus).
8. **Post-canary** : `daily-run` reconfirmé `ACTIVE`/version 2/`verify_jwt: true` ; migration parity reconfirmée 26 locales/26 remote/0 en attente. Aucune migration, aucun redéploiement Edge, aucun changement `head-coach-engine/src/**`, `ENGINE_VERSION` inchangé sur l'ensemble de V0.3_003.
9. **V0.3_003 — Planning / Session Intent dans son ensemble est COMPLETE (2026-09-02)** : les 5 jalons de la décomposition verrouillée (003A→003E) sont tous clos. `/plan` et le résumé "Prévu aujourd'hui" servent en production, le chemin complet `athlète UI → écriture RLS authentifiée → lecture Today → RawContext → arbitrage → décision persistée → production` est prouvé de bout en bout, à la fois localement (T17) et en production réelle (T18).

**Alternatives considérées** : aucune nouvelle alternative architecturale pour Planning — V0.3_003A/B/C/D restent la référence verrouillée, ce jalon exécute le rollout déjà décidé. Pour le défaut de build découvert : option retenue = graphe TypeScript de production dédié (`tsconfig.build.json`, extension + exclusion de tests, `tsBuildInfoFile` distinct) plutôt que (a) exclure les tests du graphe local complet existant (aurait dégradé la couverture de type-check éditeur/local sans compensation), (b) élargir le Root Directory Vercel pour inclure tout le monorepo (aurait uploadé `head-coach-engine/`/`supabase/`/`docs/` inutilement et couplé la correction du build web à des packages non concernés), ou (c) dupliquer le code `head-coach-engine` dans `web/` (aurait fourché la source de vérité des tests cross-package — rejeté).

**Impact** : `web/package.json`, `web/tsconfig.build.json` (nouveau) — build-only, aucun changement `web/src/**`. `docs/00_PROJECT_STATUS.md`, `docs/04_DAILY_DECISION_ENGINE.md`, `docs/05_DATA_MODEL.md`, `docs/06_ARCHITECTURE.md` (§V0.3_003E), `docs/10_TEST_PLAN.md` (§T18), `docs/12_BACKLOG.md`. Aucun changement `head-coach-engine/src/**`, `head-coach-engine/tests/supabase/testDb.ts`, `supabase/migrations/**`, `supabase/functions/**`, `longitudinal-engine/**`. Aucune donnée réelle de Louis touchée.

**Statut** :
V0.3_003E = CLOSED / PRODUCTION ROLLOUT COMPLETE
V0.3_003 (ensemble) = COMPLETE
T18 = CLOSED / PASS
Prochain jalon = aucun sélectionné/architecturé à ce stade ; `ActiveExperiment` reste le candidat suivant, différé mais non annulé — décision de priorisation Louis restant à prendre.

---

## 2026-09-04 — V0.3_004A : Coaching Runtime Athlete-Scoped CLOSED LOCALLY

**Contexte** : `head-coach-engine/src/config/athleteCoachingProfile.ts` était un singleton de code — un focus Technique et un cue Mental écrits en dur — consommé sans condition par `domains/technique.ts`/`domains/mental.ts`. Tant qu'un seul athlète (Louis) existait, invisible. Dès qu'un second athlète apparaît, ce fichier lui aurait servi le contenu personnel de Louis. C'est la première violation concrète de l'invariant multi-athlète-par-construction, découverte en revue avant tout second utilisateur réel — jamais en production.

**Décision** :
1. Implémentation (`26c74a011c8b21941938e305d26b66adaf13f81f`, `feat: make coaching runtime athlete-scoped`) poussée sur `origin/main`. Nouvelle migration `supabase/migrations/20260903090000_v0_3_004a_athlete_coaching_profiles.sql` : table `athlete_coaching_profiles(athlete_id uuid PRIMARY KEY REFERENCES athletes(id) ON DELETE CASCADE, technique_primary_focus text, mental_pre_race_cue text, created_at, updated_at)`. `athlete_id` est directement la PK (config courante mutable, pas un historique daté) — au plus une ligne par athlète, absence de ligne = absence de personnalisation, jamais une erreur. Deux `CHECK` interdisent une chaîne vide/blanche mais acceptent `NULL` (convention `_not_blank` déjà établie par `decision_outcomes`). V1 délibérément minimal : seulement les deux valeurs textuelles réellement consommées aujourd'hui — aucun champ sans consommateur runtime.
2. RLS : policy `athlete_coaching_profiles_own_data` (`FOR ALL`, `USING`/`WITH CHECK` via `athlete_id IN (SELECT id FROM athletes WHERE user_id = auth.uid())`), même famille que `weekly_availability_own_data`/`training_blocks_own_data` (athlète édite directement, pas de RPC). Grants explicites après `REVOKE ALL` : `authenticated` → `SELECT, INSERT, UPDATE, DELETE` ; `service_role` → `SELECT, INSERT, UPDATE` (délibérément **pas** `DELETE` — aucun chemin ne supprime une ligne en gardant l'athlète, la suppression n'existe que via le `ON DELETE CASCADE` de la FK) ; `anon` → aucun grant.
3. Nouveau repository read-only `src/supabase/repositories/athleteCoachingProfileRepo.ts` (`getCoachingProfileFor`, filtré explicitement par `athlete_id` même si déjà PK) + mapping `src/supabase/mapping/coachingProfileRow.ts`. `buildRawContext.ts` peuple `RawContext.coaching_profile` uniquement si une ligne existe (`coachingProfileRow ? mapCoachingProfileRow(...) : undefined`, jamais un objet par défaut). Nouveau type `CoachingProfile` dans `types/context.ts`/`types/rawContext.ts`.
4. `domains/technique.ts`/`domains/mental.ts` : le focus/cue en dur devient un paramètre pur (`personalFocus`/`personalPreRaceCue`), lu depuis `ctx.coaching_profile` au point d'appel dans `buildDailyPlan.ts`. Absence de profil, ou champ `NULL` individuel → section correspondante simplement absente (`focus`/`action_hint` non peuplé), **jamais** un texte par défaut générique, **jamais** le contenu d'un autre athlète — les deux colonnes sont traitées comme individuellement optionnelles.
5. `domains/fallbackInference.ts` simplifié : le split hebdomadaire personnel à Louis (lundi haut / mardi bas / ...) est retiré — le fallback est désormais générique, identique sur les 7 jours de la semaine, quel que soit l'athlète (`{kind:"RECOVERY_ACTIVE"}`).
6. `ENGINE_VERSION` porté à `head-coach-engine@0.2.0-m1-v0.3_004a` (`tests/engineVersion.test.ts` reste l'unique propriétaire de l'assertion exacte).
7. Nouveau test d'intégration réel `athleteCoachingProfileRLS.integration.test.ts` (245 lignes) contre une vraie stack Supabase locale : isolation RLS de la nouvelle table. `buildRawContext.integration.test.ts` étendu pour couvrir la présence/absence de profil. Tests unitaires étendus : `t11_technique.test.ts`, `t12_mental.test.ts`, `t6_fallback.test.ts` (fallback générique 7 jours). Résultats locaux (hors suite d'intégration Supabase, qui nécessite la stack locale) : `head-coach-engine` unitaire **233/233**, edge **8/8**, build PASS.
8. Aucune écriture sur l'athlète réel de Louis à ce stade — la population de sa vraie ligne `athlete_coaching_profiles` est explicitement différée à V0.3_004D (rollout production).

**Alternatives considérées** : conserver le singleton de code avec une branche conditionnelle par `athlete_id` — rejeté (recrée exactement le problème sous une autre forme, ne scale pas, viole le principe "jamais d'identité hardcodée"). Dump de profil général (objectifs/équipement/disponibilité/notes) — rejeté, aucun consommateur runtime actuel, prématuré (YAGNI).

**Impact** : `head-coach-engine/src/{config/athleteCoachingProfile.ts (supprimé), config/{nutritionPolicy,techniquePolicy}.ts, domains/{fallbackInference,mental,technique}.ts, engine/buildDailyPlan.ts, supabase/buildRawContext.ts, supabase/mapping/coachingProfileRow.ts (nouveau), supabase/repositories/athleteCoachingProfileRepo.ts (nouveau), types/{context,rawContext}.ts}`, `supabase/migrations/20260903090000_v0_3_004a_athlete_coaching_profiles.sql` (nouveau). Documentation différée à V0.3_004D par design (voir commentaire en tête de la migration).

**Statut** :
V0.3_004A = CLOSED LOCALLY
V0.3_004 (ensemble) = IN PROGRESS
Prochain jalon = V0.3_004B — bootstrap athlète authentifié

---

## 2026-09-04 — V0.3_004B : Authenticated Athlete Bootstrap CLOSED LOCALLY

**Contexte** : jusqu'ici, la ligne `athletes` de Louis existait déjà en base au moment où M4 a été construit — aucun chemin applicatif ne créait un athlète pour un nouvel utilisateur authentifié. `RequireAuth` supposait silencieusement qu'un athlète existait déjà. C'est la deuxième violation concrète de l'invariant multi-athlète : un second utilisateur réel authentifié n'avait tout simplement aucun moyen d'obtenir sa propre ligne `athletes`.

**Décision** :
1. Implémentation (`e4a6ac54ab53151d6591d853f9c499eef58082fc`, `feat: add authenticated athlete bootstrap`) poussée sur `origin/main`. `AuthContext.tsx` : nouvelle méthode `refreshAthlete()` (re-résout l'athlète courant sous RLS, jamais un état optimiste). `RequireAuth.tsx` : quand l'utilisateur authentifié résout `no_athlete`, rend `<AthleteBootstrap/>` au lieu de l'app normale.
2. Nouveau `web/src/features/athleteBootstrap/AthleteBootstrap.tsx` : formulaire minimal (un seul champ, `name` — la seule colonne `athletes` réellement `NOT NULL` sans défaut), jamais un flux d'onboarding complet. Soumission → `createOwnAthlete(user.id, name)` (`athleteBootstrapRepo.ts`, écrit `{user_id, name}` sous la policy RLS `athletes_own_data` déjà existante, **jamais** `service_role`) → **toujours** `refreshAthlete()` ensuite, y compris après un insert échoué — un double-clic, deux onglets, ou une requête retentée peuvent légitimement entrer en course avec la contrainte `UNIQUE(user_id)` tout en laissant une ligne réelle et utilisable créée par la tentative gagnante ; seul l'état re-résolu décide si c'est réellement une impasse, jamais le résultat brut de l'insert seul.
3. `athleteBootstrapValidation.ts` : validation du nom (non vide/non blanc, longueur bornée) avant tout appel réseau.
4. Aucune nouvelle policy RLS, aucun nouveau grant, aucune migration — le chemin d'écriture réutilise `athletes_own_data` (`FOR ALL`, `USING`/`WITH CHECK (user_id = auth.uid())`), déjà présente depuis la baseline V0.2/M2.
5. Tests : `AthleteBootstrap.test.tsx` (nouveau, 129 lignes), `athleteBootstrapRepo.integration.test.ts` (nouveau, 206 lignes, RLS réelle contre stack locale), `AuthContext.test.tsx`/`RequireAuth.test.tsx` étendus. Résultats locaux : `web` **499 passed / 15 skipped** (514 total, incluant les nouveaux fichiers), build PASS. Moteur `head-coach-engine` strictement inchangé — aucun fichier `head-coach-engine/src/**` touché, `ENGINE_VERSION` inchangé.
6. Aucune écriture sur l'athlète réel de Louis (Louis résout déjà `no_athlete=false`, ce chemin ne s'active jamais pour lui). Aucun déploiement remote (Supabase ou Vercel) dans ce jalon.

**Alternatives considérées** : bootstrap automatique silencieux (créer une ligne `athletes` par défaut sans interaction) — rejeté, un athlète mérite un minimum d'intention explicite (son nom), et un bootstrap totalement silencieux masquerait un bug d'auth réel derrière un athlète fantôme. Bootstrap via `service_role`/Edge Function dédiée — rejeté, RLS directe suffit déjà et évite une surface serveur supplémentaire pour une opération strictement self-service.

**Impact** : `web/src/auth/{AuthContext.tsx, RequireAuth.tsx}`, `web/src/features/athleteBootstrap/{AthleteBootstrap.tsx, athleteBootstrapRepo.ts, athleteBootstrapValidation.ts}` (nouveaux). Aucun changement `head-coach-engine/src/**`, `supabase/migrations/**`, `supabase/functions/**`.

**Statut** :
V0.3_004B = CLOSED LOCALLY
V0.3_004 (ensemble) = IN PROGRESS
Prochain jalon = V0.3_004C — contexte d'entraînement non configuré

---

## 2026-09-04 — V0.3_004C : Unconfigured Training Context (UNSPECIFIED) CLOSED LOCALLY

**Contexte** : avant ce jalon, `buildRawContext.ts` levait `NoCurrentTrainingBlockError` dès qu'un athlète n'avait aucune ligne `training_blocks` avec `is_current=true` — un état parfaitement normal et attendu pour tout nouvel athlète juste après son bootstrap (V0.3_004B). C'est la troisième violation concrète de l'invariant multi-athlète : un second utilisateur réel, fraîchement bootstrappé, aurait vu `daily-run` échouer en 4xx/5xx avant même d'avoir pu configurer un premier bloc d'entraînement.

**Décision** :
1. Implémentation (`829687531d4797c05c001f394847dc662d5e6bff`, `feat: support unconfigured training context`) poussée sur `origin/main`, HEAD actuel. Nouvelle migration `supabase/migrations/20260904090000_v0_3_004c_training_mode_unspecified.sql` : `ALTER TYPE public.training_mode ADD VALUE 'UNSPECIFIED'` — purement additive, aucune table/colonne/RLS/grant touchée, aucun backfill, aucune ligne `training_blocks` existante modifiée.
2. `trainingBlocksRepo.ts` : `getCurrentTrainingModeRaw` remplacé par `getCurrentTrainingBlock` (retourne la ligne entière ou `null`, plutôt qu'une chaîne brute ou `null` indifférenciée). `buildRawContext.ts` : `currentBlock === null ? "UNSPECIFIED" : parseTrainingMode(currentBlock.mode)` — absence de ligne courante ⇒ état explicite `UNSPECIFIED` (jamais une erreur, jamais une phase devinée) ; une ligne courante qui existe mais porte un `mode` malformé continue de lever `InvalidTrainingModeError` via `parseTrainingMode`, exactement comme avant — les deux états (absence vs donnée invalide) restent délibérément distincts, jamais confondus. `NoCurrentTrainingBlockError` retirée (classe entièrement supprimée, plus aucun appelant).
3. `rules/modes.ts` : `getModeSoftConstraints("UNSPECIFIED")` retourne `[]` (ignorance conservatrice), regroupé avec les autres modes neutres (`OTHER`/`IN_SEASON`/`PRE_SEASON`/`OFF_SEASON_DEVELOPMENT`) — jamais confondu avec un mode protecteur (`RACE_WEEK`/`INJURY_RECOVERY`).
4. `supabase/functions/daily-run/errorMapping.ts` : le mapping HTTP de `no_current_training_block` retiré (l'erreur n'existe plus). `web/src/features/dailyPlan/dailyRunErrors.ts` : cas correspondant retiré côté client.
5. Comportement du moteur en `UNSPECIFIED` prouvé par `t6_fallback.test.ts` (scénarios E/F/G) : neutre + pas de plan + pas de course ⇒ `RECOVERY_ACTIVE` valide ; `suspected_concussion` ⇒ SAFETY (`REST`) reste seule autorité, inchangée ; course en cours ⇒ le protocole de course garde le contrôle (`RACE_ACTIVITY`, jamais `RECOVERY_ACTIVE`, jamais une contrainte de mode). `modesConstraints.test.ts` prouve `getModeSoftConstraints("UNSPECIFIED") === []` et son regroupement avec les modes neutres.
6. `ENGINE_VERSION` porté à `head-coach-engine@0.2.0-m1-v0.3_004c` (valeur finale de V0.3_004, confirmée en production par V0.3_004D). Résultats locaux : `head-coach-engine` unitaire **233/233**, edge **8/8** (un cas retiré avec `no_current_training_block`, contre 9/9 avant), build PASS ; `web` **499 passed / 15 skipped**, build PASS.
7. Aucune écriture sur l'athlète réel de Louis (sa ligne `training_blocks.is_current=true` existe déjà, ce chemin ne s'active jamais pour lui). Aucun déploiement remote dans ce jalon — migration/Edge/Vercel tous différés à V0.3_004D.

**Alternatives considérées** : fabriquer un mode par défaut plausible (ex. `IN_SEASON`) pour un athlète sans bloc courant — rejeté explicitement, violerait la contrainte canonique "aucune donnée historique/état fabriqué" (`docs/05_DATA_MODEL.md`) ; `UNSPECIFIED` est une vérité honnête ("pas encore configuré"), pas une supposition déguisée. Bloquer `daily-run` tant qu'aucun bloc n'est configuré (statu quo) — rejeté, contradictoire avec le bootstrap self-service de V0.3_004B qui doit mener à un premier run utilisable.

**Impact** : `head-coach-engine/src/{engine/buildDailyPlan.ts, rules/modes.ts, supabase/buildRawContext.ts, supabase/mapping/trainingMode.ts, supabase/repositories/trainingBlocksRepo.ts, types/context.ts}`, `supabase/functions/daily-run/errorMapping.ts`, `web/src/features/dailyPlan/dailyRunErrors.ts`, `supabase/migrations/20260904090000_v0_3_004c_training_mode_unspecified.sql` (nouveau).

**Statut** :
V0.3_004C = CLOSED LOCALLY
V0.3_004 (ensemble) = IN PROGRESS
Prochain jalon = V0.3_004D — rollout production + clôture finale (preuve empirique à deux athlètes)

---

## 2026-09-04 — V0.3_004D : Production Rollout + Two-User Foundation Closure CLOSED / PRODUCTION ROLLOUT COMPLETE

**Contexte** : V0.3_004A/B/C (entrées ci-dessus) ont corrigé, localement, les trois violations concrètes de l'invariant multi-athlète découvertes en revue (profil de coaching hardcodé mono-athlète, absence de chemin de bootstrap pour un second utilisateur, échec dur sur contexte d'entraînement non configuré). Cette entrée clôt le rollout production des deux migrations en attente et, pour la première fois, **prouve empiriquement en production réelle** — pas seulement localement — que deux utilisateurs authentifiés distincts et non liés peuvent coexister sans fuite ni interférence, du bootstrap à `daily-run`.

**Décision** :
1. **Garde-fous préalables** : HEAD confirmé `829687531d4797c05c001f394847dc662d5e6bff` = `origin/main`, branche `main`, arbre propre. Cible production confirmée `uvolpldwwyvadlamulvr` (`evynmzyjhobdpmxdiwsy` jamais touché). Inventaire de migration : 26 remote / 28 local, exactement les deux attendues (`20260903090000` 004A, `20260904090000` 004C) en attente — aucune dérive inattendue.
2. **Déploiement migrations** : `supabase db push --project-ref uvolpldwwyvadlamulvr` — les deux migrations appliquées, parité reconfirmée 28/28 immédiatement après, et une seconde fois après l'ensemble du canary (aucune dérive pendant l'exécution). Vérification SQL directe post-déploiement : table `athlete_coaching_profiles` présente avec RLS activée et les grants exacts documentés en V0.3_004A (`authenticated`: SELECT/INSERT/UPDATE/DELETE ; `service_role`: SELECT/INSERT/UPDATE, pas DELETE ; `anon`: aucun) ; `enum_range(null::training_mode)` contient `UNSPECIFIED`.
3. **Profil de coaching réel de Louis** : ligne `athletes` de production confirmée unique (`count(*)=1`) avant toute résolution — évite toute ambiguïté d'identité, jamais un UUID statique ni une résolution par email/nom. `athlete_coaching_profiles` confirmée à 0 ligne avant écriture. Upsert (`ON CONFLICT (athlete_id) DO UPDATE`) des deux valeurs approuvées : `technique_primary_focus = "Fixe ta ligne, dose le freinage, laisse rouler."`, `mental_pre_race_cue = "Comme à Wiriehorn."` — les mêmes chaînes déjà utilisées comme référence de comportement par les tests T11/T12 depuis V0.3_002B/C, désormais leur source de vérité runtime réelle en production. Vérifié : exactement 1 ligne, les deux chaînes exactement égales aux valeurs approuvées, aucune ligne dupliquée.
4. **Déploiement Edge** : `daily-run` redéployée (`npm run deploy:daily-run`, cible `uvolpldwwyvadlamulvr` uniquement, aucune autre fonction touchée) — bundle confirmé inclure `athleteCoachingProfileRepo.js`/`trainingBlocksRepo.js`/`modes.js`/`fallbackInference.js`/`trainingMode.js`/`buildRawContext.js`/`buildDailyPlan.js` mis à jour. Post-déploiement : `ACTIVE`, version 2→3, `verify_jwt: true` préservé.
5. **Déploiement web** : premier build local PASS, `vercel deploy --prod --scope nalynt` réussi (`dpl_BsgxvxdPG3vtP7iKsiPcoQryAMuW`), alias canonique `https://louis-performance-system.vercel.app` reconfirmé pointer sur ce déploiement. Smoke direct des 5 routes canoniques (`/`, `/today`, `/plan`, `/history`, `/insights`) : tous HTTP 200 — insuffisant seul, donc complété par une preuve de contenu : le bundle réellement servi contient les chaînes de la nouvelle UI de bootstrap ("Configurer ton profil", "Continuer").
6. **Preuve empirique à deux athlètes (canary production éphémère, hors dépôt, jamais commité)** : deux utilisateurs Auth scratch créés (emails générés uniques, jamais imprimés, jamais l'email de Louis), tous deux vérifiés à 0 ligne `athletes` au départ. Bootstrap réel de chacun via le **chemin authentifié réel** (client signé avec le mot de passe scratch, jamais `service_role`) sous la policy `athletes_own_data` — chacun résout exactement une ligne `athletes`, les deux ids distincts. **Preuve négative de bootstrap** : A ne peut pas forger une ligne `athletes` pour `user_id=B` (rejeté par RLS, aucune ligne créée) ; une seconde tentative d'A pour son propre `user_id` est rejetée par `UNIQUE(user_id)`, A garde exactement une ligne.
7. **Isolation Planning complète (8 points, sous clients authentifiés réels, jamais `service_role`)** : intentions délibérément différentes — A = `AEROBIC_BASE`/`LIGHT`, B = `DH_TECHNICAL`/`MODERATE` (ce dernier choisi spécifiquement pour activer le domaine Technique et rendre observable la preuve anti-fuite du point 9). Chacun lit sa propre row, aucun ne lit celle de l'autre (vide), aucun ne peut modifier/supprimer celle de l'autre (vérifié via lecture admin post-tentative : contenu inchangé).
8. **`daily-run` réel en production, sous JWT de chaque utilisateur (jamais `service_role`)** : A → `HTTP 200`, `engine_version = "head-coach-engine@0.2.0-m1-v0.3_004c"`, `planned_session_before` = intention exacte d'A, décision persistée appartenant à A. **B — la preuve zéro-bloc, critique** : `training_blocks` de B confirmé à 0 ligne avant **et** après l'appel (aucune création cachée) ; `HTTP 200` ; `active_mode` = `"UNSPECIFIED"`, en mémoire **et** dans la décision persistée ; `planned_session_before` = intention exacte de B ; décision appartenant exclusivement à B.
9. **Preuve anti-fuite de personnalisation** : dans les réponses `dailyPlan` d'A et de B, les deux chaînes exactes de Louis (`technique_primary_focus`/`mental_pre_race_cue`) sont confirmées **absentes**. Chez B, le domaine Technique est confirmé `active:true` (grâce au choix du point 7) alors que son `focus` est confirmé **absent** — la preuve la plus forte possible : le domaine s'active correctement selon la logique métier, mais aucun contenu personnel d'un autre athlète n'apparaît, car B n'a lui-même aucun profil configuré.
10. **Isolation historique/decisions** : chacun ne lit que ses propres décisions (jamais celles de l'autre, testé dans les deux sens). Reachability basique `get-insights` : `HTTP 200` pour les deux, aucune activation de la Couche D testée (hors périmètre de ce jalon). **Matrice négative additionnelle** : A ne peut ni insérer ni lire le profil de coaching de B ; A ne peut pas insérer une ligne `training_blocks` forgée pour B. **Injection `athlete_id`** : une tentative d'A d'invoquer `daily-run` avec `{date, athlete_id: <B>}` dans le corps est rejetée `HTTP 400` (`ALLOWED_BODY_KEYS=["date"]`) — l'athlète est exclusivement résolu côté serveur via RLS, jamais depuis l'entrée client.
11. **Nettoyage complet et preuve de résidu zéro** : suppression en cascade via FK (`athletes.id` → cascade sur `planned_sessions`/`daily_checkins`/`decisions`/`training_blocks`/`athlete_coaching_profiles`) puis suppression des deux utilisateurs Auth. Vérifié par comptage explicite sur chaque table pour chaque id trackée : zéro partout, les deux utilisateurs Auth confirmés absents (`getUserById`).
12. **Régression Louis (avant/après le canary)** : 7 comptages (`athletes`, `planned_sessions`, `decisions`, `training_blocks`, `athlete_coaching_profiles`, `daily_checkins`, `health_flags`) capturés immédiatement après la population du profil (point 3) puis re-capturés après nettoyage complet du canary — identiques bit à bit. Seule mutation intentionnelle sur l'ensemble du jalon : la ligne `athlete_coaching_profiles` de Louis (point 3). **Aucun `daily-run` exécuté pour Louis dans ce jalon** (choix délibéré, conforme à l'instruction de rollout).
13. **Session complétée (T-optionnel)** : délibérément **SKIP** — n'aurait fait qu'étendre le canary sans preuve supplémentaire pertinente pour la fermeture de l'invariant multi-athlète ; les suites A/B/C locales déjà vertes couvrent ce chemin.
14. **Vérification finale post-canary** : parité de migration reconfirmée 28/28, `daily-run` reconfirmé `ACTIVE`/version 3/`verify_jwt: true` — production stable et inchangée par l'exécution du canary lui-même.
15. **V0.3_004 — Multi-Athlete Foundation Hardening dans son ensemble est COMPLETE (2026-09-04)** : les 4 jalons de la décomposition (004A→004D) sont tous clos. Les trois violations concrètes de l'invariant multi-athlète (profil hardcodé, bootstrap absent, contexte d'entraînement bloquant) sont corrigées et, pour la première fois dans ce projet, **prouvées empiriquement en production réelle avec deux utilisateurs authentifiés distincts et non liés**, du bootstrap à `daily-run`, sans fuite ni interférence dans un seul sens vérifié.

**Alternatives considérées** : aucune nouvelle alternative architecturale — V0.3_004A/B/C restent la référence verrouillée ; ce jalon exécute le rollout déjà décidé et le prouve empiriquement à deux athlètes réels plutôt qu'un seul scratch (durcissement délibéré par rapport au pattern à un seul scratch utilisé par V0.3_002F/V0.3_003E, jugé insuffisant pour clore spécifiquement un invariant *multi*-athlète).

**Impact** : `docs/00_PROJECT_STATUS.md`, `docs/05_DATA_MODEL.md`, `docs/06_ARCHITECTURE.md` (§V0.3_004), `docs/10_TEST_PLAN.md` (§V0.3_004), `docs/11_DECISION_LOG.md` (cette entrée). Aucun changement de code — V0.3_004D est un rollout + preuve, pas une nouvelle implémentation. Une seule mutation de donnée réelle intentionnelle : `athlete_coaching_profiles` de Louis.

**Statut** :
V0.3_004A = CLOSED LOCALLY
V0.3_004B = CLOSED LOCALLY
V0.3_004C = CLOSED LOCALLY
V0.3_004D = CLOSED / PRODUCTION ROLLOUT COMPLETE
V0.3_004 (ensemble) = COMPLETE
Prochain jalon = aucun sélectionné/architecturé à ce stade ; `ActiveExperiment` reste le candidat suivant, différé mais non annulé — décision de priorisation Louis restant à prendre.

---

## 2026-09-08 — V0.3_005 : Dogfood Round 1 Corrections COMPLETE / PRODUCTION ROLLOUT COMPLETE

**Contexte** : premier cycle de dogfood réel (Louis utilisant le produit en conditions réelles, hors développement) a révélé sept défauts concrets, groupés en tickets NAL-001 à NAL-007, plus NAL-007A — un défaut d'architecture découvert et corrigé pendant la revue de NAL-007 avant tout commit. Aucune nouvelle fonctionnalité : uniquement des corrections directement motivées par l'usage réel.

**Décision — NAL-001 (Activité engagée), CLOSED / PROD (`02fdce3fd423159f28156e0e95b18dd7cf8b3239`, `feat: preserve committed activity during race taper`)** :
1. Défaut observé : une journée réellement engagée (ex. sortie DH prévue) était remplacée par une activité d'une autre famille (ex. renforcement, aérobie) simplement parce qu'une recommandation T-X générique s'appliquait ce jour-là — le protocole T-X substituait toute la famille d'activité sans jamais savoir si l'athlète comptait réellement faire cette séance.
2. Root cause tracée précisément : `buildDailyPlan.ts` faisait `baseline = raceProtocol.recommended_session` inconditionnellement dès qu'un `raceProtocol` existait, sans jamais consulter `ctx.planned_session`. Les deux exemples réels du dogfood correspondent exactement aux entrées T-7 (`STRENGTH_UPPER`/léger) et T-5 (`AEROBIC_BASE`/léger) de la table `PRE_EVENT_TABLES`.
3. Modèle verrouillé : nouveau champ purement additif `planned_sessions.is_committed BOOLEAN NOT NULL DEFAULT FALSE` (migration `20260904100000`), jamais fusionné dans `TrainingIntervention` (métadonnée de planification distincte, cohérente avec `RawContext.planned_session_committed` séparé de `RawContext.planned_session`). Nouvelle fonction pure `rules/committedActivityFamily.ts#preserveCommittedActivityFamily` réutilisant exclusivement des destinations déjà établies ailleurs dans le moteur (DH-family → `DH_LIGHT`, downgrade de charge) — aucun nouveau `kind` inventé.
4. Branches hard vs ordinaires : `RaceProtocolRecommendation` gagne un champ `hard: boolean`, calculé à la source (`in_progress` → hard, `POST_EVENT` → hard, toute entrée PRE_EVENT dont le `kind` recommandé est `REST` ou `RACE_ACTIVITY` → hard). Une séance engagée face à une branche hard reste intégralement surclassée — l'engagement n'est jamais absolu. `computeEventContext`/`raceProtocol.ts`'s date windows/`UpcomingRace` restent inchangés.
5. `ENGINE_VERSION` → `head-coach-engine@0.2.0-m1-v0.3_005a`.
6. Tests : 13/13 (`committedActivityFamily.test.ts`, pure logique) + 16/16 (`t4_raceProtocol.test.ts`, matrice complète flexible/engagé × hard/ordinaire, incluant les deux bugs dogfood exacts en régression littérale) + 2/2 preuve DB→engine réelle (`committedActivityFamily.integration.test.ts`, contre une vraie stack Supabase locale).

**Décision — NAL-003 (Restauration de la décision persistée), CLOSED / PROD (`16220c8c68561b09ccd9937160d4512e6b6af6ac`, `feat: restore persisted daily decision`)** :
1. Défaut observé : après check-in + génération du plan, naviguer puis revenir sur `/today` faisait disparaître le plan — l'athlète devait le régénérer, alors que la décision était déjà persistée (append-only, inchangé).
2. Nouveau `historyRepo.ts#loadLatestDecisionForDate(athleteId, date)` réutilise la policy RLS `decisions_own_select` déjà existante — aucune nouvelle policy. Correction critique découverte en revue : la sélection ne doit jamais être simplement "la plus récente" (`created_at DESC LIMIT 1`) mais **la plus récente VALIDE** — toutes les rows du jour sont chargées, triées, et la première dont `daily_plan` passe `isValidDailyPlan` (la même garde déjà utilisée par `/history`) est retenue, pour qu'une row plus récente mais malformée/legacy ne masque jamais une décision valide plus ancienne le même jour.
3. `daily-run` n'est jamais invoqué par ce chemin de lecture. Une erreur de lecture (`TodayDecisionLoadError`, distincte de `HistoryLoadError`) reste un état UI séparé, jamais confondu avec "aucune décision, génère un plan". Le bouton "Générer mon plan" existant sert aussi de régénération explicite — aucun nouveau mécanisme d'invalidation inventé.
4. Tests : `historyRepo.test.ts` (21/21, incluant les 3 régressions latest-valid-selection : newest-invalide+older-valide, newest-valide+older-valide, tout-invalide→null) + `DailyPlanPanel.test.tsx` (23/23, incluant zéro flicker "Générer", zéro second `daily-run`, remount restaure le même plan).

**Décision — NAL-007 (Race Calendar × Planning overlay), CLOSED / PROD (`f5b883c88b08ae5d11302f58fc863cb7d12cd11a`, `feat: show race calendar in planning`)** :
1. Défaut observé : `/plan` affichait "Non planifié" pour des jours où une course était pourtant déjà connue de `race_calendar`.
2. Nouveau `web/src/features/planning/raceOverlayRepo.ts` — lecture seule stricte, `race_calendar` reste l'unique source de vérité. Affichage sur chaque jour de l'horizon `/plan` (aujourd'hui→J+6) dont `[start_date, end_date]` intersecte l'horizon. `RACE_ACTIVITY` reste exclusivement dérivé du moteur, jamais athlete-plannable ; aucune écriture `planned_sessions` dérivée d'une course.
3. "Non planifié" devient "Aucune séance ajoutée" spécifiquement quand une course existe sans séance planifiée (jamais l'inverse — la copie normale reste inchangée pour un jour réellement vide).
4. Tests : `raceOverlayRepo.test.ts` (13/13, incluant A-E multi-jour/chevauchement d'horizon partiel) + isolation RLS réelle A/B contre Supabase local (`planningRepo.integration.test.ts`, 21/21).

**Décision — NAL-007A (Contrat de statut de course pour la pertinence coaching), CLOSED / PROD (`aa647208f0f1cd268ec43871f1445281ffdde5a4`, `fix: respect race status in coaching context`)** :
1. Défaut d'architecture découvert pendant la revue de NAL-007, avant tout commit : `race_calendar.status` n'avait **aucun** consommateur applicatif — ni écrivain (aucun chemin ne le modifie jamais), ni lecteur côté moteur (`UpcomingRace`/`getRacesInWindow` ne le sélectionnaient même pas). L'overlay Planning filtrait déjà `planned`/`registered`/`confirmed`, créant une incohérence potentielle : un événement `cancelled`/`skipped` resterait invisible en Planning mais pleinement actif côté moteur (taper, `RACE_ACTIVITY`, récupération).
2. Table de vérité canonique verrouillée (`docs/04_DAILY_DECISION_ENGINE.md` §Statut de la course) : `planned`/`registered`/`confirmed` → toujours inclus ; `cancelled`/`skipped` → exclus sans condition de toutes les phases ; `completed` → exclu de PRE_EVENT/IN_PROGRESS mais **inclus** pour POST_EVENT (aucun writer ne transitionne jamais vers `completed`, l'exiger désactiverait silencieusement la récupération post-course pour toute vraie course).
3. Implémentation strictement dans la couche adaptateur (`raceCalendarRepo.ts#isRaceCoachingRelevant`) — `UpcomingRace`, `computeEventContext`, `raceProtocol.ts` (M1, frozen) restent status-unaware, exactement comme avant. Aucune migration (`status` et son enum existent déjà).
4. `ENGINE_VERSION` → `head-coach-engine@0.2.0-m1-v0.3_005b` — seul bump de provenance de toute la série V0.3_005 (NAL-002/003/004/005/006/007 sont des changements web-only, sans effet sur `DailyPlan`).
5. Tests : `raceCalendarStatus.test.ts` (12/12, table de vérité pure) + `buildRawContext.integration.test.ts` §V0.3_005B (26/26 réel contre Supabase local, incluant l'interaction directe avec NAL-001 : une course `confirmed` engagée continue de déclencher `COMMITTED_FAMILY_PRESERVED`, une course `cancelled` n'atteint jamais l'arbitrage race-protocol).

**Décision — NAL-002 / NAL-005 / NAL-006 (Polish UI/wording), CLOSED / PROD (`19efd9133e6e05e31801601cc504c7f962f67c2c`, `fix: polish dogfood user-facing copy`)** :
1. **NAL-002** : `dailyPlan.sleep.notes` (chaîne engine fixe contenant `PROVISIONAL`/référence `docs/03_COACHING_MODEL.md`) était rendue verbatim dans `DailyPlanView.tsx`. Fix côté présentation uniquement — la donnée engine reste inchangée et jamais supprimée (toujours visible dans `decisions.daily_plan`/le panneau de debug dev-only). Audit étendu a aussi trouvé le rendu brut de `DailyRunResponse.warnings[]` (section "Avertissements") — deux origines confirmées produisant des références `docs/` (`mapPlannedSessionRow.ts`, `mapRaceCalendarRow.ts`) : rendu supprimé entièrement (ces avertissements n'ont jamais été de la copie athlète, seulement du diagnostic interne).
2. **NAL-005** : au-delà des deux chaînes littéralement observées ("How did today go?"/"Log session"), audit source a trouvé et corrigé : "Password"/"Sign in"/"Loading…"/deux messages "Configuration error"/"Daily Check-in"/"Daily Plan"/4× "Logout"/"Edit"/"Save"/deux questions douleur en anglais. Garde finale (guard séparé, avant commit) : `LoginPage.tsx` rendait `signInError.message` (texte brut du provider Supabase Auth, ex. "Invalid login credentials") directement à l'athlète — remplacé par une copie française unique ("Impossible de se connecter. Vérifie tes identifiants et réessaie."), aucun mapping de code d'erreur introduit (l'app ne distinguait déjà aucune raison d'échec). Aucun framework i18n.
3. **NAL-006** : audit du contrat RPE confirmé cohérent avant tout changement — `completed_sessions.rpe integer CHECK (rpe >= 0 AND rpe <= 10)`, alimentant `session_load = actual_duration_min * rpe / 10.0` (formule sRPE conventionnelle confirmant que `rpe` est déjà traité comme un effort global de séance, jamais une dimension isolée). Seuls le libellé ("Effort global ressenti") et l'aide ("À quel point cette séance t'a sollicité globalement ?") ont changé — champ persisté, plage, payload, validation strictement inchangés.

**Décision — V0.3_005D (Rollout production + clôture finale)** :
1. **Garde-fous préalables** : HEAD confirmé `19efd9133e6e05e31801601cc504c7f962f67c2c` = `origin/main`, branche `main`, arbre propre. Cible production confirmée `uvolpldwwyvadlamulvr`.
2. **Migration** : inventaire préalable confirme exactement une migration en attente (`20260904100000_v0_3_005a_planned_sessions_is_committed.sql`), aucune dérive inattendue. Déployée via `db push` ; parité 29/29 confirmée avant et après ; colonne vérifiée par requête directe (`boolean`, `NOT NULL`, `default=false`).
3. **Edge** : `daily-run` seule redéployée (les deux seuls fichiers engine modifiés — `rules/committedActivityFamily.ts` et `supabase/repositories/raceCalendarRepo.ts` — appartiennent tous deux à son graphe de dépendances). Bundle confirmé inclure les deux fichiers changés. Post-déploiement : `ACTIVE`, version 3→4, `verify_jwt: true` préservé.
4. **Web** : build local PASS, `vercel deploy --prod --scope nalynt` réussi (`dpl_4pKuKiVjY8g7FuudwubvXhN2BAoh`), alias canonique `https://louis-performance-system.vercel.app` confirmé repointé. 5 routes canoniques `HTTP 200`. Preuve de contenu (au-delà du simple 200) : le bundle réellement servi contient les 9 marqueurs attendus des huit corrections ("Activité engagée", "Aucune séance ajoutée", "Effort global ressenti", "sollicité globalement", "Course / événement", "Impossible de se connecter", "Repère générique", "Déconnexion", "Check-in du jour").
5. **Canary production à deux athlètes scratch distincts et non liés** (script éphémère hors dépôt, jamais commité, supprimé après exécution) : bootstrap réel des deux via le chemin authentifié (jamais `service_role`) → athlète A reçoit une séance engagée `DH_PERFORMANCE`/`HEAVY` + une course `confirmed` à T-5 (même contexte que le dogfood réel) ; athlète B reçoit la même séance engagée + une course `cancelled` sur les mêmes dates T-5. Preuve de persistance de l'engagement : lu `TRUE`, une édition d'un champ non lié (charge) n'affecte pas `is_committed`, relu `TRUE`. `daily-run` réel sous JWT de chacun (jamais `service_role`) : athlète A → `HTTP 200`, `engine_version=..._005b`, `planned_session_before` exact, `final_session=DH_LIGHT/LIGHT` (jamais `AEROBIC_BASE`), `COMMITTED_FAMILY_PRESERVED` tracé ; athlète B → `HTTP 200`, `engine_version=..._005b`, `event_context` absent, `final_session` jamais `RACE_ACTIVITY`, aucune trace `COMMITTED_FAMILY_PRESERVED` ni règle `RACE_*` — la course annulée n'a strictement aucun effet race-protocol, exactement la preuve requise par NAL-007A. Lecture "dernière décision valide" (chemin NAL-003 réel) confirmée identique à la décision du `daily-run` déjà exécuté, aucune seconde décision créée par la lecture. Overlay Planning (chemin NAL-007 réel) : course `confirmed` d'A visible dans son horizon, course `cancelled` de B absente. Matrice d'isolation croisée : A ne peut lire ni la séance planifiée ni la course de B (et réciproquement), écriture forgée cross-user refusée par RLS.
6. **Régression Louis** : 8 comptages (`athletes`, `planned_sessions`, `decisions`, `training_blocks`, `race_calendar`, `athlete_coaching_profiles`, `daily_checkins`, `health_flags`) capturés avant et après le canary — identiques bit à bit. Aucun `daily-run` exécuté pour Louis dans ce jalon (choix délibéré).
7. **Nettoyage** : complet et vérifié — zéro résidu sur toutes les tables pour chaque id trackée, les deux utilisateurs Auth scratch confirmés absents.
8. **Post-canary** : migration parity reconfirmée 29/29.
9. **V0.3_005 — Dogfood Round 1 Corrections dans son ensemble est COMPLETE (2026-09-08)** : les huit corrections (NAL-001 → NAL-007A) sont toutes closes et prouvées empiriquement en production.

**Alternatives considérées** : aucune nouvelle alternative architecturale pour les sept tickets — chacun corrige exactement le défaut observé sans redesign. Pour NAL-007A spécifiquement : ajouter `status` à `UpcomingRace`/M1 (rejeté — le traitement souhaité est uniforme à travers les trois phases pour `cancelled`/`skipped`, un simple filtre au niveau adaptateur suffit et évite de toucher le moteur frozen) plutôt qu'un filtrage SQL uniquement sur `planned`/`registered`/`confirmed` côté moteur (rejeté explicitement — aurait aussi exclu `completed`, désactivant silencieusement la récupération post-course puisqu'aucun writer ne transitionne jamais vers ce statut).

**Impact** : `head-coach-engine/src/{engine/buildDailyPlan.ts, rules/committedActivityFamily.ts (nouveau), rules/raceProtocol.ts, supabase/repositories/raceCalendarRepo.ts, types/rawContext.ts, supabase/{buildRawContext.ts, mapping/plannedSessionIntervention.ts, repositories/plannedSessionsRepo.ts}}`, `supabase/migrations/20260904100000_v0_3_005a_planned_sessions_is_committed.sql` (nouveau), `web/src/**` (Planning, DailyPlan, check-in, auth, completed-session — détail dans les commits individuels), `docs/00_PROJECT_STATUS.md`, `docs/04_DAILY_DECISION_ENGINE.md` (§Statut de la course, §Activité engagée), `docs/05_DATA_MODEL.md` (§planned_sessions, §decisions, §race_calendar), `docs/06_ARCHITECTURE.md` (§V0.3_005), `docs/10_TEST_PLAN.md` (§V0.3_005), `docs/11_DECISION_LOG.md` (cette entrée). Une seule mutation de donnée réelle intentionnelle : aucune sur Louis (contrairement à V0.3_004D, ce jalon n'a peuplé aucune nouvelle donnée réelle — seule la migration additive s'applique à ses rows existantes, toutes restant `is_committed=FALSE`).
...
NAL-006 = CLOSED / PROD
NAL-007 = CLOSED / PROD
NAL-007A = CLOSED / PROD
V0.3_005D = CLOSED / PRODUCTION ROLLOUT COMPLETE
V0.3_005 (ensemble) = COMPLETE
Prochain jalon = aucun sélectionné/architecturé à ce stade ; Louis Dogfood Round 2 est prêt à démarrer. `ActiveExperiment` reste le candidat suivant après tout retour de dogfood, différé mais non annulé — décision de priorisation Louis restant à prendre.
