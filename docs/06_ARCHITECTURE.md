...
### V0.3_007B — Performed Session Correctness Foundation (CLOSED / PRODUCTION ROLLOUT COMPLETE)

Active `completed_sessions.intervention` comme fait riche et **authoritative** pour une session réellement effectuée (voir `docs/05_DATA_MODEL.md` §completed_sessions pour le contrat complet champ par champ). Trois volets, strictement scopés (pas de `technical_outcome`/`change_reason`, pas de carte « Réalisé » en History, pas de changement au détecteur longitudinal `recommendationVsActualExecution`) :

1. **Vocabulaire riche côté écriture** — `web/src/features/completedSession/performedInterventionTypes.ts` reprend le vocabulaire `TrainingIntervention` de Planning et y ajoute `RACE_ACTIVITY` (jamais plannable, mais une réalité performée légitime). `CompletedSessionCard.tsx` expose un sélecteur kind+load riche (`performedKindGroups.ts`, réutilise `PLANNING_KIND_GROUPS` + groupe « Course ») pour `done`/`partial`/`replaced` ; `skipped` garde son ancien sélecteur coarse inchangé (`skipped_session_type`). `session_type` n'est plus jamais une valeur choisie indépendamment par l'athlète pour `done`/`partial`/`replaced` — toujours `mapTrainingInterventionToSessionType(intervention)`.
2. **Application côté serveur** — `supabase/functions/completed-session/validation.ts` porte désormais sa propre copie (délibérément dupliquée, même discipline que le mapping engine/web déjà dupliqué deux fois) du vocabulaire riche et de `mapTrainingInterventionToSessionType`. Rejette : `intervention = NULL` pour `done`/`partial`/`replaced` (`invalid_body_for_status`), `intervention` non-`NULL` pour `skipped` (`invalid_body_for_status`), forme/`kind`/`load_profile` invalide (`invalid_intervention`), et toute incohérence `session_type` ≠ dérivé (`session_type_mismatch`). Aucune évolution de la RPC `persist_completed_session` (M5_001A, frozen) n'a été nécessaire — elle acceptait déjà `intervention` sans l'inspecter ; seule la couche de validation Edge Function durcit le contrat. Preuve par un harness HTTP réel (`head-coach-engine/tests/edge/completedSession/orchestrate.ts`, 76/76 contre Edge Function + RPC + trigger DB réels en local).
3. **Liaison décision/session ambiguë même-jour** — nouvelle lecture RLS-scoped `loadValidDecisionsForDate` (`web/src/features/history/historyRepo.ts`) remplace entièrement l'ancien mécanisme `onLiveContextChange`/`LiveDailyPlanContext` (supprimé de `DailyPlanPanel.tsx`/`TodayPage.tsx`) — jugé sémantiquement dangereux en V0.3_007A2 (pouvait pointer vers une décision régénérée après coup, jamais celle réellement suivie). `CompletedSessionCard` résout désormais lui-même, de façon indépendante : 0 décision valide du jour → lien `NULL`, 1 → lien + préremplissage automatique (uniquement pour une nouvelle row), 2+ → sélecteur explicite (« Quel plan as-tu suivi ? »), sans présélection pour une nouvelle row, présélectionné sur la valeur persistée pour une row existante (corrigeable, mais la correction n'altère jamais rétroactivement la performance déjà enregistrée).

`recentLoad.ts` : `skipped` exclu explicitement (garde ceinture-et-bretelles, en plus de l'exclusion en amont par `mapCompletedSessionRow`) ; `done`/`partial`/`replaced` comptent tous via le `load_profile` **réellement effectué**, sans pondération fractionnaire. `ENGINE_VERSION` : `head-coach-engine@0.2.0-m1-v0.3_007b`.

Dette explicitement reportée (non résolue ici, à traiter séparément) : la collision de correspondance coarse du détecteur longitudinal `recommendationVsActualExecution` (deux `TrainingIntervention` riches distincts peuvent coarsen vers le même `DbSessionType`, ce qui limite la précision de « supporting/neutral/contradicting » — voir V0.3_007A2) ; l'absence de snapshot des inputs de génération de décision (limitation pré-existante, acceptée telle quelle).

### V0.3_007C — Athlete Debrief Fields (CLOSED / PRODUCTION ROLLOUT COMPLETE, 2026-09-10)

Ajoute la plus petite couche de debrief athlète utile, délibérément **inerte** pour le moteur et le longitudinal (voir `docs/05_DATA_MODEL.md` §completed_sessions pour le contrat champ par champ complet). Cible UX : 20–40 secondes, souvent une seule question rapide pour une séance normale.

1. **`technical_outcome`** — « as-tu exécuté la tâche technique SPÉCIFIQUE prescrite par la décision LIÉE », jamais une note générale de technique. La tâche évaluée vient exclusivement du `dh_or_technical.execution_task` de la décision **actuellement liée** (`completed_sessions.decision_id`) — jamais de la Planning courante, jamais d'une autre décision du même jour, jamais inféré de `focus`/texte libre personnel (même invariant que V0.3_006C1). Visible uniquement quand TOUTES les conditions tiennent : `done`/`partial`, une décision liée, cette décision porte un `execution_task` réel, et l'activité performée est elle-même DH-family (`web/src/features/completedSession/dhFamilyKind.ts`, miroir de `plannedDurationPolicy.ts`/`isDhFamilyKind` — une tâche DH ne peut pas être évaluée par une séance de force). **La condition DH-family est appliquée aussi bien côté serveur que côté UI** (revue finale) : `supabase/functions/completed-session/validation.ts` porte sa propre petite copie de la même liste de 4 kinds (une troisième duplication délibérée, pas un second moteur — même discipline que le reste de ce fichier), jamais une confiance aveugle dans le masquage UI seul. **Relation-dependent** : `CompletedSessionCard.tsx` le remet systématiquement à vide sur tout changement de lien décision (`handleDecisionChange`) OU de statut (`handleStatusChange`) — jamais une réponse périmée réexposée comme si elle répondait à la nouvelle tâche.
2. **`change_reason`** (+ `change_reason_note`) — « pourquoi la séance n'a pas été un DONE ordinaire », taxonomie structurée à choix unique (9 catégories), jamais un essai libre. Visible pour tout statut non-`done`. `'coach_criterion'` exposé uniquement quand une décision est liée (filtré côté UI **et** rejeté côté serveur). Contrairement à `technical_outcome`, décrit l'événement réel (fatigue, douleur, météo, mécanique…) — **préservé** à travers un changement de lien décision, sauf `'coach_criterion'` spécifiquement quand le lien devient `NULL` (redevient sémantiquement impossible, seul ce cas précis est effacé). `change_reason_note` est contextuelle au motif **actuellement sélectionné**, pas au lien : `handleChangeReasonChange` (revue finale, Issue A) vide systématiquement la note dès que l'athlète change de motif vers une valeur différente — jamais une précision qui survit silencieusement rattachée au mauvais motif ; un changement de lien décision qui préserve le motif préserve aussi la note. **`'other'` exige une note** (revue finale, décision V1 : « autre » seul ne porte presque aucune information utile) — appliqué côté web (label devient « obligatoire pour « Autre » », bloque `canSave` via l'erreur de validation elle-même) **et** côté serveur (`change_reason_note_required`) ; aucune autre catégorie n'exige de note. **`change_reason = 'pain'` et `daily_checkins`/`new_pain` restent délibérément indépendants** (revue finale, Issue B) : « la douleur a affecté l'exécution » (fait sur la séance) vs « une NOUVELLE douleur est apparue » (fait informationnel athlete-reported, inchangé depuis M5_003) répondent à des questions différentes — aucun couplage, aucune synchronisation cachée, aucune nouvelle politique Safety, aucune écriture `health_flags` (cet endpoint n'écrit jamais que `completed_sessions`, contrairement à `persist_daily_run`).
3. **Application côté serveur, compatibilité ancien client** — `supabase/functions/completed-session/validation.ts` traite ces trois clés comme **optionnelles** dans le corps de requête (seul cas dans tout ce contrat, autrement strict) : absentes, elles se normalisent en `NULL` avant l'appel RPC, pour qu'un onglet web resté ouvert sur un bundle antérieur à V0.3_007C continue de fonctionner sans erreur. Une valeur **présente** reste validée strictement : `technical_outcome` non-`NULL` exige `done`/`partial` + `decision_id` non-`NULL` + `intervention` DH-family (`technical_outcome_not_applicable`, 400, statique) puis, côté `index.ts` (lecture DB du `daily_plan` déjà effectuée pour le préflight de liaison existant), que ce `daily_plan` porte réellement un `execution_task` (`technical_outcome_no_task`, 422, seule moitié de la règle qui a besoin d'une lecture DB — jamais dupliqué dans le validateur pur). `change_reason` non-`NULL` pour `done` → rejeté (`invalid_body_for_status`) ; `'coach_criterion'` sans `decision_id` → rejeté (`coach_criterion_requires_decision`) ; `change_reason_note` non-vide sans `change_reason` → rejeté (`invalid_change_reason_note`) ; `change_reason = 'other'` sans note → rejeté (`change_reason_note_required`). **Le serveur n'exige jamais leur présence** pour `partial`/`skipped`/`replaced` — cette exigence reste strictement côté UI web (`CompletedSessionCard`'s propre gate `canSave`, même pattern déjà établi par `decisionLinkResolved`), jamais un blocage serveur qui casserait un vieux client.
4. **RPC** — `persist_completed_session` étend son contrat FULL REPLACEMENT strict existant des trois nouvelles clés, avec une **exception délibérée et unique** : contrairement aux 13 clés existantes (toutes requises-présentes-mais-nullables, ex. `decision_id`), ces trois clés sont **optionnelles au niveau RPC lui-même** — absentes, elles se résolvent en `NULL` en interne. Découverte au gate rollout (revue finale) : requises-présentes comme le reste du contrat, elles auraient rejeté **tous** les écrits de l'Edge Function V0.3_007B actuellement déployée en production (qui ne les envoie jamais) pendant toute la fenêtre entre le déploiement de cette migration et celui de la nouvelle Edge Function — prouvé empiriquement par appel RPC direct avant correction (`P0001`, « technical_outcome key is required »), puis re-prouvé accepté après correction. Aucune autre clé concernée (rejet des clés inconnues, présence stricte des 13 clés existantes, contrôles ownership/date/decision inchangés), aucune validation de cohérence métier ajoutée dans la RPC elle-même (reste la responsabilité de l'Edge Function, comme pour tout le reste du contrat M5_003/V0.3_007B). **Ordre de déploiement sûr et prouvé** : (1) migration/RPC en premier — l'ancienne Edge Function V0.3_007B doit continuer à écrire sans erreur contre elle ; (2) nouvelle Edge Function ; (3) nouveau bundle web — un onglet navigateur resté ouvert sur l'ancien bundle doit continuer à fonctionner contre la nouvelle Edge Function (déjà garanti par le point 3 ci-dessus).
5. **Sémantique de durée DH clarifiée (petit fix UX, V0.3_007A)** — l'aide contextuelle du champ « Durée (minutes) » précise désormais, pour une activité performée DH-family, qu'il s'agit du temps total de session (remontées/pauses/attente compris), jamais le seul temps de pédalage — même convention que la durée prévue de Planning (V0.3_006C2). Purement UI, aucun changement de schéma/unité.
6. **Copie durée Pumptrack corrigée (canary UI, revue finale)** — le canary UI athlete-facing en production a révélé que PUMPTRACK, bien que DH-family (`isDhFamilyKind`), n'est jamais lift-served : le texte d'aide de durée réclamait à tort des remontées/pauses. Corrigé des deux côtés par un prédicat strictement plus étroit, DH_PERFORMANCE/DH_TECHNICAL/DH_LIGHT uniquement — `isUpliftServedDhDurationKind` (`completedSession/dhFamilyKind.ts`, durée performée) et `isUpliftServedDhPlannableKind` (`planning/plannedDurationPolicy.ts`, durée prévue, dupliqué localement plutôt qu'importé cross-feature). `isDhFamilyKind`/`isDhFamilyPlannableKind` eux-mêmes restent inchangés et demeurent l'unique autorité pour `technical_outcome`/validation serveur/éligibilité du champ durée — PUMPTRACK conserve son champ durée prévue/performée, seule la copie change. RACE_ACTIVITY garde la copie générique des deux côtés (jamais garanti lift-served). Copie uniquement, aucune migration, aucun changement Edge/RPC/moteur.

Preuve empirique (local) : harness HTTP réel (`head-coach-engine/tests/edge/completedSession/orchestrate.ts`, 97/97 contre Edge Function + RPC + trigger DB réels en local, incluant le round-trip complet des trois champs, le rejet `technical_outcome_no_task` contre une vraie décision sans tâche, le rejet `technical_outcome_not_applicable` pour une activité performée non-DH, le rejet/acceptation `change_reason_note_required` pour `'other'`, la compatibilité ancien-client explicite, et 8 scénarios de sécurité de rollout appelant `persist_completed_session` directement en contournant l'Edge Function). Suite `web` finale 850/850 (33 skipped), suite `head-coach-engine` 557/557 (13 skipped), les deux builds clean.

Preuve empirique (production, `uvolpldwwyvadlamulvr`/`nalynt`) : preuve critique intermédiaire (Edge Function V0.3_007B encore déployée → RPC migrée, payload exact à 12 clés HTTP → `200`, colonnes `NULL`) PASS avant tout autre déploiement ; smoke post-Edge (ancien/nouveau payload) PASS ; canary backend (legacy, round-trip `technical_outcome` contre une vraie décision `daily-run`, round-trip `change_reason`, rejets `OTHER`-sans-note/non-DH/clé-inconnue) PASS ; canary UI athlete-facing PARTIAL/REPLACED PASS, ayant révélé et fait corriger le défaut de copie Pumptrack ci-dessus (point 6). Nettoyage scratch complet (cascade RLS, zéro ligne sur 5 tables) ; le rejet `technical_outcome_no_task` et l'isolation cross-athlete n'ont volontairement pas été reproduits en production (voir `docs/11_DECISION_LOG.md` pour le raisonnement exact) — déjà prouvés respectivement par le harness local 97/97 et le canary V0.3_007B.

Reste explicitement hors périmètre (V0.3_007C) : aucune consommation moteur (`RawContext`/`recentLoad`/Safety/arbitrage inchangés, `ENGINE_VERSION` toujours `head-coach-engine@0.2.0-m1-v0.3_007b`) ; aucune consommation longitudinale (`recommendationVsActualExecution`/pattern evidence inchangés) ; pas de carte « Réalisé » en History ; la dette de correspondance coarse du détecteur longitudinal (V0.3_007B, ci-dessus) reste non résolue. **Prochaine tranche retenue : V0.3_007D — History : Prescribed vs Performed** (rendre visible côté athlète la prescription persistée à côté de la vérité performée — non implémentée, ne consomme toujours pas la comparaison côté moteur).
...
## V0.3_002 — Domain Coaching Enrichment (COMPLETE — 2026-08-30 ; V0.3_002A/B/C/D/E/F tous CLOSED — 2026-08-28/28/29/30/30/30)

**Statut : V0.3_002 COMPLETE (2026-08-30)** — V0.3_002A CLOSED (architecture/contrats verrouillés) — V0.3_002B CLOSED LOCALLY (2026-08-28) — V0.3_002C CLOSED LOCALLY (2026-08-29) — V0.3_002D CLOSED LOCALLY (2026-08-30) — V0.3_002E CLOSED LOCALLY (2026-08-30, test-only) — **V0.3_002F CLOSED / REMOTE ROLLOUT COMPLETE (2026-08-30)**. Cette section documente une décision d'architecture approuvée avant implémentation, sur le modèle de la section V0.3_001 ci-dessus ; les portées verrouillées V0.3_002B/C/D sont désormais implémentées et testées localement, leur composition cross-domaine est régression-testée par V0.3_002E, leur compatibilité web est verrouillée durablement, et le runtime enrichi est déployé en production (voir sous-sections de clôture respectives, et `docs/11_DECISION_LOG.md`).
...
#### V0.3_002C — CLOSED LOCALLY (2026-08-29)

**Résultat d'implémentation** (conforme à la portée verrouillée ci-dessus) : `computeMentalDomain` est exécuté après stabilisation complète de l'arbitrage Training et de `final_session`, `focus`/`action_hint` orthogonaux — `focus` uniquement depuis `event_context.phase === "PRE_EVENT"` (toute priorité de course), `action_hint` uniquement depuis `dimensions.mental` AMBER ou RED. AMBER : précédence `stress_high` puis `motivation_low`, exactement un signal consommé (`MENTAL_AMBER_STRESS`/`MENTAL_AMBER_MOTIVATION`). RED : zéro `consume()` par Mental — lecture non consommante (`consumedByRule() === "MENTAL_RED"`) avant d'émettre le texte de support ; si non vérifiable, aucun texte fabriqué. `mental.active = focus !== undefined || action_hint !== undefined`. Aucune section POST_EVENT/RACE_DAY_GENERIC-spécifique — ces phases n'inhibent jamais globalement Mental, elles empêchent seulement `focus` ; l'AMBER/RED authentique reste actif. `ENGINE_VERSION` → `head-coach-engine@0.2.0-m1-v0.3_002c`, désormais testé directement. Commits `06e878887e3e09f8737bfdc81af999977b92e9ac` / `3b1753b4eabb73cdf9050fbf1a994bd1f1a72e8a`. Tests `npm test` 310/310, `npm run test:edge` 9/9, build PASS. Aucune migration, aucun changement web, aucun déploiement remote.

**Comportement Training existant enregistré, non modifié** : la précédence `stress_high` puis `motivation_low` de la règle `MENTAL_RED` (`training.ts`) sélectionne toujours `stress_high` dès qu'il est présent dans `raw_signals`, y compris dans le cas mixte stress AMBER + motivation RED où `motivation_low` est la cause réelle de sévérité RED. Mental reflète fidèlement la propriété effectivement créée par Training plutôt que de réimplémenter sa propre sélection — ce comportement Training préexistant est observé et couvert par régression, jamais reconçu par 002C.

**Isolation `triggered_rules` (late-push)** : la `TriggeredRule` AMBER de Mental est calculée avant la construction du littéral `DailyPlan` (le `consume()` doit précéder), mais n'est poussée dans le tableau partagé `triggeredRules` qu'après la fermeture du littéral. Chronologie exacte : `override_reason` est déjà finalisé (calculé comme `const` séparée) avant même que le littéral ne commence ; `training.objective` et `reasoning` sont dérivés pendant la construction du littéral ; la `TriggeredRule` Mental n'est ajoutée qu'après. Les trois champs dérivés de Training sont donc tous figés avant cet ajout. `plan.triggered_rules` reste la même référence de tableau que la variable locale `triggeredRules` et reflète bien l'ajout après coup, sans jamais influencer rétroactivement `training`/`reasoning`/`override_reason`. Prouvé par comparaison appariée réelle GREEN/AMBER (égalité stricte de `decision`/`final_session`/`training`/`reasoning`/`override_reason`, différence isolée à `mental`/`triggered_rules`). Décision bornée à cette occasion d'implémentation, pas une politique d'architecture générale.

### Portée verrouillée V0.3_002D — Nutrition

Champs utilisés : `nutrition.{active, focus, hydration_target_l, notes}` uniquement. DO NOW, seulement quand le contexte rend le conseil matériel : rappel race-week (`active_mode === "RACE_WEEK"`, distinct de `PRE_EVENT` — Nutrition n'utilise jamais `PRE_EVENT`), guidance hydratation jour DH, guidance récupération pour un jour avec séance de force **planifiée** (le moteur ne connaît pas l'exécution/complétion de la séance à ce stade — la guidance porte sur une séance planifiée du jour, formulée comme conseil générique à appliquer après la séance, jamais comme si le moteur savait qu'elle a déjà eu lieu), timing repas jour de course (`event_context.in_progress`, C5.6 — pas une fenêtre "pré-course" avant le jour même) **relatif générique uniquement** (`UpcomingRace.event_start` est une date, jamais un timestamp — aucune heure de départ exacte n'existe dans le modèle de données ; aucun conseil à heure fixe possible).
...
#### V0.3_002D — CLOSED LOCALLY (2026-08-30)

**Résultat d'implémentation** (conforme à la portée verrouillée ci-dessus) : `computeNutritionDomain` lit le contexte déjà stabilisé (aucune participation à l'arbitrage Training). `focus` (rappel race-week) et la branche primaire (`notes`/`hydration_target_l`) sont dérivés indépendamment. Sources exactes : race-week = `active_mode === "RACE_WEEK"` ; jour de course = `event_context.in_progress` (jamais `phase === "RACE_DAY_GENERIC"`, pour rester valide si une phase granulaire est un jour curée) ; jour DH = `final_session.kind` (séance déjà arbitrée, l'hydratation doit suivre ce qui se passe réellement) ; séance de force = `final_session.kind` **(corrigé DOG-001, 2026-09-15 — lisait auparavant `ctx.planned_session.kind` brute ; voir `docs/11_DECISION_LOG.md` DOG-001)**. Précédence déterministe explicite de la branche primaire : RACE DAY > JOUR DH > SÉANCE DE FORCE > aucune — une branche de priorité inférieure ne peut jamais faire fuiter `hydration_target_l` (structurellement impossible : ce champ n'est assigné que dans la branche force). `hydration_target_l` ne prend jamais que la valeur `2` (C5.3) ; la plage C5.4, le délai C5.2 et le délai C5.6 restent texte uniquement, jamais convertis en champ numérique structuré. Aucune interaction `SignalTrace`, aucune `TriggeredRule` — confirmé par la signature de `computeNutritionDomain` (retourne `NutritionSection` seul). Aucune extension `athleteCoachingProfile.ts` — les quatre constantes numériques génériques (`baselineHydrationTargetL`, `dhHydrationRangeL`, `strengthPostWindowMinutes`, `raceBreakfastLeadHours`) vivent dans `src/config/nutritionPolicy.ts` (NEW) et en sont la source de vérité runtime réelle : `baselineHydrationTargetL` pilote le champ structuré `hydration_target_l`, les trois autres pilotent les chaînes de texte déterministes (durcissement, formatteur décimal français minimal, aucune copie numérique dupliquée). `ENGINE_VERSION` → `head-coach-engine@0.2.0-m1-v0.3_002d` ; son assertion exacte n'a désormais qu'un seul propriétaire (`tests/engineVersion.test.ts`) ; les copies qui existaient dans T12 et T13 ont été retirées — durcissement applicable à toute future décision de provenance, pas une politique de versioning universelle. Commits `eac175869a8f30ddd49352c97c942ea574f5106b` / `792608c633922aeea5464cc385382a544d1b74c4`. Tests `npm test` 347/347, `npm run test:edge` 9/9, build PASS. Aucune migration, aucun changement web, aucun déploiement remote.
...
Le futur jalon d'intégration/régressions devra prouver au minimum : tous les tests frozen M1-M4 pré-existants toujours verts ; `decision`/`session_type`/`triggered_rules` de Training inchangés pour les fixtures pré-existantes ; Safety A1-A5 inchangée ; les plans Safety gardent les nouvelles sections inactives ; sortie de domaine identique pour entrée identique ; aucune double-consommation de signal, prouvée par comportement (fixture `mental RED` pré-existante : décision/action/session Training strictement inchangées, règle `MENTAL_RED` toujours présente dans `triggered_rules`, `SignalTrace` rapporte `MENTAL_RED` comme propriétaire de décision, sortie Mental peuplée en lecture de support, aucun second `consume()` ne réussit) — `Training → Mental` reste un contrat normatif d'implémentation, jamais vérifié par une assertion sur la structure du code source ; aucune chaîne de downgrade générique ; aucune recommandation sans fait de support observable/configuré ; les domaines non pertinents restent inactifs ; le renderer web existant accepte le `DailyPlan` enrichi réel — cette preuve n'a pas été ré-exercée dans le périmètre test-only `head-coach-engine`-only de 002E (`web/**` explicitement hors scope approuvé) et reste un gate obligatoire de V0.3_002F avant rollout/clôture finale ; aucune influence M5/Couche D.

#### V0.3_002E — CLOSED LOCALLY (2026-08-30)

**Résultat d'implémentation** (test-only, conforme au contrat verrouillé ci-dessus) : `head-coach-engine/tests/t14_crossDomainIntegration.test.ts` (NEW, seul fichier créé — aucun fichier de production, aucun fichier de test historique modifié) prouve, via de vrais `RawContext` passés à `buildDailyPlan` (jamais d'injection directe de dimension dérivée, jamais d'instrumentation de `SignalTrace`) : indépendance structurelle `PRE_EVENT` (`event_context.phase`, source exclusive Mental)/`RACE_WEEK` (`active_mode`, source exclusive Nutrition) sur les 3 quadrants (l'un seul, l'autre seul, les deux ensemble — les deux `focus` coexistent sans interférence) ; coexistence réelle des trois domaines (séance DH finale + Mental AMBER → Technique/Mental/Nutrition tous actifs, exactement une `TriggeredRule` `MENTAL_AMBER_STRESS`, zéro `TECHNIQUE_*`/`NUTRITION_*`) ; extension de la preuve de late-push Mental (§V0.3_002C ci-dessus) à une paire appariée GREEN/AMBER avec Technique et Nutrition simultanément actifs des deux côtés — `decision`/`final_session`/`training`/`reasoning`/`override_reason`/`dh_or_technical`/`nutrition`/`recovery` strictement égaux entre les deux plans, seuls `mental`/`triggered_rules` diffèrent par exactement une règle ; propriété `MENTAL_RED` confirmée sur une séance DH réelle (le kind DH survit à la seule dégradation de charge appliquée par `training.ts`, Technique/Nutrition suivent le `final_session` stabilisé, aucune seconde règle Mental observable — `triggered_rules` ne contient qu'un seul `MENTAL_RED`, jamais de `MENTAL_AMBER_*` associé ; T14 n'instrumente jamais `SignalTrace`, il s'agit d'une preuve de composition cross-domaine observable via `DailyPlan`/`triggered_rules`, pas d'une preuve directe d'appel `consume()` — la preuve directe qu'aucun second `consume()` n'a lieu reste couverte par T12, qui instrumente réellement `SignalTrace`) ; propagation `final_session` prouvée sur un même fixture réel (`GRIP_WORK` planifiée + `arms_grip`/`legs` RED → Training pivote vers `RECOVERY_ACTIVE` ; Technique et Nutrition suivent toutes deux correctement et deviennent inactives — **corrigé DOG-001, 2026-09-15** : la branche force de Nutrition lisait auparavant le `planned_session` brut au lieu du `final_session`, documentée à tort comme une asymétrie acceptée ; c'était en réalité un défaut réel, corrigé pour lire exclusivement `final_session.kind` comme la branche DH juste au-dessus — voir `docs/11_DECISION_LOG.md` DOG-001) ; composition jour de course réel (`computeEventContext`, jamais un `EventContext` construit à la main) avec Mental AMBER coexistant et `mental.focus` structurellement absent (la phase ne peut jamais être `PRE_EVENT` quand `in_progress` est vrai — garantie de construction, pas seulement observée) ; Safety dominant une pression maximale (`suspected_concussion` + `RACE_WEEK` + `PRE_EVENT` réel + séance DH planifiée) avec `triggered_rules` strictement `[A1]` et les trois sections enrichies exactement `{active:false}` ; absence de tout `rule_id` `TECHNIQUE_*`/`NUTRITION_*` à travers l'ensemble des plans combinés ; régression Recovery appariée (mêmes `final_session`/`modeConstraints`/`eventContext.phase` pertinents des deux côtés, Mental/Technique/Nutrition s'activant pourtant indépendamment) ; déterminisme d'un `DailyPlan` riche multi-domaine. `ENGINE_VERSION` **inchangé** — aucune sémantique persistée du `DailyPlan` modifiée par ce jalon ; `tests/engineVersion.test.ts` reste l'unique propriétaire de l'assertion exacte, non modifié.

**Vérification web durant 002E : NON EXÉCUTÉE / hors périmètre approuvé** — `web/**` était explicitement exclu du hard file-scope lock de 002E ; aucun changement de production/web n'a eu lieu (donc aucun nouveau risque de compatibilité introduit), mais la preuve bout-en-bout que le renderer web existant accepte réellement le `DailyPlan` enrichi B/C/D **n'a pas été apportée par 002E** — ce gate a ensuite été fermé durablement par V0.3_002F (`web/src/features/dailyPlan/DailyPlanView.enriched.test.tsx`, voir §V0.3_002F — CLOSED ci-dessous), avant rollout remote et avant que V0.3_002 dans son ensemble ne soit marqué COMPLETE.

Safety/Training/Technique/Mental/Nutrition/Recovery/`SignalTrace`/`eventContext.ts`/`raceProtocol.ts` strictement inchangés — aucun fichier de production modifié. Commit `fa8d1c740c6a4971384d46172ca70a5bb9f85e20`. Tests `npm test` 359/359, `npm run test:edge` 9/9, build PASS. Aucune migration, aucun changement web, aucun déploiement remote.

### Frontière rollout V0.3_002F

Le déploiement remote est intervenu uniquement après revue/clôture locale de 002B, 002C, 002D et 002E. 002F a couvert séparément : déploiement du runtime déjà revu, vérification remote authentifiée contrôlée, zéro mutation de donnée applicative non liée et clôture documentaire canonique. Aucun déploiement n'est intervenu pendant 002A-E sans approbation explicite.

#### V0.3_002F — CLOSED / REMOTE ROLLOUT COMPLETE (2026-08-30)

**Gate web durable (préalable au rollout)** : `web/src/features/dailyPlan/DailyPlanView.enriched.test.tsx` (commit `a59a1bce07a2a85cf5bfc095284c5096800d7803`, déjà présent avant toute action remote) ferme le gate reporté par 002E — un vrai `RawContext` passé au vrai `buildDailyPlan` (import relatif direct de `head-coach-engine/src/**`, empiriquement vérifié résoluble par Vite/Vitest sans aucune dépendance/workspace/alias ajouté ; `tsc -b` du build web normal l'accepte aussi) produit un plan réellement Technique+Mental+Nutrition actif, validé par le vrai `isValidDailyPlan` (jamais relâché) et rendu par le vrai `DailyPlanView` — aucun objet `DailyPlan` copié à la main, le test échoue naturellement si un futur changement d'engine devient web-incompatible. `web` **294/294** (28 fichiers), build PASS. Aucune modification de `dailyPlanValidation.ts`/`DailyPlanView.tsx`/`dailyPlanTypes.ts`.

**Rollout remote** : seule `daily-run` a été redéployée (`npm run deploy:daily-run` → `supabase functions deploy daily-run --use-api --project-ref uvolpldwwyvadlamulvr --workdir ..`, commande non modifiée) — `completed-session`/`get-insights`/`refresh-longitudinal`/`submit-review` non touchées (confirmé par `functions list` post-déploiement : seule `daily-run` a un `updated_at`/`version` changé, 1→2). Le graphe transitif uploadé confirme `head-coach-engine/dist/domains/{technique,mental,nutrition}.js` et leurs configs (`nutritionPolicy`/`techniquePolicy`/`athleteCoachingProfile`). Post-déploiement : `daily-run` **ACTIVE**, version 2, `verify_jwt: true` (inchangé).

**Preuve de provenance et de composition remote** : canary fonctionnel sur un unique athlète scratch temporaire (jamais l'athlète réel de Louis), créé/utilisé/nettoyé via `head-coach-engine/tests/supabase/testDb.ts` (le même mécanisme sanctionné déjà prouvé en M3_006, pointé sur `uvolpldwwyvadlamulvr` via `SUPABASE_URL`). Entrées réelles : bloc d'entraînement, séance planifiée `DH_TECHNICAL`/`MODERATE`, check-in avec `work_stress=6` (Mental AMBER). Un seul `POST /functions/v1/daily-run` authentifié → `HTTP 200`, `dailyPlan.engine_version = "head-coach-engine@0.2.0-m1-v0.3_002d"`, `decision = "KEEP"` (aucune fuite Safety inattendue), `dh_or_technical.active`/`mental.active`/`nutrition.active` tous `true`. Relecture de la décision persistée (lecture seule, athlète scratch uniquement) : `engine_version` dénormalisé et `daily_plan->>'engine_version'` identiques, `daily_plan.{dh_or_technical,mental,nutrition}.active` tous `true` dans le JSONB réellement persisté — la composition remote et la survie des sections enrichies à la persistance sont donc prouvées empiriquement, au-delà de la garantie structurelle déjà établie par `dailyPlanToDecisionRow.ts` (`daily_plan: dailyPlan` verbatim).

**Nettoyage** : complet et vérifié en lecture seule — 0 ligne restante sur `athletes`/`decisions`/`daily_checkins`/`training_blocks`/`planned_sessions`/`health_flags`/`completed_sessions` pour l'identité scratch, utilisateur d'authentification confirmé absent. **Écritures sur l'athlète réel : zéro** sur l'ensemble du rollout.

**Parité de migration** : reconfirmée en lecture seule avant et après déploiement — 26 locales/26 remote/0 en attente, inchangé. Aucune migration, aucun changement de schéma introduit par V0.3_002.

**Web/Vercel** : aucune source `web/**` de production modifiée par V0.3_002 (seul le nouveau fichier de test l'a été) ; aucun déploiement Vercel effectué ni requis.

**`ENGINE_VERSION`** : reste `head-coach-engine@0.2.0-m1-v0.3_002d` — ni 002E ni 002F n'ont changé la sémantique persistée du `DailyPlan` ; un déploiement n'est pas en soi une décision de provenance.

**Rollback** : non requis, non exécuté. Candidat d'urgence audité et confirmé valide en lecture seule : `b9a39dfe39d1c07116c85f4b41cc74f0b8387b85` (parent immédiat pré-002B — wrapper d'authentification M3, `runDailyFor`, layout `head-coach-engine` source, `ENGINE_VERSION="head-coach-engine@0.2.0-m1"`, script `deploy:daily-run` tous présents à ce commit). Aucun rollback DB n'aurait été nécessaire (aucune migration V0.3_002).

**V0.3_002 — Domain Coaching Enrichment dans son ensemble est COMPLETE (2026-08-30)** : les 6 jalons de la décomposition verrouillée en V0.3_002A (002A→002F) sont tous clos, `daily-run` sert l'engine enrichi en production, aucune régression locale ou remote détectée.
...
Moteur strictement inchangé : `head-coach-engine` build PASS, **359/359**, edge **9/9**, `ENGINE_VERSION` inchangé. Aucune migration, aucun déploiement remote, aucune action Vercel. **À la clôture de V0.3_003B**, `/plan` restait différé à V0.3_003C — voir la sous-section suivante pour sa clôture réelle.

### V0.3_003C — clôture locale, preuve de test (2026-08-31)

Fichiers de production (3 nouveaux + 3 modifiés) : `web/src/pages/PlanPage.tsx`, `web/src/features/planning/PlanningDayCard.tsx`, `web/src/features/planning/planningKindGroups.ts` (nouveaux) ; `web/src/App.tsx` (route `/plan`), `web/src/components/AppNav.tsx` (entrée "Plan"), `web/src/lib/date.ts` (`addDays`, arithmétique calendaire sur les composantes Y/M/D, jamais `+ jours × 86400000`) (modifiés). Fichiers de test (3 nouveaux + 3 modifiés en miroir) : `PlanPage.test.tsx`, `PlanningDayCard.test.tsx`, `planningKindGroups.test.ts` ; `App.test.tsx`, `AppNav.test.tsx`, `date.test.ts`. Implémenté (`5c66385`, `feat: add V0.3_003C web planning workflow`, incluant le durcissement pré-commit du même commit).

**Contrat produit implémenté** : route authentifiée `/plan` (`RequireAuth`, même pattern que `/today`/`/history`/`/insights`), entrée `AppNav` "Plan" (ordre Aujourd'hui → Plan → Historique → Insights), horizon aujourd'hui→J+6 (exactement 7 jours, `addDays`/`todayLocal`, sémantique calendaire locale navigateur, jamais UTC). Sélecteur natif `<select>`/`<optgroup>` (4 groupes de présentation, `planningKindGroups.ts`) sur les 15 kinds plannables ; `RACE_ACTIVITY` jamais exposé. Les 11 kinds à charge variable exigent un choix explicite HEAVY/MODERATE/LIGHT (aucun défaut) ; les 4 kinds à charge fixe (`MOBILITY`/`RECOVERY_ACTIVE`/`REST`/`BIKE_MAINTENANCE`) n'affichent aucun sélecteur de charge. Libellés français réutilisés depuis `TRAINING_KIND_LABELS`/`LOAD_PROFILE_LABELS` (`dailyPlanLabels.ts`) — aucune duplication de mapping. "Non planifié" (aucune row) et "Repos" (row `{kind:"REST"}` explicite, sauvegardée par le chemin normal `savePlannedSession`, jamais par suppression) restent deux états visuellement distincts.

**Contrat d'état confirmé implémenté** : `PlanPage` est l'unique source de vérité en mémoire des rows persistées pour les 7 dates ; `PlanningDayCard` ne porte que son état de brouillon/édition (kind/load en cours de saisie, état de sauvegarde) — jamais une copie indépendante de "ce qui est persisté". Un brouillon non sauvegardé n'est jamais affiché comme persisté. Une mutation réussie ne met à jour l'affichage qu'après confirmation serveur (aucune mise à jour optimiste avant confirmation) ; une mutation échouée laisse l'éditeur ouvert, le brouillon intact, l'affichage persisté inchangé, et montre un message sûr — jamais un message PostgREST brut (seules les erreurs typées `PlanningSaveError`/`PlanningDeleteError`/`InvalidPlannedInterventionError` du dépôt sont affichées verbatim ; toute autre exception, ex. un échec réseau brut, est masquée par un message générique). **Durcissement course contre la montre asynchrone inter-jours** : une mutation qui se résout après que l'athlète a déjà ouvert un autre jour ne referme plus ce dernier — seul le jour ayant réellement produit la mutation se referme.

**Compatibilité row legacy `intervention=NULL`** : une row antérieure à M2_003 (coarse `session_type` seul, sans `intervention` riche) est affichée via le label français coarse existant réutilisé (`SESSION_TYPE_LABELS`, `completedSessionTypes.ts` — jamais l'enum brut `STRENGTH_A` etc. affiché à l'athlète). Ouvrir une telle row pour édition ne fabrique jamais un `kind`/`load_profile` riche inventé : le sélecteur démarre non sélectionné, une note explique la situation, et une sélection explicite est requise avant sauvegarde. La suppression reste disponible sans condition.

Résultats de test finaux : `web` par défaut **473 passed / 9 skipped** (482 total, 35 fichiers, aucune stack Supabase requise) ; `web` complet avec opt-in **482/482 PASS**. `head-coach-engine` build PASS, **359/359** ; edge **9/9** ; `ENGINE_VERSION` inchangé. Aucune migration, aucune action Supabase explicite. **Aucune action de déploiement Vercel explicite dans V0.3_003C — l'état effectif du déploiement Vercel après push Git n'a pas été vérifié dans ce jalon** : `/plan` est implémenté sur `origin/main`, son état réel en production reste à ré-auditer en V0.3_003E. **À la clôture de V0.3_003C**, l'intégration `/today` restait différée à V0.3_003D — voir la sous-section suivante pour sa clôture réelle.

### V0.3_003D — clôture locale, preuve de test (2026-08-31)

Fichiers de production (1 nouveau + 2 modifiés) : `web/src/features/planning/TodayPlanningSummary.tsx` (nouveau) ; `web/src/pages/TodayPage.tsx` (nouvelle section, modifié) ; `web/src/features/planning/PlanningDayCard.tsx` (correctif ciblé, modifié — voir ci-dessous). Fichiers de test/infrastructure de test (3 nouveaux + 4 modifiés) : `TodayPlanningSummary.test.tsx`, `head-coach-engine/tests/supabase/testDbSafety.test.ts`, `head-coach-engine/tests/supabase/t17_planningE2E.integration.test.ts` (nouveaux) ; `TodayPage.test.tsx`, `PlanningDayCard.test.tsx`, `head-coach-engine/tests/supabase/testDb.ts` (modifiés). Implémenté (`05b3123`, `feat: integrate V0.3_003D planning with today`, incluant le durcissement pré-commit du même commit). Aucun fichier `head-coach-engine/src/**` modifié.

**Contrat Today implémenté** : section lecture-seule "Prévu aujourd'hui" sur `/today`, placée après la date et avant le check-in, représentant **uniquement** la row `planned_sessions` courante de l'athlète — jamais le fallback, le protocole de course, ni `DailyPlan.final_session` (qui reste la section "Daily Plan"/Head Coach existante, plus bas sur la page, sémantiquement distincte). Réutilise `loadPlannedSessions(athleteId, today, today)` (003B, aucune nouvelle méthode de dépôt) ; chargement au montage uniquement, aucun contexte Planning, aucun cache persistant, aucun `planningRevision` — le démontage/remontage de route reste l'unique mécanisme de fraîcheur (`daily-run` ne mute jamais `planned_sessions`, confirmé par audit source, donc aucun refetch post-run n'a été ajouté). États : "Non planifié" (aucune row), "Repos" (REST explicite), libellé riche partagé (`formatIntervention`), label coarse legacy sûr (`SESSION_TYPE_LABELS[row.session_type] ?? "Séance planifiée (ancienne)"` — jamais l'enum brut, y compris pour une valeur runtime non mappée). Lien "Modifier dans Plan" vers `/plan` ; aucune mutation inline (`savePlannedSession`/`deletePlannedSession` jamais importés). Un échec de lecture Planning est non-critique : "Planning indisponible" localement, sans jamais bloquer check-in/daily-run/décision Head Coach.

**Correctif ciblé 003C inclus** : `PlanningDayCard.tsx` (fichier clos en V0.3_003C) retombait sur l'enum brut `row.session_type` pour une row legacy dont le coarse type n'était pas dans `SESSION_TYPE_LABELS` — corrigé pour utiliser le même label générique sûr que `TodayPlanningSummary.tsx` ("Séance planifiée (ancienne)"). Défaut de cohérence UI découvert en revue, pas une refonte de l'architecture Planning ; aucun changement de sémantique d'édition/sauvegarde/suppression.

**T17 — preuve cross-stack réelle locale (CLOSED / PASS)** : `head-coach-engine/tests/supabase/t17_planningE2E.integration.test.ts` exécute `runDailyFor(admin, athleteId, date)` directement (même pattern déjà établi par `buildRawContext.integration.test.ts`/`runDailyFor.integration.test.ts`, jamais via l'Edge Function HTTP `daily-run` — aucun service local requis) contre une vraie stack Supabase locale, athlètes scratch (`testDb.ts`), aucun mock. Six scénarios destructifs, tous PASS : (1) jamais planifié — `planned_session=null`, repli `INFERENCE_FALLBACK`, `final_session` = le split hebdomadaire déterministe exact ; (2) planifié puis supprimé (row insérée puis effacée localement avant `runDailyFor`) — converge vers exactement le même contrat que (1), preuve distincte que DELETE ≠ REST ; (3) REST explicite — survit intact jusqu'à la persistance, `final_session.kind="REST"` sous conditions neutres ; (4) `STRENGTH_LOWER`/`HEAVY` — charge préservée de bout en bout, branche Nutrition force existante activée (`NUTRITION_POLICY.baselineHydrationTargetL`) ; (5) `DH_TECHNICAL`/`MODERATE` — domaine Technique existant activé ; (6) **précédence protocole de course** — intention athlète `DH_TECHNICAL`/`MODERATE` + course du jour réelle (`event_start=event_end=today`) ⇒ le protocole de course gagne l'arbitrage réel (`final_session.kind="RACE_ACTIVITY"`, coarse persisté `"RACE_PREP"`) **tandis que** `planned_session_before` reste exactement `DH_TECHNICAL`/`MODERATE`, riche et coarse, en mémoire et dans `decisions.daily_plan.planned_session_before`/`decisions.planned_session_before` persistés — la preuve empirique exacte de la distinction verrouillée §Intention brute athlète vs baseline moteur ci-dessus. Chaque assertion de persistance identifie sa décision via `result.persistence.decision_id` (jamais "dernière row"/ordre non spécifié).

**Durcissement infrastructure de test partagée** : `head-coach-engine/tests/supabase/testDb.ts`'s `createTestClient()` refuse désormais de construire un client privilégié contre toute cible dont l'URL résolue (même précédence que l'existant : `process.env.SUPABASE_URL ?? LOCAL_URL`) n'est pas exactement `http://127.0.0.1:54321` ou `http://localhost:54321` (hôte **et** port stricts — un `http://localhost:9999` ou `http://127.0.0.2:54321` est refusé) — avant toute construction de client ou mutation réseau. Corrige une lacune de sécurité pré-existante (aucun des fichiers d'intégration `head-coach-engine` existants n'avait de garde de cible locale avant ce jalon ; les fichiers déjà existants restent inchangés et continuent de fonctionner normalement contre la stack locale réelle). Preuve négative empirique : `SUPABASE_URL` pointé sur le projet de production réel + opt-in + clé locale valide présents ⇒ les 6 scénarios T17 restent SKIPPED, complétés en moins d'une seconde (aucune tentative réseau).

Résultats de test finaux : `web` par défaut **485 passed / 9 skipped** (494 total, 36 fichiers, aucune stack Supabase requise) ; `web` complet avec opt-in **494/494 PASS**. `head-coach-engine` avec clé locale sans opt-in T17 **378 passed / 6 skipped** (384 total — les 6 skips sont exactement les scénarios destructifs T17) ; avec clé locale + opt-in T17 **384/384 PASS** ; edge **9/9** ; `ENGINE_VERSION` inchangé. Aucune migration, aucune action Supabase explicite, aucun changement `head-coach-engine/src/**`. **Aucune action de déploiement Vercel explicite dans V0.3_003D — l'état effectif du déploiement Vercel après push Git n'a pas été vérifié dans ce jalon** : implémenté sur `origin/main`, son état réel en production reste à ré-auditer en V0.3_003E (qui commence par cet audit avant toute décision de rollout).
...
## V0.3_006A — Safety Follow-up Coherence : investigation CLOSED (2026-09-09) ; V0.3_006A1 CLOSED / PRODUCTION ROLLOUT COMPLETE — 2026-09-09

### Objectif produit

Un second reviewer externe de production a déclaré une suspicion de commotion (REST complet, SAFETY A1), puis observé qu'un check-in ultérieur (`suspected_concussion = false`) avec le flag toujours non résolu produisait un plan `RECOVERY_ACTIVE` ("mobilité douce, marche") n'interdisant explicitement que le DH — une apparente contradiction avec le message initial "aucune activité physique", sans aucune explication visible du changement, et exposant des identifiants internes (`concussion_suspect`, `A · A5`) dans le texte athlete-facing.

### Investigation V0.3_006A (lecture seule)

Classification retenue : **CLASS D** (présentation + lifecycle combinés), **CLASS A explicitement rejetée**. Preuve : `rules/safety.ts` encode déjà, par construction, une politique à deux paliers — **A1** (checkbox du jour = `true`) → `action: "REST"`, arrêt complet immédiat via `buildSafetyPlan` (bypass total du pipeline de domaines) ; **A5** (checkbox du jour = `false` **mais** un flag `concussion_suspect` `active`/`monitoring` existe dans `RawContext.active_health_flags`) → `action: "ZERO_DH"`, seul le DH est remplacé (`RECOVERY_ACTIVE`), le reste du pipeline de domaines s'exécute normalement. Les deux entrées (checkbox true vs. flag persistant sans nouvelle déclaration) sont **différentes** — ce n'est donc jamais la même entrée produisant deux verdicts Safety distincts. `activeHealthFlags` n'est consommé **qu'à cet unique endroit** dans tout le moteur (aucun autre domaine/règle ne le lit) — confirmé par audit exhaustif. La contradiction réelle est double :
1. **Présentation** : `computeRecoveryDomain` (Couche C, générique) émet "mobilité douce, marche" pour tout `finalSession.kind === "RECOVERY_ACTIVE"` **sans aucune connaissance** d'un état Safety A5 actif — juxtaposé sans transition au message A1 initial. Le slug interne `concussion_suspect` est littéralement embarqué dans `A5.triggered_rule.detail`, qui alimente à la fois `plan.reasoning` (hero toujours visible) et le rendu brut `{rule.layer} · {rule.rule_id}` du panneau "Pourquoi cette décision ?" (non gated dev-only, contrairement au panneau technique).
2. **Lifecycle** : le schéma `health_flags` supporte déjà la résolution (`status`, `resolved_at`, `resolution_note`, `professional_consulted`, `professional_type`) mais **aucun code applicatif** (web/Edge/RPC) n'écrit jamais ces champs — `healthFlagsRepo.ts` est délibérément lecture seule, et la seule preuve de résolution dans tout le dépôt est un commentaire de test explicite ("fixture/test DB action, not an applicative repository", `runDailyFor.longitudinal.integration.test.ts`). `illness`/`injury_suspect`/`pain_persistent` partagent l'absence de résolution mais n'ont **aucun** consommateur Safety persistant équivalent à A5 — dette de lifecycle générique, distincte, non traitée par V0.3_006A1.

### V0.3_006A1 — Safety Surface & Integrity Hardening (implémentation)

Périmètre strictement limité au sous-ensemble sûr identifié par l'investigation — **aucun changement de politique clinique, aucune modification des actions A1/A5, aucun writer de résolution créé** :

1. **Intégrité DB** (`supabase/migrations/20260909090000_v0_3_006a1_health_flags_select_only_security.sql`) : policy `health_flags_own_data` (`FOR ALL`) remplacée par `health_flags_own_select` (`FOR SELECT` uniquement, `authenticated`), écritures directes révoquées (`authenticated`), tout privilège révoqué (`anon`) — même schéma que `decisions_append_only_security` (2026-08-19). Ferme la fuite découverte par l'investigation : un athlète authentifié pouvait auparavant résoudre directement son propre flag via PostgREST, hors de tout workflow produit. `persist_daily_run`/`service_role` inchangé (seul chemin d'écriture légitime). Voir `docs/05_DATA_MODEL.md` §health_flags.
2. **Sanitization présentation** (`web/src/features/dailyPlan/safetyPresentation.ts`, nouveau) : substitution ciblée par sous-chaîne exacte (jamais une reconstruction globale de `reasoning`) — seul `A5.detail` (le seul texte de règle Safety contenant un slug interne, vérifié par inspection exhaustive des textes A1–A5) est remplacé par une phrase factuelle verrouillée ("Un signal de suspicion de commotion déclaré précédemment est toujours actif. Les restrictions de sécurité associées restent appliquées.") — jamais une déclaration de politique clinique. `{rule.layer} · {rule.rule_id}` retiré du rendu athlete-facing. La provenance technique (`triggered_rules`, `reasoning` bruts) reste intégralement inchangée dans le `DailyPlan` sous-jacent — accessible via le panneau debug dev-only et `decisions.daily_plan`. **Web-only : `ENGINE_VERSION` inchangé** (`head-coach-engine@0.2.0-m1-v0.3_005b`).
3. **Précédence de présentation** : quand une règle de couche A (Safety) est active, "À éviter" (`protection.do_not_do`) est rendu avant "Récupération" dans `DailyPlanView.tsx` — ordre DOM conditionnel uniquement, contenu de chaque section inchangé, jamais une redéfinition de ce qui est cliniquement autorisé.
4. **Bandeau de provenance lecture seule** (`web/src/features/healthFlags/`, nouveau : `openHealthFlagsRepo.ts`, `HealthFlagBanner.tsx`, `healthFlagLabels.ts`) : affiché sur `/today`, **scopé exclusivement à `concussion_suspect`** — seul type avec un consommateur Safety persistant (A5). Copie factuelle verrouillée : libellé français + date de déclaration, "Ce signal reste actif même si tu réponds « Non » aujourd'hui.", et explicitement "NALYNT ne permet pas encore de clôturer ce suivi depuis l'application." — jamais une promesse de résolution future, jamais une instruction admin/DB. N'affiche délibérément rien pour un flag `illness`/`injury_suspect`/`pain_persistent` seul (aucun consommateur Safety persistant équivalent — afficher un bandeau générique aurait créé une nouvelle incohérence produit en impliquant un suivi que le moteur n'assure pas réellement).

### Production V0.3_006A1

Migration unique déployée sur `uvolpldwwyvadlamulvr` (parité 30/30). Web redéployé (`nalynt`/`louis-performance-system`, bundle réellement servi vérifié). Aucun déploiement Edge, aucun changement engine. Canary production à deux athlètes scratch distincts (jamais Louis, jamais le compte reviewer externe) : SELECT propre autorisé, SELECT croisé/INSERT/UPDATE/DELETE directs tous refusés dans les deux sens, persistance privilégiée `daily-run` toujours fonctionnelle (flag Safety créé via le chemin canonique réel). Preuve UI sur donnée réellement persistée en production. Résidu zéro après nettoyage. Détail complet : `docs/11_DECISION_LOG.md` (2026-09-09).
...
## V0.3_006C1 — DH Execution Guidance (implémentation locale, non déployée)

### Objectif produit

Suite de V0.3_006B : la revue externe confirme la prescription V1 utile (REV2-001/REV2-002 PARTIALLY FIXED) mais l'athlète devait encore traduire la recommandation en comportement de conduite concret — charge qualitative peu actionnable, terrain générique quel que soit le contexte, douleur non-SAFETY sans critère d'interruption immédiat, priorité mentale nommée mais jamais convertie en action, et trois fuites de copie interne (`wrist_L`, "non-SAFETY", "Mental RED"). Toujours aucun `DailyPlan.prescription` séparé — enrichissement exclusif des structures déjà authoritatives (`final_session`, `dh_or_technical`, `mental`, `monitoring`).

### Deux nouveaux champs sur `dh_or_technical` (aucun nouvel objet top-level) : `execution_task` et `load_guidance`

`DhTechnicalSection.execution_task?: string` — `focus` = quoi, `execution_task` = comment travailler ce focus aujourd'hui. Peuplé uniquement sur le chemin de repli générique (`resolveDhExecutionTask`, `domains/dhPrescription.ts`) — jamais dérivé du texte libre personnel `technique_primary_focus`, qui reste prioritaire mais ne produit jamais de tâche déterministe (aucun LLM, aucune tentative d'inférence de texte libre). `DhTechnicalSection.load_guidance?: string` — comment rouler selon la charge du jour (`resolveDhLoadGuidance`, même fichier ; voir la section dédiée ci-dessous). Trois champs distincts, jamais fusionnés : `focus`/`execution_task`/`load_guidance`. Tous deux optionnels, non enum-shaped — même absence de risque de dérive REV-001 que `duration_min` (V0.3_006B).

### Terrain — de deux booléens à une précédence à cinq rangs

`selectSpotHint` (`domains/technique.ts`) passe d'un système de combinaison (race+fatigue concaténés) à une précédence stricte à un seul gagnant, jamais concaténée : **douleur non-SAFETY constrainante** (`upper_grip`/`lower`, jamais "other") **> fatigue significative** (**AMBER ou RED** — corrigé lors de la vérification finale avant commit, voir ci-dessous) **> Mental RED > proximité course (inchangé) > frais/défaut**. `computeTechniqueDomain` reçoit deux nouveaux paramètres optionnels, tous deux lus comme contexte descriptif non-causal (aucune nouvelle interaction `SignalTrace`, le domaine ne consomme toujours aucun signal) : `painZoneCategory` (vient de `PainNonSafetyResult.zone_category`, désormais exporté par `rules/painNonSafety.ts` plutôt que recalculé) et `mentalRed` (`dimensions.mental.level === "RED"`). Les deux variantes terrain douleur (`sessionPrescriptionPolicy.ts#DH_SPOT_HINT.painUpperGrip/painLower`) restent un langage générique de réduction de sollicitation mécanique — jamais une affirmation de sécurité clinique pour une zone donnée (limite explicitement respectée, jamais franchie par ce jalon).

#### Correction finale avant commit — fatigue RED incluse dans le terrain

Le premier passage d'implémentation ne couvrait que `systemicLevel/legsLevel/armsGripLevel === "AMBER"` (variable interne `fatigueAmber`), héritage du C1.6 d'origine (`docs/03_COACHING_MODEL.md`, "Si fatigue AMBER : préférer spot proche") qui excluait explicitement RED — voir le test pré-existant "systemic RED alone → does NOT trigger C1.6". Une fatigue RED (ex. `leg_fatigue`/`grip_fatigue` ≥ 7, scénario de revue externe legs=9/grip=9) aboutit souvent à une session déjà réduite par Training (pivot `DH_LIGHT`/`LIGHT`, C3.5/C3.6) mais celle-ci **reste DH-family** : le terrain doit donc aussi refléter la demande réduite. Corrigé avant tout commit : renommé `fatigueAmber` → `meaningfulFatigue`, calculé via `level === "AMBER" || level === "RED"` sur chacune des trois dimensions — aucun changement aux seuils GREEN/AMBER/RED eux-mêmes (calculés en amont), à l'arbitrage Training, aux durées, ni à Safety. C1.6 mis à jour en conséquence dans `docs/03_COACHING_MODEL.md`. Couvert par la matrice de régression A-F (`t11_technique.test.ts`) et un scénario `buildDailyPlan` de bout en bout équivalent à la revue externe (`t15_dhSessionPrescription.test.ts`).

### Mental — action pré-run unifiée, remplaçant plutôt qu'ajoutant

`domains/mental.ts` : quand une action AMBER/RED se déclenche et que la session finale reste DH-family, `action_hint` devient `dhPreRunActionHint(focus)` — un template unique ("Avant de partir, fais quelques respirations lentes puis rappelle-toi ta priorité : [focus]. Pendant le run, reviens uniquement à ce focus.") **remplaçant** (jamais ajoutant à) le texte de base AMBER_STRESS/AMBER_MOTIVATION/RED_SUPPORTIVE — décision de revue corrigeant l'approche V0.3_006B initiale (append), qui aurait dupliqué la mention de respiration déjà présente dans le texte AMBER_STRESS de base.

### Monitoring immédiat douleur non-SAFETY

`resolveDhImmediatePainMonitoringNote` (`domains/dhPrescription.ts`) ajoute une phrase fixe à `monitoring.observe` quand `PAIN_NON_SAFETY` s'est déclenché et que la session finale reste DH-family — additive au monitoring 24–48h déjà produit par `evaluatePainNonSafety` (jamais un remplacement). Un critère d'interruption uniquement : jamais "tu peux rouler", jamais "la séance est sûre", jamais "commence prudemment", jamais un nouveau seuil numérique de douleur (SAFETY A2/A4 restent l'unique seuil dur).

### Nettoyage de présentation — extension du mécanisme V0.3_006A1, jamais un moteur de remplacement générique

`web/src/features/dailyPlan/safetyPresentation.ts` : `FIXED_RULE_OVERRIDES` (rule_id → chaîne fixe, mécanisme A5 inchangé) gagne `MENTAL_RED`. `PAIN_NON_SAFETY` — dont le `detail` est dynamiquement interpolé (code de zone, solicité ou non) — reçoit sa propre fonction de reconstruction (`sanitizePainNonSafetyDetail`) plutôt qu'une entrée fixe. Nouveau : `athleteSafeMonitoring`/`athleteSafeProtection`, qui remplacent tout code `pain_location_code` brut (les 33 valeurs connues, remplacement `\b`-délimité, jamais un remplacement générique non borné) dans `monitoring.observe`/`protection.do_not_do` — sections jamais couvertes par le sanitizer avant ce jalon. Source canonique unique du libellé français : `web/src/features/checkin/checkinTypes.ts#PAIN_LOCATION_LABELS` (33 valeurs, une par `pain_location_code`), réutilisée telle quelle par le sélecteur `<select>` du check-in lui-même (qui affichait le code brut avant ce jalon) — jamais une seconde copie de la table. `triggered_rules`/`decisions.daily_plan` restent byte-for-byte ce que le moteur a émis dans tous les cas ; seule la présentation athlete-facing change.

### Charge — comportement d'exécution, engine-emitted et persisté (`load_guidance`)

Nouveau champ optionnel `DhTechnicalSection.load_guidance?: string`, résolu par `resolveDhLoadGuidance(kind, load_profile)` (`domains/dhPrescription.ts`) depuis la copie approuvée `sessionPrescriptionPolicy.ts#DH_LOAD_GUIDANCE` (LIGHT/MODERATE/HEAVY — texte exact repris tel quel de la version initialement web-only, voir `docs/03_COACHING_MODEL.md` §DH Execution Guidance). Calculé dans `computeTechniqueDomain` à partir de `finalSession.load_profile` (la session déjà entièrement arbitrée), donc automatiquement absent pour un kind non DH-family (A1 REST, A5 `RECOVERY_ACTIVE` → `dh_or_technical` reste `{active:false}`, aucune valeur périmée). Le rendu web (`DailyPlanView.tsx`) affiche `dh_or_technical.load_guidance` tel quel quand présent, et ne recalcule **jamais** cette copie depuis `final_session.load_profile` seul ; un plan sans ce champ (legacy) retombe sur le libellé neutre existant `dailyPlanLabels.ts#LOAD_PROFILE_LABELS` (ex. "charge lourde"), jamais sur la nouvelle instruction comportementale.

#### Correction d'architecture, avant tout commit — invariant canonique d'historique

Cette copie était initialement conçue web-only (`dhPrescriptionLabels.ts#DH_LOAD_DESCRIPTION`, un lookup statique enum-keyed comme `TRAINING_KIND_LABELS`, recalculé depuis `final_session.load_profile`). Revue corrigée : c'est une prescription de coaching substantielle, pas un simple libellé d'UI — un `DailyPlan` persisté doit rester ce que le coach a réellement prescrit à la génération. Une recomputation web-side aurait fait gagner rétroactivement ce texte à un `DailyPlan` V0.3_006B déjà persisté qui ne l'a jamais réellement contenu, dès qu'un futur déploiement web changerait le lookup — violation de l'invariant "History ne recalcule jamais côté navigateur, ne fait que présenter le persisté". Corrigée avant tout commit de ce jalon : `DH_LOAD_DESCRIPTION` supprimé de `dhPrescriptionLabels.ts`, remplacé par `DH_LOAD_GUIDANCE` engine-side (ci-dessus). `execution_task`/`spot_hint`/le nettoyage de présentation restent inchangés par cette correction (déjà engine-side ou déjà purement présentationnels sans contenu de coaching propre).

### Fenêtre de session — copie UX, aucun changement moteur

`DH_SESSION_WINDOW_CAPTION` ajoute une légende sous la ligne "Fenêtre de session" existante ("Inclut les remontées, pauses et temps d'attente — pas seulement le temps de descente.") — web-only, aucun changement `head-coach-engine/src/**`.

### `ENGINE_VERSION`

`head-coach-engine@0.2.0-m1-v0.3_006c1` — bump requis : le contenu réellement émis par `buildDailyPlan` change pour tout kind DH-family (`execution_task` nouveau, `load_guidance` nouveau, `spot_hint` désormais précédence-déterministe avec de nouvelles valeurs possibles, `mental.action_hint` restructuré, `monitoring.observe` gagne potentiellement une entrée). Conservé inchangé pour la correction `load_guidance` elle-même (même jalon non déployé, non commité au moment du bump initial — pas de second bump pour une correction pré-commit). Le nettoyage de présentation et la copie de fenêtre restent web-only, sans effet sur `ENGINE_VERSION` à eux seuls.

### Contrat engine/web

`execution_task` et `load_guidance` suivent tous deux exactement le précédent `duration_min` (V0.3_006B) : champs optionnels, non enum-shaped, jamais validés par `isValidIntervention`/`isValidDailyPlan` au-delà de la présence de la section — aucun nouveau risque de dérive de type REV-001. Chaîne complète (émission moteur → acceptation web → rendu Today → persistance/restauration → rendu History → compatibilité legacy) verrouillée par des tests permanents dans `head-coach-engine/tests/{t11_technique,t14_crossDomainIntegration,t15_dhSessionPrescription}.test.ts` et `web/src/features/{dailyPlan/{DailyPlanResult,DailyPlanPanel},history/HistoryDetail,checkin/{CheckinForm,checkinTypes}}.test.ts`.
