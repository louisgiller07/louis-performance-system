...
- **V0.3_003 — Planning / Session Intent** : architecture verrouillée le 2026-08-31 (voir `docs/06_ARCHITECTURE.md` §V0.3_003 et `docs/11_DECISION_LOG.md`) ; décomposition V0.3_003A (architecture, CLOSED) → V0.3_003B (data-access/write path, **CLOSED LOCALLY 2026-08-31**) → V0.3_003C (web planning workflow, **CLOSED LOCALLY 2026-08-31**) → V0.3_003D (Today integration/régressions, **CLOSED LOCALLY 2026-08-31**) → V0.3_003E (rollout production, **NEXT**). **Objectif produit** : donner à l'athlète un chemin produit réel pour définir `planned_sessions` (saisie manuelle, distincte d'un planificateur automatique — voir ci-dessous). **État atteint après V0.3_003D** : l'intention Planning de l'athlète est désormais connectée localement à la boucle quotidienne — `/today` affiche un résumé lecture-seule "Prévu aujourd'hui" (`web/src/features/planning/TodayPlanningSummary.tsx`), et `head-coach-engine/tests/supabase/t17_planningE2E.integration.test.ts` prouve contre une vraie stack Supabase locale (aucun mock) que `planned_sessions → RawContext → arbitrage existant → DailyPlan → persist_daily_run → decisions` fonctionne exactement comme déjà documenté, y compris le cas critique où le protocole de course gagne l'arbitrage réel (`final_session` = `RACE_ACTIVITY`) tout en préservant l'intention brute athlète (`planned_session_before` = `DH_TECHNICAL`/`MODERATE`, riche et coarse, en mémoire et persistée). **Aucune action de déploiement Vercel explicite dans V0.3_003D — l'état effectif du déploiement Vercel après push Git n'a pas été vérifié dans ce jalon** ; V0.3_003E commence par ré-auditer le mécanisme réel de déploiement et l'état actuellement servi avant toute décision de rollout. — **le moteur ne change pas** : `planned_session` est déjà consommé exactement comme documenté ci-dessus (§1 RawContext, §5 "Le moteur part du `planned_session`"), désormais prouvé par un test réel plutôt que seulement audité. Aucun nouveau champ `DailyPlan`, aucune migration, aucun changement `ENGINE_VERSION` — confirmé sur l'implémentation réelle de V0.3_003B/C/D (`head-coach-engine/src/**` non touché).
