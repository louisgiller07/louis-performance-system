...
- **V0.3_003 — Planning / Session Intent** : architecture verrouillée le 2026-08-31 (voir `docs/06_ARCHITECTURE.md` §V0.3_003 et `docs/11_DECISION_LOG.md`) ; décomposition V0.3_003A (architecture, CLOSED) → V0.3_003B (data-access/write path, **CLOSED LOCALLY 2026-08-31**) → V0.3_003C (web weekly planner, **NEXT**) → V0.3_003D (Today integration/régressions) → V0.3_003E (rollout production). **Objectif produit** : donner à l'athlète un chemin produit réel pour définir `planned_sessions` (saisie manuelle, distincte d'un planificateur automatique — voir ci-dessous). **État atteint après V0.3_003B** : la fondation data-access/write est implémentée localement (`web/src/features/planning/planningRepo.ts`, CRUD Supabase authentifié direct sous la RLS existante) — **aucune UI `/plan` n'existe encore, l'athlète ne peut donc pas encore créer/éditer une séance planifiée via le produit normal**. V0.3_003C exposera ce chemin via la route `/plan`. — **le moteur ne change pas** : `planned_session` est déjà consommé exactement comme documenté ci-dessus (§1 RawContext, §5 "Le moteur part du `planned_session`"). Aucun nouveau champ `DailyPlan`, aucune migration, aucun changement `ENGINE_VERSION` — confirmé sur l'implémentation réelle de V0.3_003B (`head-coach-engine/src/**` non touché).
