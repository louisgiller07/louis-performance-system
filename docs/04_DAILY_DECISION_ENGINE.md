...
- **V0.3_003 — Planning / Session Intent** : architecture verrouillée le 2026-08-31 (voir `docs/06_ARCHITECTURE.md` §V0.3_003 et `docs/11_DECISION_LOG.md`) ; décomposition V0.3_003A (architecture, CLOSED) → V0.3_003B (data-access/write path, **CLOSED LOCALLY 2026-08-31**) → V0.3_003C (web planning workflow, **CLOSED LOCALLY 2026-08-31**) → V0.3_003D (Today integration/régressions, **NEXT**) → V0.3_003E (rollout production). **Objectif produit** : donner à l'athlète un chemin produit réel pour définir `planned_sessions` (saisie manuelle, distincte d'un planificateur automatique — voir ci-dessous). **État atteint après V0.3_003C** : la route authentifiée `/plan` (horizon aujourd'hui→J+6) est implémentée et committée localement (`web/src/pages/PlanPage.tsx` + `web/src/features/planning/PlanningDayCard.tsx`, sur `web/src/features/planning/planningRepo.ts` déjà clos en 003B) — l'athlète peut créer/éditer/supprimer une séance planifiée à travers cette UI **en local**, mais **rien n'est encore déployé en production** (Vercel), et `/today` ne l'intègre pas encore (V0.3_003D). — **le moteur ne change pas** : `planned_session` est déjà consommé exactement comme documenté ci-dessus (§1 RawContext, §5 "Le moteur part du `planned_session`"). Aucun nouveau champ `DailyPlan`, aucune migration, aucun changement `ENGINE_VERSION` — confirmé sur l'implémentation réelle de V0.3_003B/C (`head-coach-engine/src/**` non touché).
