...
- `recent_recovery_context` (V0.3_008A, CLOSED / PRODUCTION ROLLOUT COMPLETE — optionnel, `RecentRecoveryContext | undefined`, voir `07_GLOSSARY.md`) : contexte factuel J-1 uniquement, jamais consommé par `SignalTrace`, jamais un signal d'arbitrage — pur passthrough vers `DailyPlan.recent_recovery_context`, instantané immuable persisté tel quel
- `recent_technical_context` (V0.3_008B, CLOSED / PRODUCTION ROLLOUT COMPLETE — optionnel, `RecentTechnicalContext | undefined`, voir `07_GLOSSARY.md`) : dernière tâche technique valide dans la fenêtre stricte inter-jours D-14..D-1, jamais consommé par `SignalTrace`, jamais un signal d'arbitrage — surfacé conditionnellement (session finale du jour DH-family uniquement) comme `DailyPlan.dh_or_technical.prior_task_reference`, instantané immuable
...
**DH Execution Guidance (V0.3_006C1, corrigé V0.3_008B0)** : `dh_or_technical` gagne un champ optionnel `execution_task` — repli générique fixe par kind, présent dès que la session finale est DH-family, indépendamment de la présence d'un focus personnel (V0.3_008B0 corrige le comportement initial qui le rendait absent dès qu'un focus personnel existait ; jamais dérivé du texte libre personnel dans un cas comme dans l'autre). Le terrain (`spot_hint`, `domains/technique.ts#selectSpotHint`) devient précédence-déterministe : douleur non-SAFETY constrainante > fatigue significative > Mental RED > proximité course > frais/défaut, une seule recommandation gagnante, jamais concaténée. `mental.action_hint` (couche C) intègre désormais une action pré-run concrète + le focus déjà résolu comme priorité attentionnelle, quand la session finale reste DH-family. `monitoring.observe` gagne, pour la douleur non-SAFETY, un critère d'interruption immédiat additif au suivi 24–48h existant (jamais un remplacement, jamais une déclaration de sécurité). Aucun de ces ajouts ne touche `rules/safety.ts` ni la classification SAFETY/non-SAFETY de la douleur. Voir `docs/03_COACHING_MODEL.md` §DH Execution Guidance et `docs/11_DECISION_LOG.md`.

**Technical Continuity V1 (V0.3_008B, CLOSED / PRODUCTION ROLLOUT COMPLETE)** : `dh_or_technical` gagne un champ optionnel `prior_task_reference` — la dernière tâche technique valide (`execution_task` + `technical_outcome`) d'une décision antérieure, persisté **uniquement** quand la session finale du jour est elle-même DH-family. Source exacte : `completed_sessions.decision_id → decisions.id` (jamais une inférence par date), kind performé lu depuis `completed_sessions.intervention.kind` (jamais la prescription), fenêtre strictement inter-jours `D-14 ≤ session_date < D` (jour même et futur exclus, PROVISIONAL PRODUCT-FRESHNESS CONSTANT). DISPLAY ONLY : aucun effet d'arbitrage, n'altère jamais `execution_task` du jour, aucun matching sémantique cross-kind. Voir `docs/03_COACHING_MODEL.md` §Continuité technique et `docs/07_GLOSSARY.md` et `docs/11_DECISION_LOG.md`.
...
- **V0.3_002 — Domain Coaching Enrichment : COMPLETE (2026-08-30)** : architecture verrouillée le 2026-08-28 (voir `docs/06_ARCHITECTURE.md` §V0.3_002 et `docs/11_DECISION_LOG.md`) ; décomposition V0.3_002A (architecture, CLOSED) → V0.3_002B (Technique DH) → V0.3_002C (Mental) → V0.3_002D (Nutrition) → V0.3_002E (intégration/régressions, test-only) → V0.3_002F (rollout remote/clôture) — les 6 jalons sont tous **CLOSED**. `dh_or_technical`/`mental`/`nutrition` sont désormais tous actifs, leur composition cross-domaine est régression-testée (T14), leur compatibilité web est verrouillée par un test durable réel-engine (`DailyPlanView.enriched.test.tsx`), et `daily-run` sert cet engine enrichi (`head-coach-engine@0.2.0-m1-v0.3_002d`) en production sur `uvolpldwwyvadlamulvr`.
- **V0.3_003 — Planning / Session Intent : COMPLETE (2026-09-02)** : architecture verrouillée le 2026-08-31 (voir `docs/06_ARCHITECTURE.md` §V0.3_003 et `docs/11_DECISION_LOG.md`) ; décomposition V0.3_003A (architecture, CLOSED) → V0.3_003B (data-access/write path, **CLOSED LOCALLY**) → V0.3_003C (web planning workflow, **CLOSED LOCALLY**) → V0.3_003D (Today integration/régressions, **CLOSED LOCALLY**) → V0.3_003E (rollout production + clôture, **CLOSED / PRODUCTION ROLLOUT COMPLETE 2026-09-02**) — les 5 jalons sont tous **CLOSED**. **Objectif produit** : donner à l'athlète un chemin produit réel pour définir `planned_sessions` (saisie manuelle, distincte d'un planificateur automatique — voir ci-dessous). **État final production** : l'intention Planning de l'athlète est désormais connectée de bout en bout, en production — `/plan` (saisie 7 jours) et `/today` (résumé lecture-seule "Prévu aujourd'hui", `web/src/features/planning/TodayPlanningSummary.tsx`) sont servis sur `https://louis-performance-system.vercel.app`. `head-coach-engine/tests/supabase/t17_planningE2E.integration.test.ts` prouve localement (aucun mock) que `planned_sessions → RawContext → arbitrage existant → DailyPlan → persist_daily_run → decisions` fonctionne exactement comme déjà documenté, y compris le cas critique où le protocole de course gagne l'arbitrage réel (`final_session` = `RACE_ACTIVITY`) tout en préservant l'intention brute athlète (`planned_session_before` = `DH_TECHNICAL`/`MODERATE`, riche et coarse, en mémoire et persistée) — **ce même contrat athlète-intent-vs-arbitrage a été reconfirmé en production par le canary remote authentifié V0.3_003E** (scénario neutre sans course : écriture Planning authentifiée sous RLS réelle + `daily-run` de production invoqué sous JWT scratch réel, jamais `service_role` ; `planned_session_before`/`final_session` riches et coarses confirmés en mémoire et en persistance, `dh_or_technical.active=true`, `engine_version` confirmé directement `head-coach-engine@0.2.0-m1-v0.3_002d`). Le déploiement Vercel de production a nécessité un correctif de frontière de build TypeScript sans rapport avec le produit Planning (voir `docs/06_ARCHITECTURE.md` §V0.3_003E) — sans impact production, zéro régression. — **le moteur ne change pas** : `planned_session` est déjà consommé exactement comme documenté ci-dessus (§1 RawContext, §5 "Le moteur part du `planned_session`"), désormais prouvé par un test réel local ET par un canary réel en production plutôt que seulement audité. Aucun nouveau champ `DailyPlan`, aucune migration, aucun changement `ENGINE_VERSION` sur l'ensemble de V0.3_003 (`head-coach-engine/src/**` non touché).
- Debrief course post-mortem structuré
- Planificateur hebdomadaire automatique (génération de `planned_sessions` sans saisie athlète — distinct de la saisie manuelle V0.3_003 ci-dessus)
