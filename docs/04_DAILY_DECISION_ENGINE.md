...
- **V0.3_003 — Planning / Session Intent** : architecture verrouillée le 2026-08-31 (voir `docs/06_ARCHITECTURE.md` §V0.3_003 et `docs/11_DECISION_LOG.md`) ; décomposition V0.3_003A (architecture, CLOSED) → V0.3_003B (data-access/write path) → V0.3_003C (web weekly planner) → V0.3_003D (Today integration/régressions) → V0.3_003E (rollout production). Donne à l'athlète un chemin produit réel pour définir `planned_sessions` — **le moteur ne change pas** : `planned_session` est déjà consommé exactement comme documenté ci-dessus (§1 RawContext, §5 "Le moteur part du `planned_session`"). Aucun nouveau champ `DailyPlan`, aucune migration, aucun changement `ENGINE_VERSION` attendus.
