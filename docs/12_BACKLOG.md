...
**V0.3_003 — Planning / Session Intent : IN PROGRESS (depuis 2026-08-31).** V0.3_003A **CLOSED / ARCHITECTURE LOCKED** ; V0.3_003B (data-access/write path) **CLOSED LOCALLY** ; V0.3_003C (web planning workflow) **CLOSED LOCALLY** ; V0.3_003D (Today integration + e2e réel local) **CLOSED LOCALLY** (2026-08-31) ; V0.3_003E **NOT STARTED** — voir ci-dessous et `docs/11_DECISION_LOG.md`.
**Phase actuelle : V0.3_003 IN PROGRESS.** V0.3_003A (architecture) **CLOSED**, V0.3_003B (data-access/write path) **CLOSED LOCALLY**, V0.3_003C (web planning workflow, `/plan`) **CLOSED LOCALLY**, V0.3_003D (Today integration + e2e réel local) **CLOSED LOCALLY** (2026-08-31, résumé "Prévu aujourd'hui" + T17 6/6 scénarios destructifs PASS contre une vraie stack Supabase locale, implémenté sur `origin/main` ; aucune action de déploiement Vercel explicite dans ce jalon, état effectif en production non vérifié). Prochain jalon concret : **V0.3_003E — rollout production + clôture finale V0.3_003**, dont la première étape est de ré-auditer le mécanisme de déploiement Vercel réel (Git auto-deploy ou non) et l'état actuellement servi, avant tout choix de commande — portée à verrouiller dans `docs/06_ARCHITECTURE.md` §V0.3_003. `ActiveExperiment` reste le candidat suivant après V0.3_003, différé mais non annulé. V0.3_002 reste **COMPLETE** comme historique clos.
