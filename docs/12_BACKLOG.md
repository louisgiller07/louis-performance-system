...
### V0.3_008B — Continuité technique (INVESTIGATION D'ARCHITECTURE REQUISE, non démarrée — débloquée par V0.3_008B0)
Rendre une future séance DH pertinente consciente de la dernière tâche d'exécution technique persistée (`decisions.daily_plan.dh_or_technical.execution_task`) et du `technical_outcome` athlete-reported correspondant (option B de l'investigation V0.3_008). **Différent de V0.3_008A par nature, pas seulement par périmètre** : nécessite que le Head Coach lise `decisions` pour la toute première fois — aucun précédent existant dans le pipeline `daily-run`. Verrouillage d'architecture requis avant toute implémentation (matching exact sans heuristique, staleness, family-matching DH, snapshot d'audit pour la même raison qu'en V0.3_008A). Corrections d'architecture déjà verrouillées pour ce futur verrouillage (V0.3_008B0, `docs/11_DECISION_LOG.md`) : `source_decision_id` requis dans le snapshot (interne, jamais athlete-facing — plusieurs décisions possibles le même jour) ; kind performé depuis `completed_sessions.intervention.kind`, jamais dérivé de la prescription ; requête candidate minimale `session_date, decision_id, technical_outcome, intervention` ; sémantique d'échec précisée (candidat malformé → suivant ; aucun candidat → contexte absent ; échec repository réel → politique d'erreur existante, jamais silencieusement avalé) ; fenêtre PROVISIONAL 14 jours toujours non verrouillée. Ne pas implémenter avant ce verrouillage.
