...
### V0.3_008B — Continuité technique (ARCHITECTURE LOCKED, architect-approved avec corrections — non implémentée)
Rendre une future séance DH pertinente consciente de la dernière tâche d'exécution technique persistée (`decisions.daily_plan.dh_or_technical.execution_task`) et du `technical_outcome` athlete-reported correspondant (option B de l'investigation V0.3_008), désormais réellement atteignable pour Louis depuis V0.3_008B0. **Différent de V0.3_008A par nature, pas seulement par périmètre** : nécessite que le Head Coach lise `decisions` pour la toute première fois — aucun précédent existant dans le pipeline `daily-run`.

**Architecture verrouillée (`docs/11_DECISION_LOG.md` 2026-09-14, architect-approved avec corrections — aucune implémentation)** : source = `completed_sessions.decision_id` → `decisions.id` exact uniquement, jamais date/latest/similarité/Planning/Today ; `source_decision_id` **requis** dans le futur instantané `dh_or_technical.prior_task_reference` (interne, jamais athlete-facing) ; kind performé = `completed_sessions.intervention.kind` uniquement, jamais reconstruit depuis la prescription. **Fenêtre strictement inter-jours** : pour une décision générée le jour D, éligible ⟺ `D-14 ≤ session_date < D` — le jour même D et le futur sont **exclus** (une séance complétée plus tôt le même jour D ne peut jamais nourrir une autre décision générée plus tard ce même jour D). Requête : **2 appels bornés** (coût explicitement architect-approved, jamais présenté comme zéro), aucun N+1 — candidats `completed_sessions` (`session_date, decision_id, technical_outcome, intervention`, filtrés `athlete_id`, `decision_id`/`technical_outcome` non-null, fenêtre ci-dessus, `LIMIT 14` — dérivé mathématiquement de la contrainte `unique_completed_per_day UNIQUE (athlete_id, session_date)`, jamais un chiffre arbitraire) puis un batch `decisions` unique (`.in("id", ...)`, `id, decision_date, daily_plan`, filtré `athlete_id`), résolution client-side du plus récent au plus ancien avec vérification défensive `decision.id === candidate.decision_id ET decision.decision_date === candidate.session_date`. Sémantique d'échec, chaque cas SKIP énuméré : decision introuvable dans le batch, désaccord de date, `execution_task` absent/vide (cas réel pour toute décision pré-008B0), `intervention.kind` invalide, `technical_outcome` hors énumération → candidat ignoré, suivant essayé (le plus récent n'est pas nécessairement le plus valide) ; aucun candidat valide → contexte absent ; échec repository réel → politique d'erreur existante, jamais silencieusement avalé. Type `RawContext.recent_technical_context?: RecentTechnicalContext` (`source_decision_id, session_date, kind, execution_task, technical_outcome, age_days` — `age_days` calculé à la lecture, jamais stocké, jamais dans l'instantané). `DailyPlan.dh_or_technical.prior_task_reference` persisté **uniquement** quand la session finale du jour est elle-même DH-family (même gate que `focus`/`execution_task`), instantané immuable via `decisions.daily_plan` existant (même mécanisme que V0.3_008A, aucune nouvelle table). Consommateur V1 = **DISPLAY ONLY** (aucune SignalTrace, aucun triggered_rule, aucun effet d'arbitrage, n'altère jamais `execution_task` du jour) ; progression automatique (répéter/simplifier/progresser selon `technical_outcome`) explicitement **différée**. Pumptrack pleinement éligible, non special-cased ; V1 ne fait aucun matching sémantique cross-kind. Copie UI distincte de « Tâche du jour » : sous-bloc « Tâche précédente » dans la carte Séance DH existante, tâche citée verbatim + phrase factuelle neutre par `technical_outcome` — **« dernière séance technique »**, jamais « séance DH » (faux pour une source Pumptrack), jamais l'enum brut, jamais de décompte de jours. Récence = **14 jours, PROVISIONAL PRODUCT-FRESHNESS CONSTANT** — non physiologique, non spécifique à Louis, recalibrable depuis un futur dogfood, jamais présentée comme une vérité Elite/scientifique. Sécurité réaffirmée sans changement : `daily-run` reste `service_role`/RLS-non-protecteur en interne, isolation par filtrage explicite `athlete_id` sur les deux nouvelles requêtes. Aucune migration DB, aucune RPC, aucune Edge Function. `ENGINE_VERSION` attendu : `head-coach-engine@0.2.0-m1-v0.3_008b`. **Ne pas implémenter avant autorisation explicite séparée.**
