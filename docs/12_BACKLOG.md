...
**V0.3_003 — Planning / Session Intent : COMPLETE (2026-09-02).** V0.3_003A **CLOSED / ARCHITECTURE LOCKED** ; V0.3_003B (data-access/write path) **CLOSED LOCALLY** ; V0.3_003C (web planning workflow) **CLOSED LOCALLY** ; V0.3_003D (Today integration + e2e réel local) **CLOSED LOCALLY** ; V0.3_003E (rollout production + clôture) **CLOSED / PRODUCTION ROLLOUT COMPLETE** (2026-09-02) — voir ci-dessous et `docs/11_DECISION_LOG.md`.
**Phase actuelle : V0.3_003 COMPLETE.** V0.3_003A (architecture) **CLOSED**, V0.3_003B (data-access/write path) **CLOSED LOCALLY**, V0.3_003C (web planning workflow, `/plan`) **CLOSED LOCALLY**, V0.3_003D (Today integration + e2e réel local) **CLOSED LOCALLY** (résumé "Prévu aujourd'hui" + T17 6/6 scénarios destructifs PASS contre une vraie stack Supabase locale), V0.3_003E (rollout production + clôture) **CLOSED / PRODUCTION ROLLOUT COMPLETE** (2026-09-02 — défaut de frontière de build TypeScript découvert et corrigé sans impact production, déploiement `dpl_HQDuP893LoDSuqxdfzWXMbABmVzT` réussi, T18 canary remote authentifié PASS, zéro écriture athlète réel). Aucun prochain jalon concret n'est encore sélectionné/architecturé. `ActiveExperiment` reste le candidat suivant après V0.3_003, différé mais non annulé — pas automatiquement autorisé par cette clôture, décision de priorisation Louis restant à prendre. V0.3_002 reste **COMPLETE** comme historique clos.
...
## V0.3_002B — Technique DH (CLOSED LOCALLY, 2026-08-28)

- [x] `computeTechniqueDomain` (`src/domains/technique.ts`) — actif pour `DH_TECHNICAL`/`DH_PERFORMANCE`/`DH_LIGHT`/`PUMPTRACK` (séance finale post-arbitrage), inactif pour les 12 autres kinds
- [x] Focus unique approuvé : "Fixe ta ligne, dose le freinage, laisse rouler." — aucune rotation
- [x] `spot_hint` — allowlist exact à 4 chaînes catégorielles, aucune venue nommée
- [x] C1.5 — proximité course J+1..J+14 sur `RawContext.upcoming_races` (`TECHNIQUE_POLICY.raceProximityWindowDays`), adaptateur `raceCalendarRepo.ts` élargi en lecture seule (borne future uniquement — exception M2 read-only explicitement approuvée après le preflight, à la frontière frozen M1-M4, sans modification d'aucun contrat de persistance/écriture M2), `EventContext`/`PRE_EVENT` (7 jours) inchangé et jamais le sélecteur C1.5, inertie prouvée par intégration réelle (6 cas)
- [x] C1.6 — fatigue AMBER = `systemic` OU `legs` OU `arms_grip`, RED seul exclu, zéro interaction `SignalTrace`
- [x] Config profil athlète bornée (`athleteCoachingProfile.ts`) + config de politique de domaine distincte (`techniquePolicy.ts`)
- [x] `ENGINE_VERSION` → `head-coach-engine@0.2.0-m1-v0.3_002b` (provenance comportementale, `package.json` inchangé)
- [x] Safety/Training/`SignalTrace`/`eventContext.ts`/`raceProtocol.ts` strictement inchangés
- [x] Tests : `npm test` 275/275, `npm run test:edge` 9/9, build PASS
...
## V0.3_002D — Nutrition (CLOSED LOCALLY, 2026-08-30)

- [x] `computeNutritionDomain` (`src/domains/nutrition.ts`) — lit le contexte déjà stabilisé, `focus`/branche primaire orthogonaux
- [x] `focus` — uniquement `active_mode === "RACE_WEEK"` (jamais `PRE_EVENT`), texte unique de rappel énergétique générique
- [x] Branche primaire — précédence RACE DAY (`event_context.in_progress`) > JOUR DH (`final_session.kind`) > SÉANCE DE FORCE PLANIFIÉE (`ctx.planned_session.kind` brute) > aucune ; contrôle `notes`+`hydration_target_l` ensemble, aucune fuite structurellement possible
- [x] `hydration_target_l = 2` uniquement sur la branche force (C5.3, seule cible numérique canonique unique) ; C5.4/C5.2/C5.6 restent texte uniquement
- [x] Config `nutritionPolicy.ts` (NEW) — source de vérité runtime réelle des quatre constantes : `baselineHydrationTargetL` pilote le champ structuré `hydration_target_l`, les trois autres pilotent les textes déterministes (durcissement), aucune extension `athleteCoachingProfile.ts`
- [x] `ENGINE_VERSION` → `head-coach-engine@0.2.0-m1-v0.3_002d`, propriétaire unique désormais `tests/engineVersion.test.ts` (copies retirées de T12 et T13)
- [x] Safety/Training/Technique/Mental/`SignalTrace`/`eventContext.ts`/`raceProtocol.ts` strictement inchangés ; aucune `TriggeredRule` Nutrition
- [x] Tests : `npm test` 347/347, `npm run test:edge` 9/9, build PASS
- [x] Aucune migration, aucun nouveau champ `daily_checkins`, aucun changement web, aucun déploiement remote

### Explicitement hors périmètre V0.3_002D (reporté)

Intégration/régressions cross-domaine formelles (V0.3_002E), rollout remote (V0.3_002F), suivi calories/repas/macros, C5.5 (`ActiveExperiment`).

---

## V0.3_002E — Intégration/régressions cross-domaine (CLOSED LOCALLY, 2026-08-30, test-only)

- [x] `head-coach-engine/tests/t14_crossDomainIntegration.test.ts` (NEW, seul fichier créé) — aucun fichier de production, aucun test historique modifié
- [x] `PRE_EVENT` (Mental)/`RACE_WEEK` (Nutrition) — indépendance prouvée sur les 3 quadrants via `buildDailyPlan` réel
- [x] Coexistence des trois domaines (DH + Mental AMBER) — exactement une `TriggeredRule` `MENTAL_AMBER_STRESS`, zéro `TECHNIQUE_*`/`NUTRITION_*`
- [x] Late-push Mental étendu à une paire GREEN/AMBER avec Technique+Nutrition actifs des deux côtés — tous les champs Training-dérivés strictement égaux
- [x] Propriété `MENTAL_RED` confirmée sur DH réel — kind survit, Technique/Nutrition suivent le `final_session` stabilisé, aucune seconde règle Mental observable (preuve directe `consume()` = T12, non T14)
- [x] Propagation `final_session` + asymétrie `planned_session` acceptée — prouvées sur un fixture réel (`GRIP_WORK`+RED double)
- [x] Composition jour de course réel (`computeEventContext`) — Nutrition race-day + Mental AMBER coexistent, `mental.focus` structurellement absent
- [x] Safety dominant une pression maximale (concussion + RACE_WEEK + PRE_EVENT + DH planifiée) — `triggered_rules=[A1]`, zéro fuite cross-domaine
- [x] Aucun `rule_id` `TECHNIQUE_*`/`NUTRITION_*` à travers tous les plans combinés
- [x] Régression Recovery appariée sous charge Technique+Mental+Nutrition
- [x] Déterminisme d'un `DailyPlan` riche multi-domaine
...
## V0.3_003A — Planning / Session Intent, architecture/contrats (CLOSED / ARCHITECTURE LOCKED, 2026-08-31)

- [x] Jalon parent verrouillé : V0.3_003A (architecture, ce jalon) → V0.3_003B (data-access/write path) → V0.3_003C (web weekly planner) → V0.3_003D (Today integration/régressions) → V0.3_003E (rollout production)
- [x] Aucune migration — schéma `planned_sessions` existant (unicité `(athlete_id,planned_date)`, RLS athlète-propre déjà `FOR ALL`) confirmé suffisant
- [x] Architecture d'écriture verrouillée — CRUD authentifié direct sous RLS existante, aucune Edge Function/RPC nouvelle
- [x] Contrat mutabilité verrouillé, preuve source exacte (`buildDailyPlan.ts:302`) — `planned_session_before` = `ctx.planned_session` directement, jamais `baseline` ; `decisions` append-only inchangée
- [x] DELETE vs REST explicite verrouillé comme deux états distincts
- [x] `planned_intent` = `NULL` systématique ; 5 colonnes engine-inertes préservées (`OMIT AND PRESERVE`, empiriquement prouvé en local le 2026-08-31)
- [x] Catalogue verrouillé — 16 kinds réels, 15 plannables, `RACE_ACTIVITY` exclu
- [x] Mapping `session_type` réutilisé (`trainingInterventionToSessionType.ts`), aucune duplication
- [x] Aucun mécanisme d'invalidation nouveau requis (routes à démontage confirmé) ; résumé `/today` doit recharger l'intention au montage
- [x] Aucun changement `head-coach-engine/src/**`, aucun changement `DailyPlan`, aucun bump `ENGINE_VERSION`
- [x] Canary remote scratch et smoke web production verrouillés comme **requis** (non optionnels) à V0.3_003E
...
## V0.3_003D — Planning / Session Intent, Today integration + e2e réel local (CLOSED LOCALLY, 2026-08-31)

- [x] Implémentation + durcissement (`05b3123`, `feat: integrate V0.3_003D planning with today`) poussés sur `origin/main`
- [x] Résumé lecture-seule "Prévu aujourd'hui" sur `/today` (`TodayPlanningSummary.tsx`, après la date, avant le check-in) — représente uniquement l'intention `planned_sessions` courante, jamais le fallback/protocole de course/décision Head Coach
- [x] Réutilise `loadPlannedSessions(athleteId, today, today)` (003B), aucune nouvelle méthode de dépôt, aucun contexte/cache/`planningRevision`
- [x] États Non planifié / Repos / rich / legacy coarse sûr / enum inconnu sûr (jamais l'enum brut) ; lien "Modifier dans Plan" vers `/plan` ; aucune mutation inline
- [x] Échec de lecture Planning isolé ("Planning indisponible"), n'affecte jamais check-in/daily-run/décision Head Coach
- [x] Correctif ciblé 003C : `PlanningDayCard.tsx` ne retombe plus sur l'enum brut pour une row legacy non mappée
- [x] T17 réel local CLOSED / PASS : 6/6 scénarios destructifs contre une vraie stack Supabase locale (jamais planifié, planifié puis supprimé, REST explicite, force+Nutrition, DH+Technique, précédence protocole de course)
- [x] Précédence protocole de course prouvée empiriquement : `final_session` = `RACE_ACTIVITY` (coarse `RACE_PREP`) tandis que `planned_session_before` (riche et coarse, mémoire et persisté) reste l'intention athlète brute `DH_TECHNICAL`/`MODERATE`
- [x] Chaque décision T17 identifiée via `result.persistence.decision_id`, jamais par ordre non spécifié
- [x] Durcissement infrastructure de test partagée : `testDb.ts`'s `createTestClient()` refuse toute cible autre que `http://127.0.0.1:54321`/`http://localhost:54321` avant toute mutation ; preuve négative empirique contre le projet de production réel
- [x] Tests finaux : `web` 485 pass/9 skip par défaut ; `web` opt-in complet 494/494 ; `head-coach-engine` avec clé locale sans opt-in T17 378 pass/6 skip ; avec opt-in T17 384/384 ; edge 9/9
- [x] Aucune migration, aucun changement `head-coach-engine/src/**`, aucune action Supabase explicite ; aucune action de déploiement Vercel explicite dans ce jalon — implémenté sur `origin/main`, état effectif en production non vérifié (V0.3_003E)

### Explicitement hors périmètre V0.3_003D (reporté à la slice suivante)

Tout déploiement production, vérification `/plan`/`/today` en production, canary Planning remote — restent V0.3_003E.

### Dette de test-infrastructure notée (non un blocker V0.3_003D)

Les fichiers d'intégration `head-coach-engine` pré-existants (`buildRawContext.integration.test.ts`, `runDailyFor.integration.test.ts`, etc.) requièrent toujours une clé serveur locale et n'ont pas tous le comportement d'opt-in explicite introduit par T17/`testDbSafety.test.ts` — un `npm test` sans clé sur ces fichiers échoue plutôt que de passer en self-contained. Dette pré-existante, non introduite par V0.3_003D ; une extension de la garde/opt-in à ces fichiers reste un futur durcissement optionnel, pas un jalon dédié.
...
- [x] T18 canary remote authentifié CLOSED / PASS : écriture `planned_sessions` sous RLS réelle (client scratch authentifié, jamais `service_role`) + `daily-run` de production invoqué sous JWT scratch réel (jamais `service_role`) + persistance vérifiée via l'id de décision exact retourné par l'endpoint
- [x] Nettoyage scratch complet, zéro résidu vérifié ; zéro écriture sur l'athlète réel
- [x] Smoke web production (bundle + routes directes + canary authentifié bout-en-bout) ; smoke navigateur visuel non automatisé (non requis pour T18)
- [x] Aucune migration, aucun redéploiement Edge, aucun changement `head-coach-engine/src/**`, `ENGINE_VERSION` inchangé
- [x] **V0.3_003 — Planning / Session Intent dans son ensemble : COMPLETE (2026-09-02)**

### Note d'infrastructure de build (retenue pour référence future)

Le build web de production utilise désormais un typecheck TypeScript dédié (`web/tsconfig.build.json`, extension de `tsconfig.app.json` excluant les fichiers `*.test.ts(x)`) tandis que le typecheck local complet (`npm run typecheck`) continue de couvrir l'intégralité du repo, y compris les tests cross-package important `head-coach-engine/**`. Tout futur test `web/src/**` qui importerait à nouveau hors de la frontière `web/` doit rester couvert par le typecheck local complet, jamais par le graphe de build de production.
...
## V0.3_007B — Performed Session Correctness Foundation (CLOSED / PRODUCTION ROLLOUT COMPLETE, 2026-09-10)

Suite directe de l'investigation V0.3_007A/V0.3_007A2 ci-dessus. Corrige trois problèmes concrets sans construire un nouveau système : `intervention` (le vocabulaire `TrainingIntervention` riche, REV-003) devient le fait authoritative pour `done`/`partial`/`replaced` (résout la collision `STRENGTH_LOWER`/`STRENGTH_UPPER`/`POWER`/`GRIP_WORK` sur `STRENGTH_A`/`STRENGTH_B`) ; `recentLoad` exclut désormais correctement les séances `skipped` du calcul de charge 7 jours ; la liaison décision↔session ambiguë même-jour a désormais un mécanisme explicite (`loadValidDecisionsForDate`, sélecteur 0/1/2+), remplaçant l'ancien `onLiveContextChange` jugé sémantiquement dangereux ; une identité `SKIPPED` liée à une décision est dérivée de la prescription et verrouillée tant que le lien persiste. **Deux hotfix de production** ont corrigé un défaut réel de préservation (sélectionner/changer le plan lié écrasait une activité performée déjà saisie — corrigé en `ff7636f` ; la transition spécifique "lien non résolu → première sélection" a ensuite été prouvée exhaustivement non défectueuse en `21d161b`, test-only). **REV-003 est CLOSED côté athlete-facing** — l'athlète peut désormais enregistrer fidèlement une activité performée riche. Détail complet : `docs/11_DECISION_LOG.md` (2026-09-10, V0.3_007B). Preuve production complète (canary athlète scratch), déployé sur web/`completed-session`/`daily-run`, aucune migration DB/RLS/RPC.
...
**V0.3_008B0 — Personal Focus / Execution Task Compatibility (CLOSED / PRODUCTION ROLLOUT COMPLETE)** : précondition bloquante de V0.3_008B, découverte par sa propre investigation — `resolveDhExecutionTask` rendait `execution_task` systématiquement absent dès qu'un focus personnel était configuré (cas réel de Louis), rendant `technical_outcome` inaccessible. Classifié CROSS-SLICE CONTRACT MISMATCH (V0.3_006C1 vs V0.3_007C), corrigé : `focus`/`execution_task` sont désormais des concepts indépendants, la tâche générique par kind coexiste avec le focus personnel sans jamais en dériver le contenu (invariant anti-inférence inchangé, prouvé en local ET en production par égalité byte-exacte). UI : labels explicites « Focus : »/« Tâche du jour : » ajoutés, confirmés dans le bundle servi. Aucun DB/RPC/Edge, `completed-session` jamais redéployé. `ENGINE_VERSION → head-coach-engine@0.2.0-m1-v0.3_008b0`. Commit `60d40e1`, déployé (web puis `daily-run` v8→9), canary production PASS (`technical_outcome=partial` accepté par le `completed-session` existant), nettoyage vérifié résidu zéro. Détail complet : `docs/11_DECISION_LOG.md` (V0.3_008B0). **Débloque V0.3_008B**, dont l'architecture est désormais verrouillée (voir ci-dessous).

### V0.3_008B — Continuité technique (CLOSED / PRODUCTION ROLLOUT COMPLETE)
Rend une séance DH du jour consciente de la dernière tâche d'exécution technique persistée (`decisions.daily_plan.dh_or_technical.execution_task`) et du `technical_outcome` athlete-reported correspondant (option B de l'investigation V0.3_008), réellement atteignable pour Louis depuis V0.3_008B0. **Différent de V0.3_008A par nature, pas seulement par périmètre** : première lecture `decisions` par le Head Coach dans le pipeline `daily-run`, désormais livrée. `ENGINE_VERSION → head-coach-engine@0.2.0-m1-v0.3_008b`, déployé (`e7623ff`, web + `daily-run` v9→10), canary production PASS (immuabilité de l'instantané prouvée contre une vraie correction de source). Correction de gate final avant commit : `completion_status` re-vérifié défensivement (`done`/`partial` uniquement). Détail complet : `docs/11_DECISION_LOG.md` (2026-09-14, V0.3_008B). **Progression technique automatique (répéter/simplifier/progresser) explicitement différée — prochaine direction : SERIOUS DOGFOOD / LONGITUDINAL COACHING LOOP, pas encore un jalon numéroté.**

**Architecture et preuve complètes** : voir `docs/11_DECISION_LOG.md` (2026-09-14, V0.3_008B) — source exact-ID, fenêtre stricte D-14..D-1, `LIMIT 14` dérivé de `unique_completed_per_day`, sémantique d'échec énumérée, DISPLAY ONLY, copie UI neutre, preuve d'immuabilité locale et production. Non dupliqué ici.
...
### DOG-001 — Nutrition suit `final_session`, jamais un `planned_session` remplacé (CLOSED / PRODUCTION ROLLOUT COMPLETE, 2026-09-15)

Premier bug réel remonté par la phase **SERIOUS DOGFOOD — LONGITUDINAL COACHING LOOP** ouverte par V0.3_008B ci-dessus. `computeNutritionDomain` lisait `planned_session.kind` (prescription brute) au lieu de `final_session.kind` (déjà arbitré) dans sa branche force, contrairement à sa branche sœur DH — une note de séance de force pouvait donc rester affichée après un `REPLACE` de l'arbitrage Training vers `RECOVERY_ACTIVE`. Documenté à tort comme une asymétrie intentionnelle (`nutrition.ts`, `docs/06_ARCHITECTURE.md` §V0.3_002D) — corrigé aux deux endroits. Fix : `plannedSession` retiré de la signature de `computeNutritionDomain`. `ENGINE_VERSION → head-coach-engine@0.2.0-m1-v0.3_008b-dog001`. Commit `9c4ee23` poussé sur `origin/main`. Déployé : `daily-run` uniquement (`uvolpldwwyvadlamulvr`, version 10→11) ; `completed-session`/web/DB/RPC non touchés, aucune migration. Détail complet : `docs/11_DECISION_LOG.md` (2026-09-15, DOG-001).

### DOG-002 — POST_EVENT hard face à un état GREEN + prochaine course éloignée (INVESTIGATION SEULE — NEEDS MORE EVIDENCE, 2026-09-15)

Second point d'observation du dogfood. T+2 après Verbier iXS, état matinal excellent (toutes dimensions GREEN), Planning `STRENGTH_LOWER`/MODERATE, prochaine course (St-Luc) à ~13 jours — décision produite `REPLACE → RECOVERY_ACTIVE`. Investigation (lecture seule) : comportement conforme au contrat actuel de `raceProtocol.ts` (`POST_EVENT` = recommandation hard inconditionnelle, `today − event_end ≤ 2j`, ne lit ni les dimensions ni `planned_session`) ; St-Luc n'entre même pas dans les courses candidates (`preEventWindowDays=7` dans `provisionalThresholds.ts`). **Classifié EXPECTED LIMITATION / POLICY QUESTION, pas un bug.** Aucun changement de code — un seul point d'observation est insuffisant pour distinguer règle trop conservatrice de protection légitime contre un biais de sur-optimisme post-course. **N'est PAS un ticket d'implémentation** : aucune piste (post-event soft/hard, `days_to_next_race`, fenêtre intermédiaire) n'est engagée ni planifiée — nécessite d'autres occurrences réelles T+1/T+2 avant toute décision d'architecture. Détail complet : `docs/11_DECISION_LOG.md` (2026-09-15, DOG-002).
...
- Planificateur hebdomadaire **automatique** (`planned_sessions` généré automatiquement sans saisie athlète — distinct de V0.3_003, qui est une saisie manuelle athlète ; reste ici tant qu'aucune décision de génération auto n'est architecturée)
