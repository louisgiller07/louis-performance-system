...
**V0.3_008B0 — Personal Focus / Execution Task Compatibility (IMPLÉMENTÉ LOCALEMENT, pas encore CLOSED)** : précondition bloquante de V0.3_008B, découverte par sa propre investigation — `resolveDhExecutionTask` rendait `execution_task` systématiquement absent dès qu'un focus personnel était configuré (cas réel de Louis), rendant `technical_outcome` inaccessible. Classifié CROSS-SLICE CONTRACT MISMATCH (V0.3_006C1 vs V0.3_007C), corrigé : `focus`/`execution_task` sont désormais des concepts indépendants, la tâche générique par kind coexiste avec le focus personnel sans jamais en dériver le contenu (invariant anti-inférence inchangé). UI : labels explicites « Focus : »/« Tâche du jour : » ajoutés. Aucun DB/RPC/Edge. `ENGINE_VERSION → head-coach-engine@0.2.0-m1-v0.3_008b0`. Aucun commit/push/déploiement — en attente de revue. Détail complet : `docs/11_DECISION_LOG.md` (V0.3_008B0). **Débloque V0.3_008B** (candidats réels existeront désormais pour Louis) sans l'implémenter.
