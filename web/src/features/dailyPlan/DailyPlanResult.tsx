...
// M4_005 — production rendering of a real DailyPlan. Every value shown
// comes straight from the server response; this file only decides layout
// and French labels (dailyPlanLabels.ts) — it never classifies, diagnoses,
// or invents a coaching/safety recommendation.
    ...
    <div className="flex flex-col gap-3">
      ...
      {hasSafetySignal && (
        <div className="rounded-lg border border-red-300 bg-red-50 p-3">
          ...
          <p className="mt-0.5 text-xs text-red-600">
            {dailyPlan.health_flag_to_create?.reason ?? "Le coach a généré un signal de santé pour cette décision."}
          </p>
        </div>
      )}
      ...
      {import.meta.env.DEV && (
        <details className="text-xs text-gray-400">
          ...
          <p className="mt-1">decisionId : {decisionId}</p>
          <p>engine_version : {dailyPlan.engine_version}</p>
